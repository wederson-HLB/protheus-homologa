...
*----------------------------------------------------------------*
WsService GTFLG001 Description "Integracao GT de Fluig x Protheus "
*----------------------------------------------------------------*
...
WsData aRetPV		as Array of GETPVRET
WsData aRetPVITEM   as Array of GETPVITEMRET
WsData aRetPvCab    as Array of GETPVCABRET
WsData aRetVendor	as Array of GETVENRET
...
WsMethod GETPV 	Description "Retorna os pedidos de vendas liberados."
WsMethod GETPVCAB 	Description "Retorna o cabe�alho dos pedidos de vendas."
WsMethod GETPVITEM Description "Retorna os itens do  pedido de venda."
WsMethod GETVENDOR	Description "Retorna o cadastro de vendedores de acordo com o CNPJ da empresa."
...
WSDATA cFciCod	as String
WSDATA cCodIss	as String

ENDWSSTRUCT

WSSTRUCT GETPVCABRET
*----------------*

WSDATA C5_TIPO	as String
WSDATA C5_P_NUMPV	as String
WSDATA C5_XTIPO	as String
...
WSDATA C5_ESPECI1	as String
WSDATA C5_VOLUME1	as Float
WSDATA C5_PESOL	as Float
WSDATA C5_MENNOTA 	as String

WSDATA E4_TIPO    as String  //- Tipo da condi��o de pagamento
WSDATA E4_DESCRI as String   //- Descri��o da condi��o de pagamento
WSDATA C5_XORDEM as String
WSDATA A4_NOME  as String    //- Nome da transportadora
WSDATA C5_PBRUTO as Float    //- Peso bruto
...
WSSTRUCT GETVENRET
*----------------*
WSDATA Vendedores as Array of stDadoGet
ENDWSSTRUCT
...
While TMP->(!EOF())
	...
	aTail(::aRetAmb):Ambiente	:= ALLTRIM(TMP->AMB)//Iif("HOM" $ cDbAlias,ALLTRIM(TMP->AMB)+"_HOM",ALLTRIM(TMP->AMB))
...
If Len(aCab) == 0 .OR. Len(aItens) == 0
	aAdd(::aRetSts,WSClassNew("SETSTSRET"))
	aTail(::aRetSts):Codigo		:= "Erro"
	aTail(::aRetSts):Descricao	:= "Dados de entrada invalidos."
	RETURN .T.
EndIf

If (nPosPv := aScan(aCab,{|x| x[1] == "C5_NUM"}) ) > 0
	cNumPv:= aCab[nPosPv][2]
EndIf
...
BEGIN SEQUENCE
    ...
    If Empty(cNumPv) 
		...
		If lMsErroAuto
			...
			aAdd(::aRetSts,WSClassNew("SETSTSRET"))
	    	aTail(::aRetSts):Codigo		:= "Erro"
	    	aTail(::aRetSts):Descricao	:= "Dados do pedido inv�lidos. "+cErroCon
      
        	RETURN .T.
		Else
			cPedido := SC5->C5_NUM
			//Apagfa o conteudo do campo FCI quando esta em branco no Fluig.(quando em branco, o fluig envia um ponto apenas)
			If (TCSQLEXEC("UPDATE "+RetSqlName("SC6")+" SET C6_FCICOD='' WHERE C6_FILIAL = '"+xFilial("SC6")+"' AND C6_NUM = '"+cPedido+"' AND C6_FCICOD='.'")) <0
				aAdd(::aRetSts,WSClassNew("SETSTSRET"))
        		aTail(::aRetSts):Codigo		:="Erro"
        		aTail(::aRetSts):Descricao	:= TCSQLError()
            Else
				aAdd(::aRetSts,WSClassNew("SETSTSRET"))
        		aTail(::aRetSts):Codigo		:= If(!Empty(cErro),"Erro","Ok")
        		aTail(::aRetSts):Descricao	:= If(!Empty(cErro),cErro,"Pedido '" + cPedido + "' inclui�do com sucesso.")
            EndIf
		EndIf
		
    Else

        aCab[nPosPv][2] := cNumPv
		
		SC5->(dbSetOrder(1))
		If SC5->(dbSeek(xFilial("SC5")+cNumPv))
		   conout("GTFLG001: SETPSEMNF - Encontrou: "+cNumPv)
        ...
        Else
		    aAdd(::aRetSts,WSClassNew("SETSTSRET"))
	    	aTail(::aRetSts):Codigo		:= "Erro"
	    	aTail(::aRetSts):Descricao	:= "Pedido n�o encontrado: "+cNumPv
		EndIf 
	...
	Else
            ...
            Else
				aAdd(::aRetSts,WSClassNew("SETSTSRET"))
        		aTail(::aRetSts):Codigo		:= If(!Empty(cErro),"Erro","Ok")
        		aTail(::aRetSts):Descricao	:= If(!Empty(cErro),cErro,"Pedido '" + cPedido + "' altera�do com sucesso.")
            EndIf
	EndIf
...
*-------------------------------------------------------------------------*
WsMethod GETPV WsReceive cChave wsSend aRetPV WsService GTFLG001
*-------------------------------------------------------------------------*
...
cCodEmp := aEmp[1]
cCodfil := aEmp[2]//Right(aEmp[2],2)
cCodAmb := aEmp[3]

cQry := "SELECT C5_NUM "
cQry += " From "+cCodAmb+".dbo.SC5"+cCodEmp+"0 "
cQry += "WHERE D_E_L_E_T_ = '' "
cQry += "AND C5_FILIAL = '"+cCodFil+"' "
cQry += "AND C5_NOTA = '' "
cQry += "AND C5_BLQ = '' "
...
If TMP->(!EOF())
	While TMP->(!EOF())
		aAdd(::aRetPv,WSClassNew("GETPVRET"))
		aTail(::aRetPV):Codigo 	:= ALLTRIM(TMP->C5_NUM)
		TMP->(DbSkip())
	EndDo
Else
	aAdd(::aRetPv,WSClassNew("SETSTSRET"))
	aTail(::aRetPv):Codigo	:= "Sem pedido(s) em aberto."
EndIf

Return .T.   

*-------------------------------------------------------------------------------*
WsMethod GETPVCAB WsReceive cChave,cNumPV wsSend aRetPvCab WsService GTFLG001   
*-------------------------------------------------------------------------------*
Local aDado := {}
...
cQry :="SELECT C5_TIPO,C5_P_NUMPV,C5_XTIPO,C5_CONDPAG,C5_CLIENTE,C5_LOJACLI,C5_PBRUTO,C5_XORDEM,A1_TIPO,A1_PESSOA,A1_CGC,A1_END,A1_CEP,A1_BAIRRO,A1_EST,A1_MUN,A1_CODMUN,A1_PAIS,A1_INSCR,A1_EMAIL "
cQry += ",A1_NOME,A1_ENDCOB,A1_CEPC,A1_BAIRROC,A1_MUNC,A1_ESTC,A1_ENDENT,A1_CEPE,A1_BAIRROE,A1_MUNE,A1_ESTE,C5_TRANSP,C5_TPFRETE,C5_ESPECI1,C5_VOLUME1,C5_PESOL,C5_MENNOTA,E4_TIPO, E4_DESCRI, A4_NOME,A4_CGC "
cQry +=" ,A4_END,A4_CEP,A4_BAIRRO,A4_COD_MUN,A4_MUN,A4_EST "
cQry += "FROM "+cCodAmb+".dbo.SC5"+cCodEmp+"0 SC5, "+cCodAmb+".dbo.SA1"+cCodEmp+"0  SA1, "+cCodAmb+".dbo.SE4"+cCodEmp+"0  SE4, "+cCodAmb+".dbo.SA4"+cCodEmp+"0  SA4 "
...
cQry += "AND C5_TRANSP = A4_COD "
cQry += "AND C5_NUM = '"+cNumPV+"' "
...
TMP->(DBGoTop())
While TMP->(!EOF())
	aAdd(::aRetPvCab,WSClassNew("GETPVCABRET"))

 	aTail(::aRetPvCab):C5_TIPO	  :=TMP->C5_TIPO
 	aTail(::aRetPvCab):C5_P_NUMPV :=TMP->C5_P_NUMPV
 	aTail(::aRetPvCab):C5_XTIPO	  :=TMP->C5_XTIPO
 	aTail(::aRetPvCab):C5_CONDPAG :=TMP->C5_CONDPAG
 	aTail(::aRetPvCab):C5_CLIENTE :=TMP->C5_CLIENTE
 	aTail(::aRetPvCab):C5_LOJACLI :=TMP->C5_LOJACLI
 	aTail(::aRetPvCab):A1_TIPO	  :=TMP->A1_TIPO
 	aTail(::aRetPvCab):A1_PESSOA  :=TMP->A1_PESSOA
 	aTail(::aRetPvCab):A1_CGC	  :=TMP->A1_CGC
 	aTail(::aRetPvCab):A1_END	  :=TMP->A1_END
 	aTail(::aRetPvCab):A1_CEP	  :=TMP->A1_CEP
 	aTail(::aRetPvCab):A1_BAIRRO  :=TMP->A1_BAIRRO
 	aTail(::aRetPvCab):A1_EST	  :=TMP->A1_EST
 	aTail(::aRetPvCab):A1_MUN	  :=TMP->A1_MUN
 	aTail(::aRetPvCab):A1_CODMUN  :=TMP->A1_CODMUN
 	aTail(::aRetPvCab):A1_PAIS	  :=TMP->A1_PAIS
 	aTail(::aRetPvCab):A1_INSCR	  :=TMP->A1_INSCR
 	aTail(::aRetPvCab):A1_EMAIL   :=TMP->A1_EMAIL
 	aTail(::aRetPvCab):A1_NOME	  :=TMP->A1_NOME
 	aTail(::aRetPvCab):A1_ENDCOB  :=TMP->A1_ENDCOB
 	aTail(::aRetPvCab):A1_CEPC	  :=TMP->A1_CEPC
 	aTail(::aRetPvCab):A1_BAIRROC :=TMP->A1_BAIRROC
 	aTail(::aRetPvCab):A1_MUNC	  :=TMP->A1_MUNC
 	aTail(::aRetPvCab):A1_ESTC	  :=TMP->A1_ESTC
 	aTail(::aRetPvCab):A1_ENDENT  :=TMP->A1_ENDENT
 	aTail(::aRetPvCab):A1_CEPE	  :=TMP->A1_CEPE
 	aTail(::aRetPvCab):A1_BAIRROE :=TMP->A1_BAIRROE
 	aTail(::aRetPvCab):A1_MUNE	  :=TMP->A1_MUNE
 	aTail(::aRetPvCab):A1_ESTE	  :=TMP->A1_ESTE
 	aTail(::aRetPvCab):C5_TRANSP  :=TMP->C5_TRANSP
 	aTail(::aRetPvCab):C5_TPFRETE :=TMP->C5_TPFRETE
 	aTail(::aRetPvCab):C5_ESPECI1 :=TMP->C5_ESPECI1
 	aTail(::aRetPvCab):C5_VOLUME1 :=TMP->C5_VOLUME1
 	aTail(::aRetPvCab):C5_PESOL	  :=TMP->C5_PESOL
 	aTail(::aRetPvCab):C5_MENNOTA :=TMP->C5_MENNOTA

	aTail(::aRetPvCab):E4_TIPO    :=TMP->E4_TIPO
    aTail(::aRetPvCab):E4_DESCRI  :=TMP->E4_DESCRI
    aTail(::aRetPvCab):C5_XORDEM  :=TMP->C5_XORDEM
    aTail(::aRetPvCab):A4_NOME    :=TMP->A4_NOME
    aTail(::aRetPvCab):C5_PBRUTO  :=TMP->C5_PBRUTO
...
*-------------------------------------------------------------------------------*
WsMethod GETPVITEM WsReceive cChave,cNumPV wsSend aRetPVITEM WsService GTFLG001   
*-------------------------------------------------------------------------------*
Local aDado := {}
...
cQry := "Select C6_NUM,C6_ITEM,C6_PRODUTO,C6_DESCRI,C6_QTDVEN,C6_QTDLIB,C6_PRCVEN, C6_UM,C6_LOCAL,C6_LOTECTL,C6_NFORI,C6_SERIORI,C6_ITEMORI,C6_TES,C6_CCUSTO,C6_FCICOD,C6_CODISS "
cQry += "From "+cCodAmb+".dbo.SC6"+cCodEmp+"0 "
cQry += "WHERE D_E_L_E_T_ = '' "
cQry += "AND C6_FILIAL = '"+cCodFil+"' "
...
TCSetField( "TMP", "C6_QTDVEN", TamSX3("C6_QTDVEN")[3],TamSX3("C6_QTDVEN")[1] , TamSX3("C6_QTDVEN")[2] )
TCSetField( "TMP", "C6_QTDLIB", TamSX3("C6_QTDLIB")[3],TamSX3("C6_QTDLIB")[1] , TamSX3("C6_QTDLIB")[2] )
TCSetField( "TMP", "C6_PRCVEN", TamSX3("C6_PRCVEN")[3],TamSX3("C6_PRCVEN")[1] , TamSX3("C6_PRCVEN")[2] )

TMP->(DBGoTop())
While TMP->(!EOF())
	aAdd(::aRetPvITEM,WSClassNew("GETPVITEMRET"))
	aTail(::aRetPVITEM):cPedido	  := ALLTRIM(TMP->C6_NUM)
	aTail(::aRetPVITEM):cItem	  := ALLTRIM(TMP->C6_ITEM)
	aTail(::aRetPVITEM):cProduto  := ALLTRIM(TMP->C6_PRODUTO)
	aTail(::aRetPVITEM):cDescri	  := ALLTRIM(TMP->C6_DESCRI)
	aTail(::aRetPVITEM):nQtdVen	  := TMP->C6_QTDVEN
	aTail(::aRetPVITEM):nQtdLib	  := TMP->C6_QTDLIB
	aTail(::aRetPVITEM):nPrcVen	  := TMP->C6_PRCVEN
	aTail(::aRetPVITEM):cUm		  := ALLTRIM(TMP->C6_UM)
	aTail(::aRetPVITEM):cLocal	  := ALLTRIM(TMP->C6_LOCAL)
	aTail(::aRetPVITEM):cLoteCtl  := ALLTRIM(TMP->C6_LOTECTL)
	aTail(::aRetPVITEM):cNfori	  := ALLTRIM(TMP->C6_NFORI)
	aTail(::aRetPVITEM):cSeriOri  := ALLTRIM(TMP->C6_SERIORI)
	aTail(::aRetPVITEM):cItemOri  := ALLTRIM(TMP->C6_ITEMORI)
	aTail(::aRetPVITEM):cTes	  := ALLTRIM(TMP->C6_TES)
	aTail(::aRetPVITEM):cCcusto	  := ALLTRIM(TMP->C6_CCUSTO)
	aTail(::aRetPVITEM):cFciCod	  := ALLTRIM(TMP->C6_FCICOD)
	aTail(::aRetPVITEM):cCodIss	  := ALLTRIM(TMP->C6_CODISS)
...
WsMethod GETVENDOR WsReceive cChave,DadosGet wsSend aRetVendor WsService GTFLG001
*--------------------------------------------------------------------------------*
Local i, aCampos := {}, aDado := {}

BEGIN SEQUENCE
	If ValType(::DadosGet:Dado) <> "A"
		aAdd(::aRetVendor,WSClassNew("GETCLIRET"))
		aAdd(aDado,WSClassNew("stDadoGet"))
		aTail(aDado):Campo	:= "Erro: Estrutura de dados não compatível."
		aTail(::aRetVendor):Vendedores	:= aDado
		Break
	EndIf

	If Len(aEmp := BuscaEmpresas(cChave)) == 0
		aAdd(::aRetVendor,WSClassNew("GETVENRET"))
		aAdd(aDado,WSClassNew("stDadoGet"))
		aTail(aDado):Campo	:= "Erro: CNPJ informado não pertence a nenhuma empresa do Sistema."
		aTail(::aRetVendor):Vendedores	:= aDado
		Break
	...
	cQry := " Select * "
	cQry += " From " + aEmp[3] + ".dbo." + RetTabName("SA3") + " "
	cQry += " Where D_E_L_E_T_ <> '*' And A3_MSBLQL <> '1' "
...
Begin Sequence
	...
	If EMP->(!Eof())
		...
		aAdd(aEmp,ALLTRIM(EMP->AMB))///Iif("HOM" $ cDbAlias,ALLTRIM(EMP->AMB)+"_HOM",ALLTRIM(EMP->AMB)))
...
Else
	TCQuery "Select OBJECT_ID('P12_01_HOM.dbo.GTFLG001SX2') as ID" ALIAS "ID" NEW   //HOMOLOGA
Endif	
If ID->ID <= 0
	If Subs(Upper(AllTrim(GetEnvServer())),1,3) == "P11"
		TCSQLEXEC("CREATE TABLE P11_01.dbo.GTFLG001SX2 (CODEMP varchar(2),X2_CHAVE varchar(3),X2_ARQUIVO varchar(6),DT_ATU varchar(8))")
    Else	
		TCSQLEXEC("CREATE TABLE P12_01_HOM.dbo.GTFLG001SX2 (CODEMP varchar(2),X2_CHAVE varchar(3),X2_ARQUIVO varchar(6),DT_ATU varchar(8))")
		//HOMOLOGA
	Endif	
EndIf
...
Else
	cQry := " Select * From P12_01_HOM.dbo.GTFLG001SX2 where CODEMP='"+aEmp[1]+"' AND X2_CHAVE='"+cAlias+"' "
	//HOMOLOGA
Endif
...
If TMPSX2->(!EOF())
	If DateDiffDay(STOD(TMPSX2->DT_ATU),DATE() ) > 7
		...
		Else
			TCSQLExec("Delete P12_01_HOM.dbo.GTFLG001SX2 where CODEMP='"+aEmp[1]+"' AND X2_CHAVE='"+cAlias+"'")
			//HOMOLOGA
		Endif	
	Else
