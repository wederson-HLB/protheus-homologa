...
#DEFINE MAXITEMC  018//012                                           // Máxima de caracteres por linha de produtos/serviços
...
Else
	//RRP - 19/07/2018 - Utilizado para deixar as configurações de impressao na rotina externa
	If StrTran(Alltrim(UPPER(FunName())),'U_','') == "SPEDNFE"
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8])$"2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							Else
								If SM0->M0_CODIGO == "EQ" //Salton
								    //Gerar o arquivo TXT da nota.
								    TXTNFSAID(aNotas[nX][05],aNotas[nX][04])
								EndIf
								dbSelectArea("SF2")
								dbSetOrder(1)
...
Local aItensAux     := {}
Local aArray		:= {}
Local lExcPe02		:= .F. //RRP - 01/08/2018 - Valida se executou o PE02DANFE

Default cDtHrRecCab := ""
Default dDtReceb    := CToD("")

Private aInfNf      := {}
Private aAuxCabec     := {} // Array que conterá as strings de cabeçalho das colunas de produtos/serviços.
...
EndIf
//Looping em todos os itens para saber se há controle de lote.
For nZ := 1 To nLenDet
	...
	If SM0->M0_CODIGO == 'U2' //Dr. Reddy's
		If Type("oDET[nPrivate]:_PROD:_MED") <> "U" 
			If Type("oDET[nPrivate]:_PROD:_MED:_NLOTE:TEXT") <> "U"
				If !Empty(oDET[nX]:_PROD:_MED:_NLOTE:TEXT)   
					lLote := .T.
				EndIf
			EndIf
		EndIf

	ElseIf SM0->M0_CODIGO $ 'FF|IS|HM|CZ' //Sumitomo, Promega, Angelmed e Ceres
	...
	ElseIf SM0->M0_CODIGO $ 'LN' //Neogen
...
//
	//InfinIT Tecnologia -----------------------------------------------
	//
	//RRP - 23/07/2014 - Inclusão empresa Exeltis. Chamado 020282    
	//RSB - 18/08/2017 - Inclusão empresa Exeltis. Re-Implantação    
	If cEmpAnt $ 'JK/SU/LG' //Mabra/Exeltis

		cLote:= 0
		nPMC := 0
		dVal := 0
		//RRP - 07/08/2018 - Inclusão do lote quando não for medicamento e ajuste na customização da Infinit
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" //Lote
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					cLote	:= Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),6,AT("Validade: ",oDet[nX]:_InfAdProd:TEXT)-7)
					dVal	:= Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),AT("Validade: ",oDet[nX]:_InfAdProd:TEXT)+10,10)   
					//CAS - 20/09/2018 - Ticket #46218 Exeltis: data de validade incompleta - Tratamento para formato de datas diferentes(ano com 2 ou 4 caracteres). 
					If Len(dVal) == 10
						dVal := SubStr(dVal,1,6)+SubStr(dVal,9,2)
					EndIF 	
				EndIf
			EndIf
		EndIf
		If Type("oDET[nPrivate]:_PROD:_MED") <> "U"
			If Type("oDet[nPrivate]:_Prod:_med:_vPMC:TEXT")<>"U"
				nPMC:= Val(oDet[nX]:_Prod:_med:_vPMC:TEXT)
			EndIf
		Endif 
	EndIf
	//fim InfinIT--------------------------------------------------------	
	...
	If SM0->M0_CODIGO == 'U2' //Dr. Reddy's
		If Type("oDET[nPrivate]:_PROD:_MED") <> "U" 
			If Type("oDET[nPrivate]:_PROD:_MED:_NLOTE:TEXT") <> "U"
				If !Empty(oDET[nX]:_PROD:_MED:_NLOTE:TEXT)   
					cLote := Alltrim(oDET[nX]:_PROD:_MED:_NLOTE:TEXT)  
				EndIf
			EndIf
		EndIf

	ElseIf SM0->M0_CODIGO == 'FF|LN' //Sumitomo e Neogen
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" //Lote
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					If At("#",oDet[nX]:_InfAdProd:TEXT) > 0
	...
	ElseIf SM0->M0_CODIGO == 'PL' //Polaris
	...
	ElseIf SM0->M0_CODIGO == "ED" //Okuma
	 ...
	 ElseIf SM0->M0_CODIGO == "EI" //ACCEDIAN
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" 
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					cCompl 	 := Alltrim(oDet[nX]:_InfAdProd:TEXT)
	...
	//RRP - 23/07/2014 - Inclusão empresa Exeltis. Chamado 020282 
	//RSB - 18/08/2017 - Inclusão empresa Exeltis. Re-Implantação    
	If SM0->M0_CODIGO $ "JK/SU/LG" // MABRA/Exeltis - InfinIT Tecnologia
		aAux := {}
		...
		AADD(aAux, AllTrim(cLote))
		AADD(aAux, AllTrim(dVal))
		AADD(aAux, AllTrim(TransForm(nPMC,TM(nPMC,TamSX3("CD7_PRECO")[1],TamSX3("CD7_PRECO")[2]))))
		AADD(aAux, AllTrim(TransForm(nBaseICM,TM(nBaseICM,TamSX3("D2_BASEICM")[1],TamSX3("D2_BASEICM")[2]))))
		AADD(aAux, AllTrim(TransForm(nDesc,TM(nDesc,TamSX3("D2_DESCON")[1],TamSX3("D2_DESCON")[2]))))   //	AADD(aAux, AllTrim(TransForm(nBaseICMST,TM(nBaseICMST,TamSX3("D2_BASEICM")[1],TamSX3("D2_BASEICM")[2]))))
	...
	//Tratamento customizado para os clientes.
	If SM0->M0_CODIGO $ 'U2|FF|LN|CN|HM' //Dr. Reddy's, Sumitomo, Neogen e Ceres, Angelmed
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
		...
		Endif
		//Tratamento customizado para os clientes.
		If SM0->M0_CODIGO $ 'U2|FF|LN|CN|HM'
		...
		EndIf
	EndDo
	//RRP - 23/06/2014 - Ajuste para impressão do Lote e validade.	
...
aAux := {{{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}}}
//Tratamento customizado para os clientes.
If SM0->M0_CODIGO $ 'U2|FF|LN|CN'
	aAdd(aAux[1],{})

ElseIf SM0->M0_CODIGO $ 'HM'
...
If SM0->M0_CODIGO $ 'U2|FF|LN|CN' //Dr. Reddy's, Sumitomo, Neogen e Ceres
...
ElseIf SM0->M0_CODIGO $ "HM" //Angelmed
...
//RRP - 23/07/2014 - Inclusão empresa Exeltis. Chamado 020282 
//RSB - 18/08/2017 - Inclusão empresa Exeltis. Re-Implantação    
ElseIf SM0->M0_CODIGO $ "JK/SU/LG" // Mabra/Exeltis - InfinIT Tecnologia 
...
For nY := 1 To nLenItens
	...
	Else
		For nI:=1 To Len(aAuxCabec)
			oDanfe:Say(nLinha, aColProd[nI][1] + 2, aAux[1][nI][nY], oFont07:oFont)
		Next
		/*
...
If nFolha == 1
	...
	nAux     := nBaseCol + 30
	For nI:=1 To Len(aAuxCabec)
		AADD(aColProd, {nAux, nAux + aTamCol[nI]})
		nAux += aTamCol[nI]
	Next
...
//	AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "PER DESC"
 //	nAux += aTamCol[10]
	...
	*/
	...
	nAux := nBaseCol + 30
	For nI:=1 To Len(aAuxCabec)
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[nI])
		oDanfe:Say(nLine+462, nAux + 2,aAuxCabec[nI], oFont08N:oFont)
		nAux += aTamCol[nI]
	Next	
