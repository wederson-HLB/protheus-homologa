...
/*/
//RRP - 14/09/2017 - Parâmetro lView é utilizado no fonte GTFAT010.
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2,	oDanfe,;
						oSetup, cFilePrint	, lIsLoja, lView , lCallExt	)
...
/*
	* Leandro Brito - Utilizado para deixar as configurações de impressao na rotina externa
*/
If lCallExt
	Private PixelX := odanfe:nLogPixelX()
	Private PixelY := odanfe:nLogPixelY()	
	DANFEProc(@oDanfe, .F. , cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)
	Return( lExistNFe ) 
EndIf
...
EndIf
If lView//JVR - 22/09/2014 - Criação da variavel para ser utilizada em chamadas externas e não exibir o Arquivo.
If lExistNFe
...
PRIVATE oFont08NI  := TFontEx():New(oDanfe,"ahorani",07,07,.T.,.T.,.T.)// 5 italic
...
Private lArt488MG := .F.
Private lArt274SP := .F. 
//RSB - 10/08/2017 - Ticket #6361 - No caso da Neogen é necessário a descrição por causa do LOTE e da data de Validade.
If cEmpAnt $ 'LN|U2' 
	nMaxDes := 015
Endif	

//WFA - 17/08/2018 - Alteração na quantidade de caracteres da coluna de descrição do produto. Ticket: #31607
If cEmpAnt $ 'BE' 
	nMaxDes := 048
EndIf
...
//RRP - 03/10/2014 - Ponto de Entrada para tratamento Customizado
If ExistBlock("PE01DANFE")
	lOkGt := ExecBlock("PE01DANFE",.F.,.F.,{oNfe})
EndIf
///////////////////////////////////////////////////////////////
//ECR - 04/12/12 - Tratamento para Kits
If SM0->M0_CODIGO $ "IS|4M|2C" //Promega e Illumina

	If oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT == "1" //Nota de Saída
    	
    	//Verifica se o alias 'TMP' está em uso.
    	If Select("TMP") > 0
    		TMP->(DbCloseArea()) 	
    	EndIf
...
///////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
//ECR - 04/12/12 - Looping para verificar se existe lote.
lLote := .F.
For nZ := 1 To nLenDet 
	...
	ElseIf SM0->M0_CODIGO $ 'LN|HB|U2' // Neogen e Hazera
		
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" //Lote
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					If AT("#",oDet[nX]:_InfAdProd:TEXT) > 0
						If Substr(strtran(Alltrim(oDet[nX]:_InfAdProd:TEXT),"/",""),1,AT("#",oDet[nX]:_InfAdProd:TEXT)) <> ''
							lLote := .T.
						EndIf  
					Else
						lLote := .T.
...
Next
/////////////////////////////////////////////////////////
//Looping  de gravação dos dados dos itens.
For nZ := 1 To nLenDet
	cCompl:="" 
	If lMv_ItDesc
		nX := aIndImp[nZ]
	...
	/////////////////////////////////////////////////////////////////
 	//ECR - 04/12/12 - Tratamento customizado para os clientes.
	If SM0->M0_CODIGO == 'FF' //Sumitomo
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" //Lote
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
	...
	ElseIf SM0->M0_CODIGO $ 'LN|HB|U2' //Neogen, Hazera
		//RRP - 20/07/2016 - Chamado 035068 - Sempre separar com pontilhado cada item da nota.
		If cEmpAnt == 'LN'
			lPontilhado := .T.
		EndIf
		...
		If Type("oDet[nPrivate]:_InfAdProd")<>"U" //Lote
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					If AT("#",oDet[nX]:_InfAdProd:TEXT) > 0
						If Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),1,AT("#",oDet[nX]:_InfAdProd:TEXT)) <> ''
							If AT("|",oDet[nX]:_InfAdProd:TEXT) > 0 .and. cEmpAnt $ 'LN|U2'
								cLote := Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),1,AT("|",oDet[nX]:_InfAdProd:TEXT)-1)
								cDtValLote := Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),AT("|",oDet[nX]:_InfAdProd:TEXT)+1,10)
							Else 
								cLote := Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),1,AT("#",oDet[nX]:_InfAdProd:TEXT)-1)
							Endif
						EndIf  
					...
					Else
				 		If AT("|",oDet[nX]:_InfAdProd:TEXT) > 0 .and. cEmpAnt $ 'LN|U2'
							cLote := Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),1,AT("|",oDet[nX]:_InfAdProd:TEXT)-1)
							cDtValLote := Substr(Alltrim(oDet[nX]:_InfAdProd:TEXT),AT("|",oDet[nX]:_InfAdProd:TEXT))
							If cEmpAnt == 'U2' 
								cDtValLote := Substr(cDtValLote,2,10)
							EndIf
						Else 
	...
	//RRP - 21/07/2014 Empresas - Angelmed, Ceres, Illumina, Dr. Reddys, Meda Phamra, Alliance e inclusão da Steelcase/Equant Brasil/Dexa/Equant Services/ALLIED/MERIT
	ElseIf SM0->M0_CODIGO $ 'PL/6H/HM/CZ/2C/U2/3R/5F/9N/LW/KQ/LX/9P/GX' //Polaris/Chery/Angelmed/Ceres/Illumina/Dr. Reddys/Meda Pharma/Alliance/Steelcase/Equant Brasil/Dexa/Equant Services/ALLIED/MERIT
		...
		If Type("oDet[nPrivate]:_InfAdProd")<>"U"
			If Type("oDet[nPrivate]:_InfAdProd:TEXT")<>"U"
				If !Empty(oDet[nX]:_InfAdProd:TEXT)
					...
					//Verifica se o produto não é um Kit
					If !ProdKit(AllTrim(oDet[nX]:_Prod:_xProd:TEXT))					
						cLote:=Alltrim(oDet[nX]:_InfAdProd:TEXT)
					EndIf   
				EndIf
			EndIf   
	        	...
	        	If !Empty(SC6->C6_P_KIT)
	       			...
	       			If SB1->(DbSeek(xFilial("SB1")+SC6->C6_P_KIT))
	        			...
	        			If aScan(aKits,SC6->C6_P_KIT) == 0
							...
							nMaxCod := MaxCod(AllTrim(SC6->C6_P_KIT), 50)
    ...
    ElseIf !(SM0->M0_CODIGO) == "BH" //RRP - 28/10/2013 - Retirada empresa SNF do tratamento. Chamado 015268 
		...
		EndIf    
    EndIf
	//RRP - 12/12/2012 - Inclusão das configurações das Colunas Cod. Produto do DANFEII Antigo.
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],4))))//RRP - 07/12/2012 - Truncar o campo V.Unitario para 4 casas decimais.
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//CAS - 06/09/2018 - Inclusão(cópia) do bloco abaixo para adição dos itens "Lote/Validade" no array, devido ao erro no estouro do array (15 of 14). Ticket #28981
		If SM0->M0_CODIGO $ 'IS' //Dr. Reddy's | Promega |
		...
		ElseIf SM0->M0_CODIGO $ 'LN|HB|U2' //Neogen, Hazera
			...
			If lLote
			   ...
			   //Campo Valid 
			   If SM0->M0_CODIGO $ 'LN|U2'//RSB - Customizado
				   aAdd(aItens[Len(aItens)],"")
				   aIns(aItens[Len(aItens)],4)
				   aItens[Len(aItens)][4] := cDtValLote
			   Endif 
			EndIf
		...
		EndIf
		//CAS - 06/09/2018 - Inclusão(cópia) do bloco acima para adição dos itens "Lote/Validade" no array, devido ao erro no estouro do array (15 of 14). Ticket #28981
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////
	//ECR - 04/12/12 - Tratamento customizado para os clientes.
	//RRP - 16/10/2012 retirada da empresa Angelmed HM.
	...
	If SM0->M0_CODIGO $ '4M|2C'//Illumina
		...
		If Empty(cAux) .And. Empty(cAuxItem)		
			If !ProdKit(oDet[nX]:_Prod:_cProd:TEXT) //Verifica se o produto não é Kit
				lPontilhado := .T.	
			EndIf
		EndIf
	EndIf
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		...
		ElseIf SM0->M0_CODIGO $ '4M|2C'//Illumina
			If ProdKit(oDet[nX]:_Prod:_cProd:TEXT) //Verifica se o produto é Kit
				lPontilhado := .F.	
	       EndIf
		EndIf
	EndDo
	...
	If SM0->M0_CODIGO $ "4M|2C" //Illumina
   		...
   		If oIdent:_TpNf:TEXT == "1" //Nf de Saida
	        ...
	        If SC6->(DbSeek(xFilial("SC6")+aTmpPed[nX,1]+aTmpPed[nX,2]))
				...
				If !Empty(SC6->C6_P_OP) //TLM 07/12/2012
						...
						While SD3->(!EOF()) .and. SD3->(D3_FILIAL+D3_OP) == xFilial("SD3")+SC6->C6_P_OP
							...
							If SD3->D3_CF <> "PR0"
								...
								If SB1->(DbSeek(xFilial("SB1")+SD3->D3_COD))
									...
									nMaxCod := MaxCod(AllTrim(SD3->D3_COD), 50)
									...
									If TMP->(!EOF() .and. !BOF())
		        						...
		        						//aItens[Len(aItens)][4] := AllTrim(TMP->D5_DTVALID)          
									...
									While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(cAuxLote)
										...
										aItens[Len(aItens)][3] := Substr(cAuxLote,1,50)
		 								//aItens[Len(aItens)][4] := "01/01/2017"                            
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	EndDo
EndIf
//RRP 20/12/2012 - Volta do fonte que estava comentado
dbSelectArea("SF1")
dbSetOrder(1)
If MsSeek(xFilial("SF1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]) .And. SF1->(FieldPos("F1_FIMP"))<>0
	If SF1->F1_TIPO == "D"
	...
	DbSkip()
EndIf
//RRP 20/12/2012 - Final volta do fonte que estava comentado
...
Else
	...
	nLinCalc	:=	065
	cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
	//ECR - 04/12/12 - Tratamento para quebrar o nome do emitente, sem quebrar a palavra.
	...
	*/
	...
	If SM0->M0_CODIGO == 'ED'
		nLinCalc+=10
		oDanfe:Say(nLinCalc,098, "Visite-nos em: www.okuma.com.br",oFont08NI:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,098, "www.okuma.com (worldwide website)",oFont08NI:oFont)
	EndIf
EndIf
...
aAux := {{{},{},{},{},{},{},{},{},{},{},{},{},{},{}}}
/////////////////////////////////////////////////////////////
//ECR - 04/12/2012 - Tratamento customizado para os clientes.
//RRP - 19/12/2012 retirada da empresa Ceres CZ.   
//RRP - 16/05/2013 retirada da empresa Ceres U2.
If SM0->M0_CODIGO $ 'FF|R7|4M|2C'
	aAdd(aAux[1],{})

//RRP - 16/10/2012 retirada da empresa Angelmed HM.
ElseIf SM0->M0_CODIGO $ "IS|HB"
	If lLote
		aAdd(aAux[1],{})
	EndIf 
//RSB - 07/08/2017 incluido o campo para a empresa Neogen.
ElseIf SM0->M0_CODIGO $ "LN|U2"
	If lLote
		aAdd(aAux[1],{})
		aAdd(aAux[1],{})
	EndIf
...
For nX :=1 To nLenItens
	...
	//RSB - 07/08/2017 incluido o campo para a empresa Neogen.
	//Incluido o 16 para que o sistema entendo os campos de LOTE e Validade.
	//Caso apenas lote mostra 15.
	If Len(aAux[1]) == 14
...
//RRP - 19/12/2012 retirada da empresa Ceres CZ.
//RRP - 16/05/2013 retirada da empresa Ceres U2.
If SM0->M0_CODIGO $ '4M|2C|FF' //Ceres, Illumina e Sumitomo
	aAuxCabec := {;
		"COD. PROD",;
...
//RRP - 16/10/2012 retirada da empresa Angelmed HM.
ElseIf SM0->M0_CODIGO $ 'IS|HB' //Promega, NeoGen e Hazera
...
ElseIf SM0->M0_CODIGO $ 'LN|U2' //NeoGen //RSB - Customizado
...
//RRP - 31/07/2018 - Ponto de entrada para ajustar colunas dos itens
If ExistBlock("PE02DANFE")
	aParam2 := {aAuxCabec, aAux, oDanfe, oFont08N:oFont, oFont07:oFont}
	aParam2 := ExecBlock("PE02DANFE",.F.,.F.,aParam2)
	If (Len(aParam2) >= 2)
		aTamCol		:= aParam2[1]
		lExcPe02	:= aParam2[2]  
	EndIf
...
nAuxH := 0
//ECR - 03/12/2012	
For nI:=1 To Len(aAuxCabec)
	oDanfe:Box(442, nAuxH, 678, nAuxH + aTamCol[nI])
	oDanfe:Say(450, nAuxH + 2,aAuxCabec[nI],oFont08N:oFont)
	nAuxH += aTamCol[nI]
Next
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			Else
				...
				cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
				//ECR - 04/12/2012 - Tratamento para quebrar a linha, sem quebrar o nome do emitente.
				cStrAux 	:= EECBrkLine(cStrAux,24)//Quebra a string por palavras
				aStrAux  	:= StrToKArr(cStrAux,ENTER) //Adiciona cada linha da string em uma posição do array.
			...
			//ECR - 03/12/2012	
			For nI:=1 To Len(aAuxCabec)
				oDanfe:Box(163+nLinhavers, nAuxH, nColLim, nAuxH + aTamCol[nI])
				oDanfe:Say(171+nLinhavers, nAuxH + 2, aAuxCabec[nI],oFont08N:oFont)
				nAuxH += aTamCol[nI]
			Next
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If SM0->M0_CODIGO == 'FF'
			...
			If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
				...
				For nX := 1 To nForTo
					...
					nLinCalc+=10
				Next nX
				 	oDanfe:Say(nLinCalc,000, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
			 	nLinCalc+=10
			 	oDanfe:Say(nLinCalc,000, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
			Else
				oDanfe:Say(nLinCalc,000, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
				nLinCalc+=10
				oDanfe:Say(nLinCalc,000, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
				nLinCalc+=10
				oDanfe:Say(nLinCalc,000, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
		...
		//ECR - 03/12/2012	
		For nI:=1 To Len(aAuxCabec)
			oDanfe:Box(163, nAuxH, nColLim, nAuxH + aTamCol[nI])
			oDanfe:Say(171, nAuxH + 2, aAuxCabec[nI],oFont08N:oFont)
			nAuxH += aTamCol[nI]
		Next
		/*
	...
	Else
		//ECR - 03/12/2012	
		For nI:=1 To Len(aAuxCabec)
			oDanfe:Say(nLinha, nAuxH + 2, aAux[1][nI][nY], oFont08:oFont )
			nAuxH += aTamCol[nI]
		Next
...
/*
Função  : ProdKit
Objetivo: Verifica se o produto é um Kit.
