...
If ExistBlock("NFSEMUNPRE")
	aPEMunic := ExecBlock("NFSEMUNPRE",.F.,.F.,{cCodMun})
	If Len(aPEMunic) # 0
		cMunPrest := aPEMunic[1]
		cDescMunP := aPEMunic[2]
	EndIf
EndIf
...
If cCodMun == "3509502" .AND. ExistBlock("PE01NFSEDESCR")
	cString += '<DescricaoRPS>' + ExecBlock("PE01NFSEDESCR",.F.,.F.,{cCodMun,,,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_CLIENTE,SF2->F2_LOJA,,.T.,.T.}) + '</DescricaoRPS>'
Else
...
For Nx := 1 to Len(aProd)
	...
	If ExistBlock("PE01NFSEDESCR")
		cXml += '<DiscriminacaoServico>' + ExecBlock("PE01NFSEDESCR",.F.,.F.,{cCodMun,,,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_CLIENTE,SF2->F2_LOJA,cCargaTrb,.T.,.F.,aProd[Nx],Nx}) + '</DiscriminacaoServico>'
	Else
