...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	If ExistBlock("NFSEMUNPRE")
		aPEMunic := ExecBlock("NFSEMUNPRE",.F.,.F.,{SM0->M0_CODMUN})
		If Len(aPEMunic) # 0
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )
	...
	for nX := 1 to len( aProd )
		...
		If ExistBlock("PE01NFSEDESCR")
			cString += '<discr>' + ExecBlock("PE01NFSEDESCR",.F.,.F.,{cCodMun,,,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_CLIENTE,SF2->F2_LOJA,cCargaTrb,.T.,.F.}) + '</discr>'
		Else
