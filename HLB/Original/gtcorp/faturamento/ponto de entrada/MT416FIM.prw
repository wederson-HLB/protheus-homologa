...
//Quando for integração OverRun
If (cAlias)->(FieldPos("CJ_P_NUM")) <> 0 .and. (cAlias)->(FieldPos("CJ_P_OVE")) <> 0
	If !EMPTY((cAlias)->CJ_P_NUM) .or. !EMPTY((cAlias)->CJ_P_OVE)
		AtuOverRun()
		Return .T.
	EndIf
EndIf

//Atualização para portal FaturamentoGT
AtuPortalGt()
...
If "GTCORP11A/GTCORP11B/GTCORP11C/GTCORP11D/GTCORPB/TESTE" $ ALLTRIM(UPPER(GetEnvServer()))//Para ambiente de testes
...
If (cAlias)->CJ_STATUS $ "C" 
	cStatus := "C" //Cancelado 
ElseIf (cAlias)->CJ_STATUS $ "B"  
	cStatus := "P" //Em Atendimento
Else
	Return .T.
Endif
...
If TCSQLExec(cUpdate) < 0
    Return MsgStop("Erro na alteração de Status do processo no Portal do cliente." + TCSQLError())
EndIf
...
/// DESATIVADO ENVIO DE WORKFLOW DE ORCAMENTOS /////
...
oEmail:cSubject	:= padr("Solicitacao '"+ALLTRIM(aHeader[nPosNum][2])+"' alterado o Status para 'Em Atendimento' no portal do cliente.",200)
oEmail:cBody   	:= cEmail
oEmail:Envia()
...
cHtml+='<html xmlns:v="urn:schemas-microsoft-com:vml"         xmlns:o="urn:schemas-microsoft-com:office:office"'
cHtml+='      xmlns:w="urn:schemas-microsoft-com:office:word" xmlns:m="http://schemas.microsoft.com/office/2004/12/omml"'
...
cHtml+="										Solicitação "+ALLTRIM(aHeader[nPosNum][2])+" esta em atendimento"
cHtml+="									</span></b>"
cHtml+="								</p>"
cHtml+="							</td>"
cHtml+="						</tr>"
...
/*
Funcao      : AtuOverRun
Objetivos   : Atualização dos campos no pedido de venda.
Autor       : Jean Victor Rocha
Data/Hora   : 24/04/2015
*/            
*--------------------------*
Static Function AtuOverRun()
*--------------------------*

If SC5->(!EOF())
	SC5->(RecLock("SC5",.F.))
	If SC5->(FieldPos("C5_P_NUM")) <> 0
		SC5->C5_P_NUM := SCJ->CJ_P_NUM
	EndIf 
	If SC5->(FieldPos("C5_MDCONTR")) <> 0
		SC5->C5_MDCONTR := SCJ->CJ_P_CONTR
	EndIf
	SC5->(MsUnlock())
EndIf	               
Return .T.