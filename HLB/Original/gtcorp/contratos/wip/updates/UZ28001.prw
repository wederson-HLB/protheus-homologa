...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : Criação da tabela de concorrentes
Autor       : Matheus Massarotto
Data/Hora   : 22/09/2014    10:14
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
Local cTexto:=""
    ...
    //Inclui dados na tabela de concorrentes
    AtuTabConco(@cTexto)

	//Atualiza o sxe e sxf    
    AtuSXESXF(@cTexto)
    
    //Cria consulta padrão para tabela de concorrentes
    ConsZ28(@cTexto)
...
*-----------------------------------*
Static Function AtuTabConco(cTexto)
*-----------------------------------*
DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000001")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000001"
		Z28->Z28_NOME	:= "PWC – Price waterhouseCoopers"
	Z28->(MsUnlock())
endif

DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000002")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000002"
		Z28->Z28_NOME	:= "Deloitte Touche Tohmatsu"
	Z28->(MsUnlock())
endif

DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000003")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000003"
		Z28->Z28_NOME	:= "EY – Ernest & Young"
	Z28->(MsUnlock())
endif

DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000004")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000004"
		Z28->Z28_NOME	:= "KPMG"
	Z28->(MsUnlock())
endif
...
DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000005")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000005"
		Z28->Z28_NOME	:= "BDO"
	Z28->(MsUnlock())
endif
...
DbSelectArea("Z28")
DbSetOrder(1)
if !DbSeek(xFilial("Z28")+"000006")
	RecLock("Z28",.T.)
		Z28->Z28_CODIGO	:= "000006"
		Z28->Z28_NOME	:= "Outros"
	Z28->(MsUnlock())
endif

cTexto += "Incluido/alterados concorrentes na tabela Z28"+NL

Return

*---------------------------------*
Static Function AtuSXESXF(cTexto)
*---------------------------------*



DbUseArea(.T., "DBFCDX", "\"+alltrim(CURDIR())+"SXE.DBF", "SXETMP", .T., .F.)
DbSelectArea("SXETMP")

cArqInd := CriaTrab(Nil,.F.)
cChave := "XE_FILIAL"
IndRegua("SXETMP",cArqInd,cChave,,,"Indexando Registros...")

DbSetOrder(1)

if !DbSeek("  \SYSTEM\Z28YY0")
	RecLock("SXETMP",.T.)
		SXETMP->XE_FILIAL:="  \SYSTEM\Z28YY0"
		SXETMP->XE_ALIAS:="Z28"
		SXETMP->XE_TAMANHO:=6
		SXETMP->XE_NUMERO:="000008"
	MsUnlock()
endif

DbUseArea(.T., "DBFCDX", "\"+alltrim(CURDIR())+"SXF.DBF", "SXFTMP", .T., .F.)
DbSelectArea("SXFTMP")

cArqInd := CriaTrab(Nil,.F.)
cChave := "XF_FILIAL"
IndRegua("SXFTMP",cArqInd,cChave,,,"Indexando Registros...")

DbSetOrder(1)

if !DbSeek("  \SYSTEM\Z28YY0")
	RecLock("SXFTMP",.T.)
		SXFTMP->XF_FILIAL:="  \SYSTEM\Z28YY0"
		SXFTMP->XF_ALIAS:="Z28"
		SXFTMP->XF_TAMANHO:=6
		SXFTMP->XF_NUMERO:="000007"
	MsUnlock()
endif


cTexto += "Incluido/alterados SXE/SXF da tabela Z28"+NL
...
*-------------------------------*
Static Function ConsZ28(cTexto)
*-------------------------------*
Local lIncSXB	:= .F.

DbSelectArea("SXB")
SXB->(DbSetOrder(1))
if SXB->(!DbSeek("Z28"+"1"+"01"+"DB"))
	lIncSXB:=.T.
else
	lIncSXB:=.F.
endif          
Reclock("SXB",lIncSXB)
	...
	SXB->XB_SEQ:="01"
	SXB->XB_COLUNA:="DB"
	SXB->XB_DESCRI:="Concorrentes"
	if FieldPos("XB_DESCSPA")>0
	SXB->XB_DESCSPA:="Concorrentes"       
	endif	
	if FieldPos("XB_DESCENG")>0
	SXB->XB_DESCENG:="Competitors"
	endif
	SXB->XB_CONTEM:="Z28"
	SXB->XB_WCONTEM:=""
SXB->(MsUnlock())	
...
Reclock("SXB",lIncSXB)
	...
	SXB->XB_SEQ:="01"
	SXB->XB_COLUNA:="01"
	SXB->XB_DESCRI:="Codigo+Nome"
	if FieldPos("XB_DESCSPA")>0
	SXB->XB_DESCSPA:="Codigo+Nome"       
	endif	
	if FieldPos("XB_DESCENG")>0
	SXB->XB_DESCENG:="Code+Name"
	endif
	SXB->XB_CONTEM:=""
	SXB->XB_WCONTEM:=""
SXB->(MsUnlock())
...
Reclock("SXB",lIncSXB)
	...
	SXB->XB_CONTEM:="Z28_CODIGO"
	SXB->XB_WCONTEM:=""
SXB->(MsUnlock())

DbSelectArea("SXB")
SXB->(DbSetOrder(1))
if SXB->(!DbSeek("Z28"+"4"+"01"+"02"))
	lIncSXB:=.T.
else
	lIncSXB:=.F.
endif          
...
Reclock("SXB",lIncSXB)
	...
	SXB->XB_CONTEM:="Z28->Z28_NOME"
	SXB->XB_WCONTEM:=""
SXB->(MsUnlock())

cTexto += "Incluido/alterado no SXB consulta Z28 para concorrentes "+NL 
...
AADD(aSix,{'Z28','1','Z28_FILIAL+Z28_CODIGO','Codigo','Codigo','Codigo','U','','','S'})
AADD(aSix,{'Z28','2','Z28_FILIAL+Z28_NOME','Nome','Nome','Name','U','','','S'})
...
AADD(aSX2,{'Z28','\SYSTEM\','Z28YY0','Cadastro de Concorrentes','Cadastro de Concorrentes','Registration of Competitors','','C','C','C','','','','S','','','',''})
...
AADD(aSX3,{'Z28','01','Z28_FILIAL'	,'C','2','','Filial','Filial','Filial do Si','Filial','Filial','Filial','','','€€€€€€€€€€€€€€€','','','','™€','','','U','N','','','','','','','','','','','033','1','','','','','','','',''})
AADD(aSX3,{'Z28','02','Z28_CODIGO'	,'C','6','','Codigo','Codigo','Code','Codigo Servico','Codigo Servico','Service Code','','','€€€€€€€€€€€€€€ ','GETSXENUM("Z28","Z28_CODIGO")','','','›€','','S','U','S','V','R','€','','','','','','','','','1','','','','','','','',''})
AADD(aSX3,{'Z28','03','Z28_NOME'	,'C','60','','Nome','Nome','Name','Nome do concorrente','Nome do concorrente','Competitor Name','','','€€€€€€€€€€€€€€ ','','','','š€','','S','U','S','A','R','€','','','','','','','','','1','','','','','','','',''})
