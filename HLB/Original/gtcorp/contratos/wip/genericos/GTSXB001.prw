...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Função para apresentar uma consulta específica com todos os funcionários de todas as empresas do ambiente, parecido com a consulta padrão
			: 
Autor       : Matheus Massarotto
Revisão		:
Data/Hora   : 19/07/2013    10:28
Módulo      : Genérico
*/

*---------------------*
User function GTSXB001
*---------------------*
...
DADTRB->(DbSetOrder(1))
			...
			For i:=1 to len(aAllGroup)
				...
				cQry+=" SELECT '"+aAllGroup[i]+"' AS EMP,RA_NOME,RA_CIC,RA_MAT,R_E_C_N_O_ AS CRECNO FROM SRA"+aAllGroup[i]+"0"
				cQry+=" WHERE D_E_L_E_T_='' AND RA_SITFOLH NOT IN ('D','T')"
        ...
        //APAGAR AKI
        if cEmpAnt $ "99"
        	cQry+=" SELECT 'ZB' AS EMP,RA_NOME,RA_CIC,RA_MAT,R_E_C_N_O_ AS CRECNO FROM SRAZB0"
        endif
