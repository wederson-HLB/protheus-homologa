...
*-------------------------------------------------*
  USER FUNCTION Z4FIN001(lSelAuto,lPreview,lAutom)   
*-------------------------------------------------*
...
If !cEmpAnt $ "4K/Z4/ZG" //AOA - 16/08/2016 - Incluido código da empresa ZG para gerar boleto Santander
	MsgInfo("Especifico GTCORP Outsourcing Consulting, verifique a empresa!","A T E N C A O ")
	return
Endif
...
If lExec
	Processa({|lEnd|MontaRel(lSelAuto,lPreview,lAutom)})
else
...
LOCAL cFileName := ""
Local cDirBol	:= "\FTP\" + cEmpAnt + "\Z4FIN001\" 
Local cCamBol	:= ""
...
//SELPORT()
If Alltrim(SM0->M0_CODIGO)=="Z4" //ER - 14/09/11 Neogen
	cPortGrv    := "033"
	cAgeGrv     := "3853 "
	cContaGrv   := "130003167"
	cCodEmp		:= "3930670"
ElseIf Alltrim(SM0->M0_CODIGO)=="ZG" //AOA - 16/08/2016 - Incluido código da empresa ZG para gerar boleto Santander
	cPortGrv    := "033"
	cAgeGrv     := "3853 "
	//cContaGrv   := "130900002"
	cContaGrv   := "8024928"
	cCodEmp		:= "8024928"
ElseIf Alltrim(SM0->M0_CODIGO)=="4K"
	cPortGrv    := "033"
	cAgeGrv     := "3853 "
	cContaGrv   := "130063879"
	cCodEmp		:= "6682260"
...
WHILE SQL->(!EOF())
	...
	If Empty(SE1->E1_PORTADO) .OR. SE1->E1_PORTADO <> "033" // GFP 03/02/2017 - Sistema ignora qualquer titulo com boleto ja gerado que seja diferente de Santander
		SQL->(dbSkip())
		Loop
	Endif
	
	If n > 0  // GFP - 16/01/2017 - Alteração para geração de um boleto por arquivo.
		cFileName := "Boleto_"+GravaData(Date(),.F.,5)+SUBS(TIME(),1,2)+SUBS(TIME(),4,2)+SUBS(TIME(),7,2)+"_"+SE1->E1_PREFIXO+ALLTRIM(SE1->E1_NUM)+ALLTRIM(SE1->E1_PARCELA)  	
		If !lPreview
			oPrint:= FWMSPrinter():New(cFileName,IMP_PDF,.T./*.F.*/,,.T.,.F.,,,,,,.F.,0)
		Else 
	...
	CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],cNroDoc,(E1_SALDO - _nVlrAbat-E1_DECRESC+E1_ACRESC),E1_VENCREA)
	...
	If File( cLocal+"totvsprinter\"+cFileName+".pdf" )
		FErase(cLocal+"totvsprinter\"+cFileName+".pdf")
	EndIf
	If !ExistDir(cDirBol)
		MakeDir(cDirBol)
	EndIf
	...
	If !CpyT2S( cLocal+"totvsprinter\"+cFileName+".pdf" , cDirBol ,.T. )
...
oPrint:Say (0150+_nLin2,1060,"Agência/Código Cedente",oFont8)
...
oPrint:Say (0300+_nLin2,1060,DTOC(aDadosTit[4]),oFont10)
oPrint:Say (0250+_nLin2,1510,"Valor do Documento",oFont8)
oPrint:Say (0300+_nLin2,1550,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)
...
oPrint:Say (_nLin+_nLin2+0644,1900,"Recibo do Sacado",oFont10)
...
oPrint:Say (_nLin+_nLin2+0750,0100,"QUALQUER BANCO ATÉ A DATA DO VENCIMENTO",oFont10)
oPrint:Say (_nLin+_nLin2+0710,1910,"Vencimento",oFont8)
oPrint:Say (_nLin+_nLin2+0750,1950,DTOC(aDadosTit[4]),oFont10)
oPrint:Say (_nLin+_nLin2+0810,0100,"Beneficiário",oFont8)
oPrint:Say (_nLin+_nLin2+0850,0100,aDadosEmp[1]+"             - "+aDadosEmp[6],oFont10) //Nome + CNPJ
oPrint:Say (_nLin+_nLin2+0810,1910,"Agência/Código Cedente",oFont8)
oPrint:Say (_nLin+_nLin2+0850,1950,aDadosBanco[3]+"/"+aDadosBanco[4]+aDadosBanco[5],oFont10)
oPrint:Say (_nLin+_nLin2+0910,0100,"Data do Documento",oFont8)
oPrint:Say (_nLin+_nLin2+0940,0100,DTOC(aDadosTit[2]),oFont10) // Emissao do Titulo (E1_EMISSAO)
...
oPrint:Say (_nLin+_nLin2+0940,1655,DTOC(aDadosTit[3]),oFont10) // Data impressao
...
oPrint:Say (_nLin+_nLin2+0940,1950,PADL(alltrim(SUBSTR(aDadosTit[6],4)),14,'0'),oFont10)
...
oPrint:Say (_nLin+_nLin2+1010,1950,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)
oPrint:Say (_nLin+_nLin2+1050,0100,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do cedente)",oFont8)
	_nLin3:=1110
...
//oPrint:Say (_nLin+_nLin2+1200,0100,aBolText[2]+" "+AllTrim(Transform(((aDadosTit[5]*0.01)/30),"@E 99,999.99")),oFont10) // JSS - ALTERADO PARA SOLUCIONAR O CASO 019604
//oPrint:Say (_nLin+_nLin2+1250,0100,aBolText[3],oFont10)
oPrint:Say (_nLin+_nLin2+1050,1910,"(-)Desconto/Abatimento",oFont8)
//oPrint:Say (_nLin+_nLin2+1080,1950,AllTrim(Transform(aDadosTit[9],"@E 999,999,999.99")),oFont10) //JSS - ADICONADO PARA SOLUCIONAR O CHAMADO 017963  
...
//oPrint:Say (_nLin+_nLin2+1290,1950,AllTrim(Transform(aDadosTit[10],"@E 999,999,999.99")),oFont10) //JSS - ADICONADO PARA SOLUCIONAR O CHAMADO 017963 
...
oPrint:Say (_nLin+_nLin2+1589,0400,"C.N.P.J.: "+TRANSFORM(aDatSacado[7],"@R 99.999.999/9999-99"),oFont10) // CGC
//oPrint:Say (_nLin+1589,1950,SUBSTR(aDadosTit[6],1,3)+"/00"+SUBSTR(aDadosTit[6],4,8),oFont10)
oPrint:Say (_nLin+_nLin2+1589,1950,SUBSTR(aDadosTit[6],4,len(aDadosTit[6])-3),oFont10)
oPrint:Say (_nLin+_nLin2+1605,0100,"Sacador/Avalista",oFont8)
...
oPrint:Say (_nLin+_nLin2+1912,0567,aDadosBanco[1]+"-7",oFont24)	// [1] Numero do Banco
oPrint:Say (_nLin+_nLin2+1934,0820,CB_RN_NN[2],oFont14n)		// [2] Linha Digitavel do Codigo de Barras
...
oPrint:Say (_nLin+_nLin2+2040,0100,"QUALQUER BANCO ATÉ A DATA DO VENCIMENTO",oFont10)
oPrint:Say (_nLin+_nLin2+2000,1910,"Vencimento",oFont8)
oPrint:Say (_nLin+_nLin2+2040,1950,DTOC(aDadosTit[4]),oFont10)
oPrint:Say (_nLin+_nLin2+2100,0100,"Beneficiário",oFont8)
oPrint:Say (_nLin+_nLin2+2140,0100,aDadosEmp[1]+"             - "+aDadosEmp[6],oFont10) //Nome + CNPJ
oPrint:Say (_nLin+_nLin2+2100,1910,"Agência/Código Cedente",oFont8)
oPrint:Say (_nLin+_nLin2+2140,1950,aDadosBanco[3]+"/"+aDadosBanco[4]+aDadosBanco[5],oFont10)
oPrint:Say (_nLin+_nLin2+2200,0100,"Data do Documento",oFont8)
oPrint:Say (_nLin+_nLin2+2230,0100,DTOC(aDadosTit[2]),oFont10)			// Emissao do Titulo (E1_EMISSAO)
...
oPrint:Say (_nLin+_nLin2+2230,1655,DTOC(aDadosTit[3]),oFont10) // Data impressao
...
oPrint:Say (_nLin+_nLin2+2230,1950,PADL(alltrim(SUBSTR(aDadosTit[6],4)),14,'0'),oFont10)
...
oPrint:Say (_nLin+_nLin2+2300,1950,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)
oPrint:Say (_nLin+_nLin2+2340,0100,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do cedente)",oFont8)
	_nLin3:=2400
For i=1 To Len(aBolText)
	oPrint:Say (_nLin+_nLin2+_nLin3,0100,aBolText[i],oFont10)
	_nLin3:= _nLin3+50 
...
//oPrint:Say (_nLin+_nLin2+2370,1950,AllTrim(Transform(aDadosTit[9],"@E 999,999,999.99")),oFont10) //JSS - ADICONADO PARA SOLUCIONAR O CHAMADO 017963 
...
//oPrint:Say (_nLin+_nLin2+2580,1950,AllTrim(Transform(aDadosTit[10],"@E 999,999,999.99")),oFont10) //JSS - ADICONADO PARA SOLUCIONAR O CHAMADO 017963
...
oPrint:Say (_nLin+_nLin2+2879,0400,"C.N.P.J.: "+TRANSFORM(aDatSacado[7],"@R 99.999.999/9999-99"),oFont10)	// CGC
//oPrint:Say (_nLin+2879,1950,SUBSTR(aDadosTit[6],1,3)+"/00"+SUBSTR(aDadosTit[6],4,8),oFont10)
oPrint:Say (_nLin+_nLin2+2879,1950,SUBSTR(aDadosTit[6],4,len(aDadosTit[6])-3),oFont10)
oPrint:Say (_nLin+_nLin2+2895,0100,"Sacador/Avalista",oFont8)
...
//oPrint:FWMSBAR("INT25" /*cTypeBar*/,27.8/*nRow*/ ,1.5/*nCol*/, CB_RN_NN[1]/*cCode*/,oPrint/*oPrint*/,.F./*lCheck*/,/*Color*/,.T./*lHorz*/,1.4/*nWidth*/,/*nHeigth*/,.F./*lBanner*/, cFont/*cFont*/,NIL/*cMode*/,.F./*lPrint*/,/*nPFWidth*/,/*nPFHeigth*/,.F./*lCmtr2Pix*/)
