...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Função para gerar relatório de contas a receber mensal por cliente
			: 
...
cQry1 := "SELECT E1_CLIENTE AS 'COD_CLIENTE', E1_LOJA  AS 'FILIAL', A1_NOME AS 'CLIENTE', A1_CGC AS 'CNPJ_CPF'"
If ( cCombo == aItems[ 1 ] )
	cQry1 += ",E1_TIPO AS 'TIPO'"
EndIf
...
cQry1 += "FROM (SELECT E1.E1_FILIAL, E1.E1_CLIENTE, E1.E1_LOJA, A1.A1_NOME, A1.A1_CGC,"+If( cCombo == aItems[ 1 ] ,"E1.E1_TIPO,","")+"Substring(E1.E1_EMISSAO,1,6) AS 'MS' "
cQry1 += ", SUM(E1.E1_VALOR) AS TOTAL "
cQry1 += "FROM SE1"+cEmp+"0 E1 "
cQry1 += "LEFT JOIN SA1"+cEmp+"0 A1 ON A1.A1_COD = E1.E1_CLIENTE AND A1.A1_LOJA = E1.E1_LOJA "
cQry1 += "WHERE E1.D_E_L_E_T_ = '' "
cQry1 += "AND A1.D_E_L_E_T_ = '' "    
cQry1 += "AND E1.E1_EMISSAO >= "+cAno+"0101 AND E1_EMISSAO <= "+cAno+"1231 "

if !empty(cFil)
	cQry1 +=" AND E1.E1_FILIAL = '"+cFil+"'
	cNomeEmp:="Filial "+cFil+" - "+FWFilialName(cEmp,cFil,1)
endif
   		
cQry1 += "GROUP BY E1.E1_FILIAL, E1.E1_CLIENTE, E1.E1_LOJA, A1.A1_NOME,A1.A1_CGC,"+If( cCombo == aItems[ 1 ] ,"E1.E1_TIPO,","")+"Substring(E1.E1_EMISSAO,1,6) )  "
...
cQry1 += "ORDER BY E1_FILIAL, A1_NOME "
...
oExcel:AddTable (cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Cod. Cliente",1,1)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Filial",1,1)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Cliente",1,1)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Cnpj_Cpf",1,1)

If ( cCombo == aItems[ 1 ] )                                                                            
	oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Tipo",1,1)
EndIf                                                                                           

oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Janeiro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Fevereiro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Março",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Abril",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Maio",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Junho",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Julho",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Agosto",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Setembro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Outubro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Novembro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Dezembro",1,3)
oExcel:AddColumn(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,"Total Geral",1,3)
...
if nRecCount>0
	...
	While TRBTEMP->(!EOF())
		...
		oExcel:AddRow(cNomeEmp,"Contas_Receber "+Capital(Alltrim(FWGrpName(cEmp)))+" - "+cAno,aRowTab)
...
/***********************GERANDO EXCEL************************************/
	...
	//cArq := "Contas_Receber_"+alltrim(CriaTrab(NIL,.F.))+".xls"

	cArq := "Contas_Receber-"+alltrim(FWGrpName(cEmp))+"-"+cAno+".xls"
