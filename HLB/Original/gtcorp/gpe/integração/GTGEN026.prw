...
oDlg := MSDialog():New( aSize[7],0,aSize[6],aSize[5],"Grant Thornton Brasil - Integração APDATA",,,.F.,,,,,,.T.,,,.T. )
...
If !lExec
	Alert("Nenhuma integração selecionada!","Grant Thornton Brasil")
	Return .T.
EndIf

If !lErro .and. !MsgYesNo("Existe integração Selecionada com erro, Deseja Continuar Assim Mesmo?","Grant Thornton Brasil")
	Return .T.
EndIf
...
If !File(LEFT(cDirArq,LEN(cDirArq)-1))
	If cDirArq == GETTEMPPATH()+"GPE2APDATA\"
		...
		If !File(LEFT(cDirArq,LEN(cDirArq)-1))
   			Alert("Não Foi possivel criar o Diretorio padrão '"+cDirArq+"', operação abortada!","Grant Thornton Brasil")
	   		Return .T.
		EndIf
	Else
		Alert("Diretorio não encontrado '"+cDirArq+"', operação abortada!","Grant Thornton Brasil")
		Return .T.
	EndIf
EndIf
...
If MsgYesNo("Arquivos gerados no diretorio selecionado: '"+cDirArq+"', Deseja Abrir a Pasta?","Grant Thornton Brasil")
...
If !lProcAll 
	If lMarcOK 
		If !MsgYesNo("Deseja Reprocessar as integrações marcadas?","Grant Thornton Brasil")
			Return .F.
		EndIf
	Else
		Alert("Necessario selecionar no minimo uma integração.","Grant Thornton Brasil")
		Return .F.
	EndIf
EndIf
...
MsgInfo("Diretorio selecionado: '"+cDirArq+"'","Grant Thornton Brasil")
...
*----------------------*
oDlgHelp := MSDialog():New( aSize[7],200,aSize[6]-100,aSize[5]-200,"Ajuda - Grant Thornton Brasil - Integração APDATA",,,.F.,,,,,,.T.,,,.T. )
...
aAdd(aHelp,{"100","Integração APDATA"+CHR(13)+CHR(10)+;
				  "A Equipe de Sistemas da Grant Thornton Brasil desenvolveu a rotina de integração APDATA para facilitar a geração"+;
...
Do Case
	...
	Case cTipoInt =='415' //Contratados"
		...
		aAdd(aRet,{'CodResRAIS'					,'A','02','Codigo rescisao RAIS','','Não',.F.})
