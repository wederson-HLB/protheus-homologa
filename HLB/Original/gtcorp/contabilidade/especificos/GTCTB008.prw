...
If EMPTY(cRevisao:=GetRevisao())
	MsgInfo("Revisão Invalida!","Grant Thornton Brasil")
	Return .T.
EndIf
If !MsgYesNo("Confirma o ajuste para a revisão '"+ALLTRIM(cRevisao)+"'?","Grant Thornton Brasil")
	Return .T.
EndIf

//Busca a Apuração        
If EMPTY(cApuracao:=GetApuracao())
	MsgInfo("Apuração Invalida!","Grant Thornton Brasil")
	Return .T.
EndIf
...
oDlg1      := MSDialog():New( 100,200,420,568,"Grant Thornton Brasil",,,.F.,,,,,,.T.,,,.T. )
...
If MsgYesNo("A data da apuração deste empresa esta em 31/12 ?","Grant Thornton Brasil")
...
oDlg1      := MSDialog():New( 100,100,420,1070,"Grant Thornton Brasil",,,.F.,,,,,,.T.,,,.T. )
