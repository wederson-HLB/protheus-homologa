...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Função para apresentar uma consulta específica com todas as propostas da empresa atual sem considerar filial, parecido com a consulta padrão
			: Ou fazer a validação do campo verificando se a propsota existe.
			: 
Autor       : Matheus Massarotto
Revisão		:
Data/Hora   : 22/10/2014    10:28
Módulo      : Genérico
*/

*----------------------------*
User function GTSXB006(cTipo)
*----------------------------*
Local oLayer 	:= FWLayer():new()
Local aOrdem	:= {"PROPOSTA","CLIENTE","SOCIO"}
...
Private cQry :="SELECT Z55_NUM,Z55_REVISA,Z55_NOME,Z55_NOMESO,Z55_NOMEGE,R_E_C_N_O_ FROM "+RETSQLNAME("Z55")+" WHERE D_E_L_E_T_='' AND Z55_STATUS='E'"
...
elseif cTipo=="SC5"
	if SC5->(FieldPos("C5_P_NUM"))>0
		cQry+=" AND Z55_NUM='"+alltrim(M->C5_P_NUM)+"'"
		...
		if nRecCount <= 0
			Alert("Não existe este código relacionado ao cadastro de propostas!")
			lRet:=.F.
		endif
...
ADD COLUMN oColumn DATA { || NUM		} TITLE "Proposta" 		DOUBLECLICK  {||  }	ALIGN 1 SIZE 14 OF oBrowse		
ADD COLUMN oColumn DATA { || REVISA    	} TITLE "Revisa"		DOUBLECLICK  {||  }	ALIGN 1 SIZE 3 	OF oBrowse		
ADD COLUMN oColumn DATA { || CLIENTE    } TITLE "Cliente"		DOUBLECLICK  {||  }	ALIGN 1 SIZE 25 OF oBrowse	
ADD COLUMN oColumn DATA { || SOCIO		} TITLE "Socio"   		DOUBLECLICK  {||  }	ALIGN 1 SIZE 25	OF oBrowse
ADD COLUMN oColumn DATA { || GERENTE	} TITLE "Gerente"  		DOUBLECLICK  {||  }	ALIGN 1 SIZE 25	OF oBrowse
...
AADD(aDadTemp,{"NUM"		,"C",TamSX3("Z55_NUM")[1]	,0})
AADD(aDadTemp,{"REVISA"		,"C",TamSX3("Z55_REVISA")[1],0})
AADD(aDadTemp,{"CLIENTE"	,"C",TamSX3("Z55_NOME")[1]	,0})
AADD(aDadTemp,{"SOCIO"		,"C",TamSX3("Z55_NOMESO")[1],0})
AADD(aDadTemp,{"GERENTE"	,"C",TamSX3("Z55_NOMEGE")[1],0})
AADD(aDadTemp,{"CRECNO"		,"N",10 					,0})

    
	//Preencho o array para o filtro
	For nHed:=1 to len(aDadTemp)
		if aDadTemp[nHed][1] $ "NUM"
			AADD(aHeader,{ "Proposta",;
						 aDadTemp[nHed][1],;
						 "",;
						 aDadTemp[nHed][3],;
	 					 aDadTemp[nHed][4],;
		...
		elseif aDadTemp[nHed][1] $ "REVISA"
			AADD(aHeader,{ "Revisao",;
						 aDadTemp[nHed][1],;
						 "",;
						 aDadTemp[nHed][3],;
	 					 aDadTemp[nHed][4],;
		...
		elseif aDadTemp[nHed][1] $ "CLIENTE"
		...
		elseif aDadTemp[nHed][1] $ "SOCIO"
			AADD(aHeader,{ "Socio",;
						 ...
						 aDadTemp[nHed][3],;
	 					 ...
	 					 "",;
	 					 "V" } )
		elseif aDadTemp[nHed][1] $ "GERENTE"
			AADD(aHeader,{ "Gerente",;
						 aDadTemp[nHed][1],;
						 "",;
						 aDadTemp[nHed][3],;
	 					 aDadTemp[nHed][4],;
...
IndRegua("DADTRB",cIndex,"SOCIO",,,"Selecionando Registro...")  
IndRegua("DADTRB",cIndex2,"CLIENTE",,,"Selecionando Registro...")  
IndRegua("DADTRB",cIndex3,"NUM",,,"Selecionando Registro...")  
...
DADTRB->(DbSetOrder(1))
        ...
        //APAGAR AKI
        if cEmpAnt $ "99"
        	cQry+=" SELECT Z55_NUM,Z55_REVISA,Z55_NOME,Z55_NOMESO,Z55_NOMEGE,R_E_C_N_O_ FROM Z55ZB0 WHERE D_E_L_E_T_='' AND Z55_STATUS='E'"
        endif
		...
		if nRecCount > 0
			...
			While QRYTEMP->(!EOF())
				Reclock("DADTRB",.T.)
					...
					DADTRB->NUM			:= QRYTEMP->Z55_NUM
					DADTRB->REVISA		:= QRYTEMP->Z55_REVISA
					DADTRB->CLIENTE		:= QRYTEMP->Z55_NOME
					DADTRB->SOCIO   	:= QRYTEMP->Z55_NOMESO
					DADTRB->GERENTE		:= QRYTEMP->Z55_NOMEGE
					DADTRB->CRECNO  	:= QRYTEMP->R_E_C_N_O_
				DADTRB->(MsUnlock())
				QRYTEMP->(DbSkip())
			Enddo
