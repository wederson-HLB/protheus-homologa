...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : Alteração ou criação dos parâmetros MV_VL13137,MV_VCPCCP,MV_VCPCCR, Adequação do Protheus para atender as alterações de regras de cálculo do Pis, Cofins e Csll 
Autor       : Matheus Massarotto
Data/Hora   : 20/08/2015    15:39
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
if SX6->(DbSeek(xFilial("SX6")+"MV_VL13137"))
	RecLock( "SX6" , .F. )
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_CONTSPA := "10.00"
		endif
		if FieldPos("X6_CONTENG")>0
			SX6->X6_CONTENG := "10.00"
		endif
			SX6->X6_CONTEUD := "10.00"
	SX6->(MsUnLock())
	cTexto += "Atualizado Parametro MV_VL13137 "+NL	
else
	RecLock("SX6",.T.)
    	SX6->X6_FIL 	 := ""
    	SX6->X6_VAR      := "MV_VL13137"
		SX6->X6_TIPO     := "N"
		SX6->X6_DESCRIC  := "Define o valor minimo para retencao do PIS/COFINS/"
		SX6->X6_DESC1    := "CSLL de acordo com a Lei 13.137                   "
		SX6->X6_DESC2    := ""
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_DSCSPA   := "Define el valor mínimo para la retención del PIS/COFINS/"
			SX6->X6_DSCSPA1  := "CSLL de acuerdo a la Ley 13137"
			SX6->X6_DSCSPA2  := ""
		endif
		if FieldPos("X6_CONTENG")>0
			SX6->X6_DSCENG   := "Defines the minimum value for retention of PIS /COFINS/"
			SX6->X6_DSCENG1  := "CSLL according to Law 13,137							  "
			SX6->X6_DSCENG2  := ""
		endif
		SX6->X6_CONTEUD  := "10.00"
		SX6->X6_CONTSPA  := "10.00"
	...
	SX6->(MsUnLock())
	cTexto += "Incluido Parametro MV_VL13137 "+NL	
endif

if SX6->(DbSeek(xFilial("SX6")+"MV_VCPCCP"))
	RecLock( "SX6" , .F. )
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_CONTSPA := "1"
		endif
		if FieldPos("X6_CONTENG")>0
			...
			SX6->X6_CONTEUD := "1"
	SX6->(MsUnLock())
	cTexto += "Atualizado Parametro MV_VCPCCP "+NL	
else
	RecLock("SX6",.T.)
    	SX6->X6_FIL 	 := ""
    	SX6->X6_VAR      := "MV_VCPCCP"
		SX6->X6_TIPO     := "N"
		SX6->X6_DESCRIC  := "Qual a data que sera considerada para a           "
		SX6->X6_DESC1    := "cumulatividade do PCC na Emissao. Contas Pagar    "
		SX6->X6_DESC2    := "1=Emissao, 2=Venc.Real, 3=Dt Contab.              "
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_DSCSPA   := "Cual es la fecha que se considerara para"
			SX6->X6_DSCSPA1  := "PCC acumulada en el numero"
			SX6->X6_DSCSPA2  := ""
		endif
		if FieldPos("X6_CONTENG")>0
			SX6->X6_DSCENG   := "What is the date that will be considered for		  "
			SX6->X6_DSCENG1  := "cumulative PCC in Issue							  "
			SX6->X6_DSCENG2  := ""
		endif
		SX6->X6_CONTEUD  := "1"
		SX6->X6_CONTSPA  := "1"
	...
	SX6->(MsUnLock())
	cTexto += "Incluido Parametro MV_VCPCCP "+NL	
endif

if SX6->(DbSeek(xFilial("SX6")+"MV_VCPCCR"))
	RecLock( "SX6" , .F. )
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_CONTSPA := "1"
		endif
		if FieldPos("X6_CONTENG")>0
			...
			SX6->X6_CONTEUD := "1"
	SX6->(MsUnLock())
	cTexto += "Atualizado Parametro MV_VCPCCR "+NL	
else
	RecLock("SX6",.T.)
    	SX6->X6_FIL 	 := ""
    	SX6->X6_VAR      := "MV_VCPCCR"
		SX6->X6_TIPO     := "N"
		SX6->X6_DESCRIC  := "Qual a data que sera considerada para a           "
		SX6->X6_DESC1    := "cumulatividade do PCC na Emissao. Contas Receber  "
		SX6->X6_DESC2    := "1=Emissao, 2=Venc.Real, 3=Dt Contab.              "
		if FieldPos("X6_CONTSPA")>0
			SX6->X6_DSCSPA   := "Cual es la fecha que se considerara para"
			SX6->X6_DSCSPA1  := "PCC acumulada en el numero"
	...
	SX6->(MsUnLock())
	cTexto += "Incluido Parametro MV_VCPCCR "+NL	
endif
