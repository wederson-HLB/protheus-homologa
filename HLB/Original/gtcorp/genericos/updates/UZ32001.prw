...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : Criar a tabela Z32(Ramo de atividade de cliente), Criar o campo(A1_P_RMATI) no cadastro de cliente para selecionar e a consulta padrão
Autor       : Matheus Massarotto
Data/Hora   : 28/04/2014    16:07
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
Local cTexto:=""
    ...
    //Cria o campo ramo de atividade no cadastro de clientes
    AltSA1(@cTexto)

    //Cria o campo ramo de atividade no cadastro de prospects
    AltSUS(@cTexto)
    
    //Cria a tabela de segmento/ramo de atividade
    AtuTab(@cTexto)

    //Carga inicial na tabela Z32
    AtuZ32(@cTexto)
...
Reclock("SX3",lIncSX3)
	...
	SX3->X3_CAMPO:="A1_P_RMATI"
	SX3->X3_TIPO:="C"
	SX3->X3_TAMANHO:=6
	SX3->X3_DECIMAL:=0
	if FieldPos("X3_TITULO")>0
		SX3->X3_TITULO:="Ramo ativida"
	endif
	if FieldPos("X3_TITSPA")>0
		SX3->X3_TITSPA:="Ramo ativida"
	endif
	if FieldPos("X3_TITENG")>0
		SX3->X3_TITENG:="Branch activ"
	endif
	if FieldPos("X3_DESCRIC")>0
		SX3->X3_DESCRIC:="Ramo atividade"
	endif
	if FieldPos("X3_DESCSPA")>0
		SX3->X3_DESCSPA:="Ramo atividade"
	endif
	if FieldPos("X3_DESCENG")>0
		SX3->X3_DESCENG:="Branch activity"
	endif
	SX3->X3_PICTURE:="@!"
	SX3->X3_USADO:="€€€€€€€€€€€€€€ "
	SX3->X3_F3:="Z32"
	...
	SX3->X3_CONTEXT:="R"
	SX3->X3_OBRIGAT:=""
	SX3->X3_VLDUSER:="ExistCpo('Z32',M->A1_P_RMATI,1)  "
	SX3->X3_CBOX:=""
	SX3->X3_CBOXSPA:=""
	SX3->X3_CBOXENG:=""
	SX3->X3_INIBRW:=""
...
cTexto += "Incluido/alterados o campo A1_P_RMATI no SX3 da tabela SA1 "+NL
...
Reclock("SX3",lIncSX3)
	...
	SX3->X3_CAMPO:="A1_P_DRMAT"
	SX3->X3_TIPO:="C"
	SX3->X3_TAMANHO:=100
	SX3->X3_DECIMAL:=0
	if FieldPos("X3_TITULO")>0
		SX3->X3_TITULO:="Descri Ativ"
	endif
	if FieldPos("X3_TITSPA")>0
		SX3->X3_TITSPA:="Descri Ativ"
	endif
	if FieldPos("X3_TITENG")>0
		SX3->X3_TITENG:="Activ Descr"
	endif
	if FieldPos("X3_DESCRIC")>0
		SX3->X3_DESCRIC:="Descricao atividade"
	endif
	if FieldPos("X3_DESCSPA")>0
		SX3->X3_DESCSPA:="Descricao atividade"
	endif
	if FieldPos("X3_DESCENG")>0
		SX3->X3_DESCENG:="Activity Description"
	endif
	SX3->X3_PICTURE:="@!"
	SX3->X3_USADO:="€€€€€€€€€€€€€€ "
	SX3->X3_F3:=""
...
cTexto += "Incluido/alterados o campo A1_P_DRMAT no SX3 da tabela SA1 "+NL

Return

*-----------------------------*
Static Function AltSUS(cTexto)
*-----------------------------*
Local lIncSX3	:= .F.

//<Inicio>Tratamento para buscar a ultima ordem
bCondicao := {|| X3_ARQUIVO == "SUS"}
DbSelectArea("SX3")
SX3->(DbSetOrder(1))
SX3->(DbSetFilter(bCondicao,"X3_ARQUIVO == 'SUS'"))
SX3->(DbGoBottom())
cUltOrdem:=SOMA1(SX3->X3_ORDEM)
SX3->(DBCLEARALLFILTER())
//<Inicio>Tratamento para buscar a ultima ordem
...
Reclock("SX3",lIncSX3)
	...
	SX3->X3_CONTEXT:="R"
	SX3->X3_OBRIGAT:=""
	SX3->X3_VLDUSER:="ExistCpo('Z32',M->US_P_RMATI,1)  "
	SX3->X3_CBOX:=""
	SX3->X3_CBOXSPA:=""
	SX3->X3_CBOXENG:=""
	SX3->X3_INIBRW:=""
...
cTexto += "Incluido/alterados o campo US_P_RMATI no SX3 da tabela SUS "+NL
...
cTexto += "Incluido/alterados o campo US_P_DRMAT no SX3 da tabela SUS "+NL

Return

//Carga inicial da tabela Z32
*----------------------------*
Static Function AtuZ32(cTexto)
*----------------------------*
Local lAtuCCQ	:=.T.
Local aArray 	:= {"Energy & Cleantech","Financial Services","Food & Beverages","Hospitality & Tourism","Mining","Not for Profit","Public Sector","Real Estate & Construction","Technology","Other Industry"}
...
For i:=1 to len(aArray)
	if !Z32->(DbSeek(xFilial("Z32")+PADR(aArray[i],TamSX3("Z32_DESCRI")[1])))
		 RecLock("Z32",.T.)
	    	Z32->Z32_CODIGO	:= GetSx8Num("Z32","Z32_CODIGO")
	    	Z32->Z32_DESCRI	:= aArray[i]
	    Z32->(MsUnLock())
...
//S32->(DbSeek(xFilial("SX5")+"60"))    
/*
While SX5->(!EOF()) .AND. SX5->X5_TABELA=="60"

    DbSelectArea("CCQ")
    DbSetOrder(1)
    if DbSeek(xFilial("CCQ")+SX5->X5_CHAVE)
    	lAtuCCQ:=.F.
    endif
    
	    RecLock("CCQ",lAtuCCQ)
	    	CCQ->CCQ_CODIGO	:= SX5->X5_CHAVE
	    	CCQ->CCQ_DESC	:= SX5->X5_DESCRI
	    CCQ->(MsUnLock())
...
SX3->(DbSetOrder(2))
SX3->(DbGotop())
if SX3->(DbSeek("B1_CODISS"))
	
	RecLock("SX3",.F.)
		SX3->X3_F3:="CCQ"
		SX3->X3_VLDUSER:="Vazio() .OR. ExistCpo('CCQ')"
	SX3->(MsUnlock())
	
	cTexto+="Atualizado campo B1_CODISS para consulta a tabela CCQ"+NL
endif
*/
cTexto+="Carga Inicial da tabela Z32"+NL
...
AADD(aSix,{'Z32','1','Z32_FILIAL+Z32_CODIGO','Codigo','Codigo','Code','U','','','S'})
AADD(aSix,{'Z32','2','Z32_FILIAL+Z32_DESCRI','Descricao','Descricao','Description','U','','','S'})
...
AADD(aSX2,{'Z32','\SYSTEM\','Z32YY0','Segmento/Ramo Atividade','Segmento/Ramo Atividade','Segment/Branch Activity','','C','','','','','','','','','',''})
...
AADD(aSX3,{'Z32','01','Z32_FILIAL','C','2','','Filial','Filial','Filial do Si','Filial','Filial','Filial','','','€€€€€€€€€€€€€€€','','','','™€','','','U','N','','','','','','','','','','','033','1','','','','','','','',''})
AADD(aSX3,{'Z32','02','Z32_CODIGO','C','6','','Codigo','Codigo','Code','Codigo','Codigo','Code','','','€€€€€€€€€€€€€€ ','GetSx8Num("Z32","Z32_CODIGO")','','','›€','','S','U','S','V','R','','','','','','','','','','1','','','','','','','',''})
AADD(aSX3,{'Z32','03','Z32_DESCRI','C','100','','Descricao','Descricao','Description','Descricao','Descricao','Description','','','€€€€€€€€€€€€€€ ','','','','›€','','S','U','S','A','R','','','','','','','','','','1','','','','','','','',''})
AADD(aSX3,{'Z32','04','Z32_USERGI','C','17','','Log de Inclu','Log de Inclu','Log de Inclu','Log de Inclusao','Log de Inclusao','Log de Inclusao','','','€€€€€€€€€€€€€€€','','','9','þÀ','','','L','N','V','R','','','','','','','','','','','','','','','N','N','',''})
AADD(aSX3,{'Z32','05','Z32_USERGA','C','17','','Log de Alter','Log de Alter','Log de Alter','Log de Alteracao','Log de Alteracao','Log de Alteracao','','','€€€€€€€€€€€€€€€','','','9','þÀ','','','L','N','V','R','','','','','','','','','','','','','','','N','N','',''})

****************************************************************************************************************************************** 
//{SX7} - Gatilhos
//AADD(aSX7,{X7_CAMPO,X7_SEQUENC,X7_REGRA,X7_CDOMIN,X7_TIPO,X7_SEEK,X7_ALIAS,X7_ORDEM,X7_CHAVE,X7_CONDIC,X7_PROPRI})

AADD(aSX7,{"A1_P_RMATI","001",'POSICIONE("Z32",1,xFilial("Z32")+M->A1_P_RMATI,"Z32_DESCRI")',"A1_P_DRMAT","P","N","","","","","U"})
AADD(aSX7,{"US_P_RMATI","001",'POSICIONE("Z32",1,xFilial("Z32")+M->US_P_RMATI,"Z32_DESCRI")',"US_P_DRMAT","P","N","","","","","U"})
...
AADD(aSXB,{'Z32','1','01','DB','Ramo Atividade','Ramo Atividade','Branch Activity','Z32',''})
AADD(aSXB,{'Z32','2','01','02','Descricao','Descricao','Description','',''})
AADD(aSXB,{'Z32','2','02','01','Codigo','Codigo','Code','',''})
AADD(aSXB,{'Z32','4','01','01','Descricao','Descricao','Description','Z32_DESCRI',''})
AADD(aSXB,{'Z32','4','01','02','Codigo','Codigo','Code','Z32_CODIGO',''})
AADD(aSXB,{'Z32','4','02','01','Codigo','Codigo','Codigo','Z32_CODIGO',''})
AADD(aSXB,{'Z32','4','02','02','Descricao','Descricao','Description','Z32_DESCRI',''})
AADD(aSXB,{'Z32','5','01','','','','','Z32->Z32_CODIGO',''}) 
