...
aAdd(aParamBox, {1,"Usuário Até:",Space(6),"@!","","USR",".T.",6,.F.} )
//  [2] : Descrição
		...
		For nY := 1 to Len(aUsers[nI][1][10])
			...
			For nX := 1 to Len(aMnuGrp)
				If Upper(Alltrim(SubStr(aMnuGrp[nX],3,1))) <> "X"
					...
					aAdd(aDados[nPUser][9],{cMenu,Val(SubStr(aMnuGrp[nX],1,2)),.T.})
					
					If aScan(aMenus,{|x| x[1] == cMenu }) == 0
						aAdd(aMenus,{cMenu,XNULoad(cMenu),Val(SubStr(aMnuGrp[nX],1,2)),.T.})
					EndIf
				EndIf
			Next nX
		Next nY
	...
	If cTpGrpUsr == "2" .Or. cTpGrpUsr == "3"
		For nY := 1 to Len(aUsers[nI][3])
			If Upper(Alltrim(SubStr(aUsers[nI][3][nY],3,1))) <> "X"
				...
				aAdd(aDados[nPUser][9],{cMenu,Val(SubStr(aUsers[nI][3][nY],1,2)),.F.})
				
				If aScan(aMenus,{|x| x[1] == cMenu }) == 0
					aAdd(aMenus,{cMenu,XNULoad(cMenu),Val(SubStr(aUsers[nI][3][nY],1,2)),.F.})
				EndIf
			EndIf
		Next nY
	EndIf
...
For nI := 1 to Len(aMenuUsr)
	...
	//Tratamento para o mesmo modulo no usuario, sendo ele no grupo e no cadastro do usuario.
	If aMenuUsr[nI][3]
		cMenu := cMenu + " - Grupo"
	EndIf
	...
	oExcel:AddColumn(cMenu,cTitulo,"Níveis de Acesso",1,1)
...
aAdd(aPerUsr,{23	,"Substituiçäo de Tìtulos"																	})
...
aAdd(aPerUsr,{28	,"Borderô Cheques Pré-Dat."																	})
...
aAdd(aPerUsr,{39	,"Form.Preços Todos Níveis"																	})
aAdd(aPerUsr,{40	,"Configura Venda Rapida"																	})
aAdd(aPerUsr,{41	,"Abrir/Fechar Caixa"																		})
aAdd(aPerUsr,{42	,"Excluir Nota/Orç. LOJA"																	})
...
aAdd(aPerUsr,{132	,"Criar Estrutura com base em Pré Estrutura"												})
...
aAdd(aPerUsr,{173	,"Se acessa APSDU, é Read-Write"															})
