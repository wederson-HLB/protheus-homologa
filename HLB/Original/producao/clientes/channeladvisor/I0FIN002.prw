...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Emissao BoletoN Citibank     
Autor     	: João Silva	
Data     	: 01/08/2014
Obs         : 
TDN         : 
...
Private nLin       := 0
Private oFont1     := TFont ():New ("Arial", , 07, , .F., , , , , .f. )	// Nome do banco na 1a linha.
Private oFont2     := TFont ():New ("Arial", , 14, , .t., , , , , .f. )
Private oFont3     := TFont ():New ("Arial", , 16, , .t., , , , .t., .f. )
Private oFont4     := TFont ():New ("Arial", , 10, , .t., , , , , .f. )
Private oFont5     := TFont ():New ("Arial", , 09, , .F., , , , , .f. )	// Dados do campo "Instrucoes"
Private oFont6     := TFont ():New ("Arial", , 11, , .t., , , , , .f. )	// Numeracao do Codigo de Barra no cabecalho do terceiro boleto da folha
Private oFont7     := TFont ():New ("Arial", , 09, , .T., , , , , .f. )	// Ne      
Private oFont8     := TFont ():New("Times New Roman",9,20,.T.,.F.,5,.T.,5,.T.,.F.)
...
Static Function fImprime()
   ...
   //-------------------------------------------    
	...
	sBarra    := cBanco+cMoeda+cFatVenc+cValor+"3"+cPORTI+StrZero(Val(cCOSMO),9)+cNossoNum2
   ...
   While _nBarra > 0
	...
	EndIf  
	//----------------------------------------------
 	sBarra    := cBanco+cMoeda+cResto+cFatVenc+cValor+"3"+cPORTI+SubStr(cCOSMO,2,9)+cNossoNum2
	//---------------------------------------------- Formacao da linha digitavel:

	sDigit  :=""               
	sDigit1 :=cBanco+cMoeda+"3"+cPORTI+SubStr(cCOSMO,2,1)
	sDigit  :=sDigit1+CalcDig(sDigit1)
	sDigit2 :=SubStr(cCOSMO,3,8)+SubStr(cNossoNum2,1,2)
...
//      oPrn:SayBitmap (0000 + _nAjuste,0010,"LOGO745.bmp", 170, 070)
        oPrn:Say (0000 + _nAjuste,0010,"Citibank",oFont8,070)
		...
		oPrn:Say (0195 + _nAjuste,1670,"        " + AllTrim(cAgencia) + " / " + cCOSMO, oFont4, 100)
...
//AADD(aRegistros,{cPerg,"08","Impressora	   	","","","mv_ch8","N",01,00,00,"C","","Mv_Par08","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
