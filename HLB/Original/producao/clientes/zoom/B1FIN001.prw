...
±±³ Programa  ³ B1FIN001().PRW ³ Autor³ Anderson Arrais³ Data ³ 18/09/2015 ³±±
...
±±³ Uso       ³ FINANCEIRO    Mosaico                                      ³±±
±±³ Alteracoes³ Leandro Brito 05/10/2015 - Adpatado para projeto Zoom      ³±±
...
Data/Hora   : 18/09/2015
Módulo      : Financeiro.
*/

*-------------------------------------------------------------------*
USER FUNCTION B1FIN001( lPreview , cTitulo , cPrefixo )
*-------------------------------------------------------------------*
Local aArea			:= GetArea()
...
MontaRel( lPreview , cTitulo , cPrefixo )

RestArea( aArea )

Return( cFiles )

STATIC FUNCTION MontaRel( lPreview , cTitulo , cPrefixo )
LOCAL oPrint
LOCAL n         := 0
...
LOCAL aDadosBanco
LOCAL aDatSacado
LOCAL aBolText  := {"Não receber após o vencimento."}
LOCAL i         := 1
LOCAL CB_RN_NN  := {}
LOCAL nRec      := 0
LOCAL _nVlrAbat := 0
...
dbSelectArea ("SE1")
DbSetOrder(1)
DbSeek (xFilial("SE1")+cPrefixo+cTitulo, .T.)

While xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == cPrefixo .AND. SE1->E1_NUM == cTitulo
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "B1"     // ZOOM
			cBanco    := "341"
			cAgencia  := "2938 "
			cConta    := "145501  "
			cSubConta := "001"
	...
	IF EMPTY(SE1->E1_NUMBCO)
		...
		DbSelectArea("SE1")
		RecLock("SE1",.f.)
		SE1->E1_NUMBCO 	:= Strzero(val(cNroDoc),8) //MSM - 25/04/2017 - Tratamento para não gravar no E1, o nosso número menor que 8 dígitos.	//cNroDoc   // Nosso número (Ver fórmula para calculo)
		SE1->E1_PORTADO := SA6->A6_COD
		SE1->E1_AGEDEP  := SA6->A6_AGENCIA
		SE1->E1_CONTA   := SA6->A6_NUMCON
		MsUnlock()
	Else
		cNroDoc 	:= SubStr(ALLTRIM(SE1->E1_NUMBCO),5,8)
	EndIf
	DbSelectArea("SE1")
	dbGoTo(_nRecSE1)
	...
	cFileName := "Boleto_"+AllTrim( SE1->E1_NUM )+AllTrim( SE1->E1_PARCELA ) 
	...
	//oPrint:cFileName := "Boleto"+AllTrim( SE1->E1_NUM )+AllTrim( SE1->E1_PARCELA ) + '.rel'	
	...
	Else
		...
		Sleep( 5000 )
		lCompacta := .T.
		If CpyT2S(cLocal+cFileName+".pdf",cDirBol,lCompacta) 
			If !Empty( cFiles )
				cFiles += ";" 
			EndIf
			cFiles += cDirBol + cFileName+".pdf"
		Else
...
oPrint:Say (0084+_nLin2,0100,aDadosBanco[2],oFont16)				// [2] Nome do Banco
...
oPrint:Say (_nLin+_nLin2+0654,0100,aDadosBanco[2],oFont16)     	// [2]Nome do Banco
...
oPrint:Say (_nLin+_nLin2+0940,1050,"DS",oFont10) //Tipo do Titulo
...
oPrint:Say (_nLin+_nLin2+1934,0100,aDadosBanco[2],oFont16) 	// [2] Nome do Banco
...
oPrint:Say (_nLin+_nLin2+2230,1050,"DS",oFont10)					//Tipo do Titulo
...
//oPrint:Say (_nLin+_nLin2+2340,0100,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do Beneficiário)",oFont8)
...
oPrint:FWMSBAR("INT25" /*cTypeBar*/,65.2/*nRow*/ ,2/*nCol*/, CB_RN_NN[1]/*cCode*/,oPrint/*oPrint*/,.F./*lCheck*/,/*Color*/,.T./*lHorz*/,0.017/*nWidth*/,0.95/*0.8*//*nHeigth*/,.F./*lBanner*/, cFont/*cFont*/,NIL/*cMode*/,.F./*lPrint*/,2/*nPFWidth*/,2/*nPFHeigth*/,.F./*lCmtr2Pix*/)
