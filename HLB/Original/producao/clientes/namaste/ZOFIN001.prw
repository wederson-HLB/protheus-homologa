...
±±³ Programa  ³ ZOFIN001().PRW ³ Autor ³               ³ Data ³ 21/09/2011 ³±±
...
TDN         : 
Revisão     : João Silva 
Data/Hora   : 29/11/2013
Módulo      : Financeiro.
*/    

*--------------------------*
  USER FUNCTION ZOFIN001()     
*--------------------------*
...
While xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "ZO/99"     // NAMASTE
			cBanco    := "399"
			cAgencia  := "1879 "
			cConta    := "0037184   "
			cSubConta := "001"	  
...
oFont24t := TFont():New("Times New Roman",9,24,.T.,.F.,5,.T.,5,.T.,.F.)
oBrush  := TBrush():New("",4)
