...
Parametros  : 
Retorno     : 
Objetivos   : Impressao de Boleto Bancario do Banco Bradesco com Codigo de Barras, Linha Digitavel e Nosso Numero. Baseado no Fonte TBOL001 de 01/08/2002 de Raimundo Pereira.  
Autor       : João Silva
Data/Hora   : 30/10/2014
TDN         : 
Módulo      : Financeiro.
*/ 

*-----------------------*
 USER FUNCTION 0FFIN001()  
*-----------------------*
...
If Alltrim(SM0->M0_CODIGO) $ "0F"//NTT DO BRASIL 
	cPortGrv    := "237"
	cAgeGrv     := "2374 "
	cContaGrv   := "589799    "
EndIf
...
WHILE SQL->(!EOF())
	...
	aDadosBanco := {"237",;														// [1]Numero do Banco
					...
					SUBSTR(SA6->A6_AGENCIA,1,5),;								// [3]Agencia
	...
	CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",SUBSTR(aDadosBanco[3],1,4),aDadosBanco[4],aDadosBanco[5],cNroDoc,(E1_SALDO-_nVlrAbat-E1_DECRESC+E1_ACRESC),E1_VENCREA)
...
Local oBrush       
Local cBraLogo:="LOGO237.bmp"//"bradesco10.bmp"
...
oPrint:Say (0200,1060,SUBSTR(aDadosBanco[3],1,4)+"-4"+SUBSTR(aDadosBanco[3],5,1)+"/00"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
...
oPrint:Say (_nLin+0850,1950,SUBSTR(aDadosBanco[3],1,4)+"-4"+SUBSTR(aDadosBanco[3],5,1)+"/00"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
...
oPrint:Say (_nLin+1295,1950,AllTrim(Transform(aDadosTit[10],"@E 999,999,999.99")),oFont10)
oPrint:Say (_nLin+1330,1910,"(=)Valor Cobrado",oFont8) 
oPrint:Say (_nLin+1365,1950,AllTrim(Transform(aDadosTit[5]-aDadosTit[9]+aDadosTit[10],"@E 999,999,999.99")),oFont10)
...
oPrint:Say (_nLin+1589,1950,SUBSTR(aDadosTit[6],1,2)+"/"+SUBSTR(aDadosTit[6],4,len(alltrim(aDadosTit[6]))-3),oFont10)
...
oPrint:Say (_nLin+2140,1950,SUBSTR(aDadosBanco[3],1,4)+"-4"+SUBSTR(aDadosBanco[3],5,1)+"/00"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
...
oPrint:Say (_nLin+2585,1950,AllTrim(Transform(aDadosTit[10],"@E 999,999,999.99")),oFont10)
oPrint:Say (_nLin+2620,1910,"(=)Valor Cobrado",oFont8)
oPrint:Say (_nLin+2655,1950,AllTrim(Transform(aDadosTit[5]-aDadosTit[9]+aDadosTit[10],"@E 999,999,999.99")),oFont10)
...
oPrint:Say (_nLin+2879,1950,SUBSTR(aDadosTit[6],1,2)+"/"+SUBSTR(aDadosTit[6],4,len(alltrim(aDadosTit[6]))-3),oFont10)
