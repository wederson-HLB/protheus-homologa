...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Financeiro - Emissao Boleto do Banco Real com Registro 
Autor     	: Vitor Bedin
Data     	: 26/08/10
TDN         : 
...
// fCriaPerg()
   ...
   Endif
Else
    MsgInfo("Especifico Neogen ","A T E N C A O ")
Endif   
...
//cValor+substr(cAgencia,1,4)+substr(cContaEmp,1,7)+AllTrim(STR(nDigT))+StrZero(val(cNumero),13)
	...
	For _nCont := 1 to 3                      
      ...
      oPrn:SayBitmap (0000 + _nAjuste,0010,"\REAL\BancoRealSantander.jpg", 220, 070)
		...
		oPrn:Say (0195 + _nAjuste,0010,"Neogen do Brasil - 10.383.329/0001-32", oFont4, 100)
		...
		oPrn:Say (0275 + _nAjuste,1070,"S", oFont4, 100)
		...
		If ! Empty (cMsgBoleto)
			oPrn:Say (0555 + _nAjuste,0010,SUBSTR(cMsgBoleto,1,76), oFont5, 100)
			oPrn:Say (0595 + _nAjuste,0010,SUBSTR(cMsgBoleto,77,44), oFont5, 100)
		EndIf
//		If nJuros > 0
...
//	                        
		oPrn:Say (0665 + _nAjuste,0010, "APOS O VENCIMENTO COBRAR MULTA DE R$ "+Alltrim(Transform(nMultaMensal, "@E 99,999,999.99")) + " E MORA DIÁRIA DE R$ " + Alltrim(Transform(nJuros, "@E 99,999,999.99")), oFont5, 100)
...
//		oPrn:Say (0440 + _nAjuste,1850,Transform (nAbatim, "@E@Z 999,999,999,999.99"), oFont4, 100)  //NAO USADO NA PRYOR
	    ...
	    If Len(AllTrim(SA1->A1_CGC)) == 14 
          cCgc := SA1 -> (Transform (A1_CGC, "@R 99.999.999/9999-99"))
        ElseIf Len(AllTrim(SA1->A1_CGC)) == 11   
          cCgc := SA1 -> (Transform (A1_CGC, "@R 999.999.999-99"))
        Else   
          cCgc := SA1->A1_CGC
        Endif 
		
		oPrn:Say (0805 + _nAjuste,0260,SA1 -> (Alltrim (A1_NOME) + " - " + A1_COD + "/" + A1_LOJA + " - " + cCgc), oFont4, 100)
