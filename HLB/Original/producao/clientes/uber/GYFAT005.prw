...
Objetivo...........: Importar Xml e Txt de Notas de Prestação de Serviço RJ
Cliente HLB........: Uber
Data...............: 23/09/2017
*/                                                  
*--------------------------------------* 
User Function GYFAT005( aParam )
*--------------------------------------*                                     
...
While nLidos < nTotArq
	...
	EndIf
   	...
   	StartJob('u_GYImpRJ' , GetEnvServer() , .F. , aArqInt , nInicio , nFim , aFilEmp , cEmpAnt , cFilAnt , cUserName , cIdInt ,cTime)  
...
*--------------------------------------------------------------------------------------------*
User Function GYImpRJ( aArqInt , nInicio , nFim , aFilEmp , cEmp , cFil , cUser , cId, cTime)
*--------------------------------------------------------------------------------------------*
Local cXml
Local oXml
Local aNotas
		...
		Case ( cLayout == '3' )	  //** Ex. : RJ ( TXT )	
			...
			While nLidos < nTamArq
				...
				If ( SubStr( cLinha , 1 , 1 ) == '2' )
					...
					cUF 	:= SubStr( cLinha , 1192 , 02 )  //ok
					cMun	:= NoAcento( SubStr( cLinha , 1142 , 50 ) ) //ok
					cCodMun := ""
					...
					aDadosCli := {  cCodMun ,; // Codigo Municipio
									...
									AllTrim( SubStr( cLinha , 1000 , 10 ) ) ,; // Numero ok
									cUF ,; // UF
									AllTrim( SubStr( cLinha , 1070 , 72 ) ) ,; // Bairro ok
									AllTrim( SubStr( cLinha , 875 , 125 ) ) ,; // Endereço ok
									cMun ,; // Nome Municipio
									AllTrim( SubStr( cLinha , 757 , 115 ) ) ,; // Nome Cliente ok    
									AllTrim( SubStr( cLinha , 713 , 14 ) ) ,; // CNPJ/CPF ok     
									AllTrim( SubStr( cLinha , 1194 , 08 ) ) ,; // CEP ok     								
	                                ""; //** ID Estrangeiro
					               }
					...
					Aadd( aDadosNF , {  StrZero( 1 , Len( SD2->D2_ITEM ) ) ,;  // Seq. Item
										...
										1 ,; // Quantidade  
										Val( AllTrim( SubStr( cLinha , 1380 , 15 ) ) ) / 100 ,; // Valor Unitario ok
										Val( AllTrim( SubStr( cLinha , 1380 , 15 ) ) ) / 100 ,; // Valor Total   ok
					                    Val( AllTrim( SubStr( cLinha , 1375 , 05 ) ) ) / 100 ,; // Aliq. ISS  ok
					                    Val( AllTrim( SubStr( cLinha , 1380 , 15 ) ) ) / 100 ,; // Base Calculo ISS ok
					                    Val( AllTrim( SubStr( cLinha , 1530 , 15 ) ) ) / 100 ,;   // Valor ISS ok
					                    Val( AllTrim( SubStr( cLinha , 1500 , 15 ) ) ) / 100 ,; // PIS ok
					                    Val( AllTrim( SubStr( cLinha , 1440 , 15 ) ) ) / 100 ,; // Cofins ok
					                    "" ,;   // Chave NFE
					                    StrZero( Val( SubStr( cLinha , 3 , 15 ) ) , 9 ) ,; // Numero NF ok
					                    'RPS' ,; // Serie NF
					                    StoD( SubStr( cLinha , 63 , 08 ) ) ,;  // Emissao
					                    StoD( SubStr( cLinha , 63 , 08 ) ) ,;
					                    StrZero( Val( SubStr( cLinha , 3 , 15 ) ) , 9 );
					                   } )
				EndIf						
