...
//SELPORT()
If Alltrim(SM0->M0_CODIGO)=="FB" //Rokonet
	cPortGrv    := "033"
	cAgeGrv     := "3853 "
	cContaGrv   := "130007422"
	cCodEmp		:= "6540899"
...
WHILE SQL->(!EOF())
	...
	EndIf
	//JSS - Add tratamento para solucionar o caso 022085  	
	If (dDataBase-E1_VENCREA)>0 
		CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],substr(cNroDoc,1,len(cNroDoc)-1),(E1_SALDO - _nVlrAbat-E1_DECRESC+E1_ACRESC+(dDataBase-E1_VENCREA)*((E1_SALDO*0.01)/30)+(E1_SALDO*0.02)),E1_VENCREA) // JSS - 13/08/2014 - Alterado de (E1_VALOR - _nVlrAbat) para (E1_SALDO - _nVlrAbat) para solucionar o caso 020765.
	Else
		CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],substr(cNroDoc,1,len(cNroDoc)-1),(E1_SALDO - _nVlrAbat-E1_DECRESC+E1_ACRESC),E1_VENCREA) // JSS - 13/08/2014 - Alterado de (E1_VALOR - _nVlrAbat) para (E1_SALDO - _nVlrAbat) para solucionar o caso 020765.	
	EndIf    
...
//JSS - Add tratamento para solucionar o caso 022085    
If (dDataBase-aDadosTit[4])>0 
	oPrint:Say (_nLin+1220,1950,AllTrim(Transform((dDataBase-aDadosTit[4])*((aDadosTit[5]*0.01)/30)+(aDadosTit[5]*0.02),"@E 99,999.99")) ,oFont10) 
EndIf	
...
If (dDataBase-aDadosTit[4])>0 
	oPrint:Say (_nLin+1360,1950,AllTrim(Transform(aDadosTit[5]+(dDataBase-aDadosTit[4])*((aDadosTit[5]*0.01)/30)+(aDadosTit[5]*0.02)+aDadosTit[10]-aDadosTit[9],"@E 999,999,999.99")),oFont10)
Else 
	oPrint:Say (_nLin+1360,1950,AllTrim(Transform(aDadosTit[5]+aDadosTit[10]-aDadosTit[9],"@E 999,999,999.99")),oFont10)
EndIf
...
//JSS - Add tratamento para solucionar o caso 022085    
If (dDataBase-aDadosTit[4])>0 
	oPrint:Say (_nLin+2510,1950,AllTrim(Transform((dDataBase-aDadosTit[4])*((aDadosTit[5]*0.01)/30)+(aDadosTit[5]*0.02),"@E 99,999.99")) ,oFont10) 
EndIf
...
If (dDataBase-aDadosTit[4])>0 
	oPrint:Say (_nLin+2650,1950,AllTrim(Transform(aDadosTit[5]+(dDataBase-aDadosTit[4])*((aDadosTit[5]*0.01)/30)+(aDadosTit[5]*0.02)+aDadosTit[10]-aDadosTit[9],"@E 999,999,999.99")),oFont10)
Else 
	oPrint:Say (_nLin+2650,1950,AllTrim(Transform(aDadosTit[5]+aDadosTit[10]-aDadosTit[9],"@E 999,999,999.99")),oFont10)
EndIf
