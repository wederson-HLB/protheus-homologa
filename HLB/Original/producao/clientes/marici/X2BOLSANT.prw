...
*-----------------------------------------------------------*
USER FUNCTION X2BOLSANT(lSelAuto,cVer1)   
*-----------------------------------------------------------*
...
if !lSelAuto
	...
	Aadd(aPergs,{"De Numero","","","mv_ch1","C",9,0,0,"G","","MV_PAR01","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
	Aadd(aPergs,{"Ate Numero","","","mv_ch2","C",9,0,0,"G","","MV_PAR02","","","","ZZZZZZZZZ","","","","","","","","","","","","","","","","","","","","","","","","",""})
	Aadd(aPergs,{"Do Vencimento","","","mv_ch3","D",8,0,0,"G","","MV_PAR03","","","","01/01/19","","","","","","","","","","","","","","","","","","","","","","","","",""})
	Aadd(aPergs,{"Ate Vencimento","","","mv_ch4","D",8,0,0,"G","","MV_PAR04","","","","31/12/20","","","","","","","","","","","","","","","","","","","","","","","","",""})
	
	AjustaSx1("X2FIN001",aPergs)
...
BeginSql Alias 'SQL'
	...
	WHERE %notDel%
	  ...
	  AND E1_VENCTO  >= %exp:MV_PAR03% AND E1_VENCTO <= %exp:MV_PAR04%
	  AND E1_TIPO IN ('NF','BOL')
	ORDER BY E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_EMISSAO
EndSql
...
LOCAL nRec     	 	:= 0
LOCAL _nVlrAbat 	:= 0
//Local cLocal    	:= "C:\FIN004\"  //GetTempPath()
Local cFileName 	:= ""
Local cFile 		:= ""
...
cFileName := "Boleto_Santander_" + AllTrim(  SQL->E1_NUM )
...
WHILE SQL->(!EOF())
	...
	aDadosBanco := {"033",;													  //SA6->A6_COD [1]Numero do Banco
					...
					SA6->A6_DVCTA,;		  									  // [5]Digito da conta corrente                 
	...
	IF EMPTY(SE1->E1_NUMBCO)
		...
		dbSetOrder(1)
		dbGoTop()
		If dbSeek(xFilial("SEE")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA)+'010')
		//If dbSeek(xFilial("SEE")+aDadosBanco[1]+aDadosBanco[3]+" "+aDadosBanco[4]+aDadosBanco[5]+Space(TamSX3("A6_NUMCON")[1]-Len(aDadosBanco[4]+aDadosBanco[5]))+'010')
			RecLock("SEE",.F.)
...
oPrint:Say (nSalto+_nLin+1200,0100,aBolText[1]+Transform((aDadosTit[5]*0.02),"@E 9,999.99")+" de multa.",oFont10)
oPrint:Say (nSalto+_nLin+1250,0100,aBolText[2]+Transform((aDadosTit[5]*0.33/30),"@E 9,999.99")+" de juros ao dia.",oFont10)
...
oPrint:Say (nSalto+_nLin+2490,0100,aBolText[1]+Transform((aDadosTit[5]*0.02),"@E 9,999.99")+" de multa.",oFont10)
oPrint:Say (nSalto+_nLin+2540,0100,aBolText[2]+Transform((aDadosTit[5]*0.33/30),"@E 9,999.99")+" de juros ao dia.",oFont10)
