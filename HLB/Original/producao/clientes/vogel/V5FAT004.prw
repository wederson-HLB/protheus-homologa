...
*/
*-----------------------------------------------*
 User Function V5FAT004( lV5FAT, lVisua )
*-----------------------------------------------*
Local lExec         := .T.
...
oPrinter:SayAlign(nLin, nCol7c, "MODELO 22",oFont8, 500, 500, CLR_BLACK, 0, 1 )
...
oPrinter:SayAlign(nLin, nCol7c, "DE SERVIÇO DE TELECOMUNICAÇÃO",oFont8n, 500, 500, CLR_BLACK, 0, 1 )
nLin += nSalto*2

//Primeiro Box
oPrinter:SayBitmap(nLin , 241 , "\system\V5\imagens\fundo5.png", 325,66)
...
oPrinter:SayAlign(nLin, nCol5c, "MÊS/ANO REF",oFont9n, 500, 500, CLR_BLACK, 0, 1 )
...
oPrinter:SayAlign(nLin, nCol6c, "CONTA CONSUMO",oFont9n, 500, 500, CLR_BLACK, 0, 1 )
...
//Mascara para telefone 0800
If SubStr(Alltrim(SC5->C5_P_CONTA),1,4) == "0800" 
	oPrinter:SayAlign(nLin+2, nCol6c, Alltrim(Transform(SC5->C5_P_CONTA,"@R 9999 999 9999")),oFont12, 500, 500, CLR_BLACK, 0, 1 )
Else
	oPrinter:SayAlign(nLin+2, nCol6c, Alltrim(Transform(SC5->C5_P_CONTA,"@R 99 9999.9999")),oFont12, 500, 500, CLR_BLACK, 0, 1 )
EndIf
...
dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery1),"AliasST",.T.,.T.)
...
nLin2 := nLin
//Ato Cotepe 
//RPB - Alteração de texto para ATO COTEPE 23/02/2017
If SA1->A1_TPESSOA == 'AT'
	oPrinter:SayAlign(nLin2, nCol1i, "ICMS diferido, nos termos da Cláusula Primeira do Convênio nº 17/2013",oFont8n, 600, 500, CLR_BLACK, 0, 1 ) 
    nLin2 += nSalto
EndIf
//Mensagem Impostos
If SA1->A1_TPESSOA == 'ER'
	oPrinter:SayAlign(nLin2, nCol1i, "Imposto(s) Retido(s): IR("+Alltrim(Str(AliasST->D2_ALQIRRF))+"%), PIS("+Alltrim(Str(AliasST->D2_ALQPIS))+"%), COFINS("+Alltrim(Str(AliasST->D2_ALQCOF))+"%), CSLL("+Alltrim(Str(AliasST->D2_ALQCSL))+"%) - R$ "+Alltrim(Transform(SQL->(F2_VALCSLL+F2_VALCOFI+F2_VALPIS+F2_VALIRRF),"@E 999,999,999,999.99"))+".",oFont8n, 600, 500, CLR_BLACK, 0, 1 ) 
	nLin2 += nSalto
EndIf
...
oPrinter:SayAlign(nLin+3, nCol4r, Alltrim(Str(AliasST->D2_ALQCOF+AliasST->D2_ALQPIS))+"%",oFont8n, 60, 0, CLR_BLACK, 2, 1 )
oPrinter:SayAlign(nLin+3, nCol5r, Transform(AliasST->BASE,"@E 999,999,999,999.99"),oFont8n, 155, 0, CLR_BLACK, 2, 1 )
oPrinter:SayAlign(nLin+3, nCol7r, Transform(AliasST->VALOR,"@E 999,999,999,999.99"),oFont8n, 90, 0, CLR_BLACK, 1, 1 )
nLin += nSalto2
...
oPrinter:SayAlign(nLin+3, nCol5r, Transform(nBaseMt+nBaseJr+AliasST->BASE,"@E 999,999,999,999.99"),oFont8n, 155, 0, CLR_BLACK, 2, 1 )
oPrinter:SayAlign(nLin+3, nCol7r, Transform(nValMt+nValJr+AliasST->VALOR,"@E 999,999,999,999.99"),oFont8n, 90, 0, CLR_BLACK, 1, 1 )
nLin += nSalto2
oPrinter:Line( nLin , nCol1i, nLin , nCol8r, CLR_RGB_VERD1 )
//RPB - inclisão do complemento de texto incidentes sobre os serviços de telecomunicação - 23/02/2017 
oPrinter:SayAlign(nLin+2, nCol1i, "As contribuições ao FUST (1%) e FUNTTEL (0,5%), incidentes sobre os serviços de telecomunicação, não são repassadas às tarifas.",oFont8, 500, 500, CLR_BLACK, 0, 1 )
