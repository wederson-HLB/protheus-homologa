...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Nota Fiscal Werfen - Entrada e Saída  
...
If cEmpAnt $ "88"
   ...
   Endif
Else
    MsgInfo("Especifico Werfen ","A T E N C A O")  
Endif   
...
tamanho  :='G'
limite   :=220
titulo   :="Nota Fiscal - Entrada / Saida - Werfen"
cDesc1   :=' '
cDesc2   :=''
...
If _cTpMov == 1
   fGerSf1()
   RptStatus({|| fImpSF1()},"Nota de Entrada - Werfen")
Else
   fGerSf2()
   RptStatus({|| fImpSF2()},"Nota de Saida - Werfen")
Endif   
...
Do While.Not.Eof() .And. cTipo <> "C"
   ...
   If AllTrim(cTipo)$ "C"
     	...
     	@ nLin,099 pSay nVALBRUT  Picture "@E@Z 999,999,999.99"   //Vlr Total (corpo da nota)
     	nLin:= 57
     	@ nLin,120 pSay nVALBRUT Picture "@E 999,999,999.99" //Vlr Total Nota (rodapé)
     	
     	
     	IF !empty(cMensagem)
		   @ 071,002 pSay SUBSTR(cMensagem,1,70)
		   @ 072,002 pSay SUBSTR(cMensagem,71,70)
		EndIF 
		                                   
		If !Empty(cMensTes)
			@ 073,002 pSay SUBSTR(cMensTes,1,70)
			@ 074,002 pSay SUBSTR(cMensTes,71,70)
		EndIf
   ...
   Elseif AllTrim(cTipo)$ "I"    
	      While F1_DOC+F1_SERIE == cCompara           
	   		...
	   		If Ascan(cText, SF4->F4_TEXTO)==0
				...
				Endif 	  		
	     	 	...
	     	 	//@ nLin,057 pSay SB1->B1_POSIPI                            	//Classificação fiscal
	      	...
	      	//@ nLin,077 pSay D1_QUANT  Picture "@E@Z 999,999.99"       	//Quantidade
	       	//@ nLin,086 pSay D1_VUNIT  Picture "@E@Z 99,999,999.99"    	//Vlr Unitário    
	       	//@ nLin,099 pSay D1_TOTAL  Picture "@E@Z 999,999,999.99"   	//Vlr Total
	        	//@ nLin,118 pSay D1_PICM   Picture "99"               			//%ICMS    
	        	//@ nLin,123 pSay D1_IPI    Picture "99"               			//%IPI
	        	//@ nLin,127 pSay D1_VALIPI Picture "@E 99,999.99"          	//Vlr IPI
	     		IncRegua(F1_SERIE+" "+F1_DOC) 
	       	DbSkip()
	        	nLin  +=1
	        	If nLin == 60
	   	  		@ 078,000 pSay Chr(27)+"2"
	   			@ 079,000 pSay Chr(18)
	   	  		@ 080,072 pSay cNota
			  		@ 088,000 pSay ""   
	   	  		SetPrc(0,0)  
	      ...
	      EndDo
		  	...
		  	@ 054,038 pSay nVALICM  Picture "@E 999,999,999.99" //Vlr  ICMS
...
Static Function fCabSf1()
   ...
   If! AllTrim(cTipo) $ "B/D"
      ...
      If Len(AllTrim(SA2->A2_CGC)) == 14 
         @ 011,085 pSay SA2->A2_CGC Picture "@R 99.999.999/9999-99"
      ElseIf Len(AllTrim(SA2->A2_CGC)) == 11   
         @ 011,085 pSay SA2->A2_CGC Picture "@R 999.999.999-99"
      Else   
         @ 011,085 pSay SA2->A2_CGC
      Endif   
      @ 011,124 pSay Dtoc(cEMISSAO)
      @ 013,001 pSay SA2->A2_END
      @ 013,072 pSay SA2->A2_BAIRRO
      @ 013,101 pSay SA2->A2_CEP   Picture "@R 99.999-999"
      @ 015,001 pSay SA2->A2_MUN
      @ 015,053 pSay SA2->A2_TEL   
      @ 015,082 pSay SA2->A2_EST
      If AllTrim(SA2->A2_INSCR) == "ISENTO" 
         @ 015,087 pSay "ISENTO" 
      Else      
	      @ 015,087 pSay SA2->A2_INSCR Picture "@R 999.999.999.999"
      Endif       
	   Else   
      @ 011,001 pSay SA1->A1_NOME
      If Len(AllTrim(SA1->A1_CGC)) == 14 
         @ 011,085 pSay SA1->A1_CGC Picture "@R 99.999.999/9999-99"
      ElseIf Len(AllTrim(SA1->A1_CGC)) == 11   
         @ 011,085 pSay SA1->A1_CGC Picture "@R 999.999.999-99"
      Else   
         @ 011,085 pSay SA1->A1_CGC
      Endif   
      @ 011,124 pSay Dtoc(cEMISSAO)
      @ 013,001 pSay SA1->A1_END
      @ 013,072 pSay SA1->A1_BAIRRO
      @ 013,101 pSay SA1->A1_CEP   Picture "@R 99.999-999"
      @ 015,001 pSay SA1->A1_MUN
      @ 015,053 pSay SA1->A1_TEL   
      @ 015,082 pSay SA1->A1_EST
      If AllTrim(SA1->A1_INSCR) == "ISENTO" 
         @ 015,087 pSay "ISENTO" 
      Else      
         @ 015,087 pSay SA1->A1_INSCR Picture "@R 999.999.999.999"  
      Endif       
   Endif
 ...
 IF Alltrim(cTipo) $ "N/D/C/B"
   While nMerc <= Len(aMerc)
         @ nLin,000 pSay aMerc[nMerc][01]                                 //Código Produto
			...
			ENDIF
	     @ nLin,057 pSay aMerc[nMerc][03]			                          //Classificação Fiscal
	     @ nLin,071 pSay aMerc[nMerc][04]                                 //Situação Tributária
         @ nLin,076 pSay aMerc[nMerc][05]                                 //Unidade
         @ nLin,077 pSay aMerc[nMerc][06] Picture "@E@Z 999,999.99"       //Quantidade
         @ nLin,086 pSay aMerc[nMerc][07] Picture "@E@Z 9,999,999.99"     //Vlr Unitário    
         @ nLin,099 pSay aMerc[nMerc][08] Picture "@E@Z 9,999,999.99"     //Vlr Total
         @ nLin,118 pSay aMerc[nMerc][09] Picture "99"               	  //% ICMS    
         @ nLin,123 pSay aMerc[nMerc][10] Picture "99"               	  //% IPI
...
//TRANSPORTADORA
   ...
   @ 063,073 pSay SA4->A4_MUN
   @ 063,106 pSay SA4->A4_EST
   ...
   @ 070,001 pSay SUBSTR(SC5->C5_MENNOTA,1,70)         
   
	FOR I:= 1 TO LEN(xMEN_TRIB)
      IF xMEN_TRIB[I] == "B"
			   @ 071,005 pSay ALLTRIM(xCLAS_FIS[I])
				EXIT
		ENDIF	   
   NEXT
   @ 071,001 pSay SUBSTR(SC5->C5_MENNOTA,71,70)   
	FOR I:= 1 TO LEN(xMEN_TRIB)
      IF xMEN_TRIB[I] == "C"
			   @ 072,005 pSay xCLAS_FIS[I]
				EXIT
		ENDIF	   
   NEXT
   @ 072,001 pSay SUBSTR(SC5->C5_MENNOTA,141,70)      
	FOR I:= 1 TO LEN(xMEN_TRIB)
      IF !Empty(xMEN_TRIB) .and. !xMEN_TRIB[I] $ "A/B/C/"
			   @ 073,002 pSay xMEN_TRIB[I]
		ENDIF	   
   NEXT
   @ 073,001 pSay SUBSTR(SC5->C5_MENNOTA,211,70)      
	If! Empty(cMensTes)
	   @ 074,001 pSay SUBSTR(cMensTes,1,72)
	   @ 075,001 pSay SUBSTR(cMensTes,73,100)
   ENDIF
 ...
 Elseif Alltrim(cTipo) = "P"                                                                
      ...
      End 
         ...
         @ 070,001  pSay SUBSTR(SC5->C5_MENNOTA,1,80)       
 Elseif Alltrim(cTipo) = "I"                                                                
         @ nLin,018 pSay "Complemento de ICMS"                         //Descrição Produto 
         @ 054,038  pSay nVALICM        Picture "@E 99,999.99"         //Vlr Total            
         //@ 057,120  pSay nVALICM        Picture "@E 999,999,999.99"    //Vlr ICMS Rodapé
         @ 071,005  pSay SUBSTR	(SC5->C5_MENNOTA,1,80) 
EndIf
...
Static Function fCabSf2()
   ...
   If! Alltrim(cTipo) $ "B/D"
       @ 011,001 pSay SA1->A1_NOME
       If Len(AllTrim(SA1->A1_CGC)) == 14 
          @ 011,85 pSay SA1->A1_CGC Picture "@R 99.999.999/9999-99"
       ElseIf Len(AllTrim(SA1->A1_CGC)) == 11   
          @ 011,85 pSay SA1->A1_CGC Picture "@R 999.999.999-99"
       Else   
          @ 011,85 pSay SA1->A1_CGC
       Endif   
       @ 011,124 pSay Dtoc(cEMISSAO)
       @ 013,001 pSay SA1->A1_END
       @ 013,072 pSay SA1->A1_BAIRRO
       @ 013,101 pSay SA1->A1_CEP   Picture "@R 99.999-999"
       @ 015,001 pSay SA1->A1_MUN
       @ 015,053 pSay SA1->A1_TEL  // Picture "@R (99)9999-9999"
       @ 015,082 pSay SA1->A1_EST
       If AllTrim(SA1->A1_INSCR) == "ISENTO" 
          @ 015,87 pSay "ISENTO" 
       Else      
          @ 015,87 pSay SA1->A1_INSCR Picture "@R 999.999.999.999"  
       Endif    
   Else
       @ 011,001 pSay SA2->A2_NOME
       If Len(AllTrim(SA2->A2_CGC)) == 14 
          @ 011,85 pSay SA2->A2_CGC Picture "@R 99.999.999/9999-99"
       ElseIf Len(AllTrim(SA2->A2_CGC)) == 11   
          @ 011,85 pSay SA2->A2_CGC Picture "@R 999.999.999-99"
       Else   
          @ 011,85 pSay SA2->A2_CGC
       Endif   
       @ 011,124 pSay Dtoc(cEMISSAO)
       @ 013,001 pSay SA2->A2_END
       @ 013,072 pSay SA2->A2_BAIRRO
       @ 013,101 pSay SA2->A2_CEP   Picture "@R 99.999-999"
       @ 015,001 pSay SA2->A2_MUN
       @ 015,053 pSay SA2->A2_TEL  // Picture "@R (99)9999-9999"
       @ 015,082 pSay SA2->A2_EST
       If AllTrim(SA2->A2_INSCR) == "ISENTO" 
          @ 015,87 pSay "ISENTO" 
       Else      
