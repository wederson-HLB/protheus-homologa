...
Data/Hora   : 22/08/2017
Módulo      : Financeiro.
Empresa		: Rogama
*/

*-----------------------------------------*
USER FUNCTION J2FIN001(lSelAuto,lPreview)
*-----------------------------------------*
Local aArea			:= GetArea()
...
LOCAL aBolText  := {"Após 5 dias do venc. protestar.",;
					"Após a data de venc. multa de 2% ao mês.",;
					"Após a data de venc. juros de 3% ao mês."}
LOCAL i         := 1
LOCAL CB_RN_NN  := {}
LOCAL nRec      := 0
LOCAL _nVlrAbat := 0
...
Local cDirBol	:= "\FTP\" + cEmpAnt + "\J2FIN001\"    
Local cNroDoc	:= ""
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If !Empty(SE1->E1_PORTADO) .AND. SE1->E1_PORTADO <> "341" 
		SE1->(dbSkip())
		Loop
	Endif 
	If ( SE1->E1_SALDO == 0 )
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "J2"//Rogama
			cBanco    := "341"
			cAgencia  := "0259 "
			cConta    := "610442  "
			cSubConta := "001"
...
oPrint:Say (_nLin+_nLin2+0750,0100,"ATE O VENCIMENTO EM QUALQUER BANCO OU CORRESPONDENTE NAO BANCARIO. APOS O VENCIMENTO,",oFont8)
oPrint:Say (_nLin+_nLin2+0775,0100,"ACESSE ITAU.COM.BR/BOLETOS E PAGUE EM QUALQUER BANCO OU CORRESPONDENTE NAO BANCARIO.",oFont8)
...
oPrint:Say (_nLin+_nLin2+2040,0100,"ATE O VENCIMENTO EM QUALQUER BANCO OU CORRESPONDENTE NAO BANCARIO. APOS O VENCIMENTO,",oFont8)
oPrint:Say (_nLin+_nLin2+2065,0100,"ACESSE ITAU.COM.BR/BOLETOS E PAGUE EM QUALQUER BANCO OU CORRESPONDENTE NAO BANCARIO.",oFont8)
