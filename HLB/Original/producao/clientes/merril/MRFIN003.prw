...
Objetivos   : Impressao de Boleto Bancario do Banco Bank of America Merrill Lynch com Codigo de Barras,
			  Linha Digitavel e Nosso NumeroBaseado no Fonte TBOL001 de 01/08/2002 de Raimundo Pereira.
Autor       : João Silva
Revisão     : 
Data/Hora   : 
Módulo      : Financeiro.
...
Tamanho  := "M"
titulo   := "Impressao de Boleto Bank of America
cDesc1   := "Este programa destina-se a impressao do Boleto Bank of America."
cDesc2   := ""
cDesc3   := ""
cString  := "SE1"
wnrel    := "BOL755MR"
...
LOCAL aDadosEmp := 	{	SM0->M0_NOME,;	                    											//[1]Nome da Empresa																				//[1]Nome da Empresa
...
WHILE SQL->(!EOF())
	...
	dbSelectArea("SEE")
	dbSetOrder(1)
	dbSeek(xFilial("SEE")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA)+"001",.T.)
	...
	aDadosBanco := {"755",;			 		// [1]Numero do Banco
					...
					"1306",;		 		// [3]Agencia
					"1035001-",;     		// [4]Conta Corrente
					"4",;	         		// [5]Digito da conta corrente
	...
	CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",SUBSTR(aDadosBanco[3],1,4),aDadosBanco[4],aDadosBanco[5],cNroDoc,(E1_SALDO-_nVlrAbat),E1_VENCREA) //JSS - Alterado de E1_VENCTO para E1_VENCREA Chamado 020937
...
LOCAL oBrush
Local cLogoBol:="BankOfAmerica.bmp"
...
oPrint:FillRect({_nLin+640,1445,_nLin+760 ,1900},oBrush)
oPrint:FillRect({_nLin+940,1445,_nLin+1033,1900},oBrush)
                         
//Logo
oPrint:SayBitmap(_nLin-77,0100,cLogoBol,260,80 )
oPrint:Say (_nLin-38	,1550,"Recibo do Sacado",oFont10)

//Sacador
oPrint:Line(_nLin  		,0100,_nLin+450	,0100)//Primeira coluna
oPrint:Line(_nLin		,1900,_nLin+365	,1900)//ultima coluna                                    

oPrint:Line(_nLin  		,0100,_nLin		,1900)//primeira linha                                                              
oPrint:Say (_nLin+003	,0106,"Nome do Cedente"   		,oFont8)
oPrint:Say (_nLin+040	,0106,aDadosEmp[1]					,oFont10)
oPrint:Say (_nLin+070	,0106,aDadosEmp[6]					,oFont10)//CNPJ
oPrint:Say (_nLin+100	,0106,ALLTRIM(aDadosEmp[2])+" - "+aDadosEmp[4]+" - "+aDadosEmp[3]	,oFont10)

oPrint:Line(_nLin+140	,0100,_nLin+140	,1900)//Segunda Linha 
oPrint:Say (_nLin+143	,0106,"Sacado/Avalista"			,oFont8)

oPrint:Line(_nLin+280	,0100,_nLin+280	,1900)//Terceira Linha 
oPrint:Line(_nLin+280	,1000,_nLin+365	,1000)//Primeira div. Quarta linha
oPrint:Line(_nLin+280	,1390,_nLin+365	,1390)//Segunda div. Quarta linha 
oPrint:Say (_nLin+283	,0106,"Nome do Sacado"		   		,oFont8)
oPrint:Say (_nLin+325	,0106,aDatSacador[1]					,oFont8n)
oPrint:Say (_nLin+283	,1006,"Data de Vencimento"	   		,oFont8)
oPrint:Say (_nLin+325	,1146,DTOC(aDadosTit[4])   	   		,oFont10)
oPrint:Say (_nLin+283	,1396,"Valor do Documento"	   		,oFont8)
oPrint:Say (_nLin+325	,1671,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)

oPrint:Line(_nLin+365	,0100,_nLin+365	,1900)//Quarta Linha
oPrint:Line(_nLin+365	,0590,_nLin+450	,0590)//Primeira div. Quinta linha
oPrint:Line(_nLin+365	,1080,_nLin+450	,1080)//Segunda div. Quinta linha 
oPrint:Say (_nLin+368	,0106,"Agência/Código do Cedente"	,oFont8)
//oPrint:Say (_nLin+410	,0136,SUBSTR(aDadosBanco[3],1,4)+"-"+SUBSTR(aDadosBanco[3],5,1)+"/"+aDadosBanco[4]+aDadosBanco[5],oFont10)
oPrint:Say (_nLin+410	,0136,STRZERO(VAL(aDadosBanco[7]),12),oFont10)
oPrint:Say (_nLin+368	,0596,"Nosso Número"				,oFont8)
oPrint:Say (_nLin+410	,0696,aDadosTit[6],oFont10)

oPrint:Line(_nLin+450	,0100,_nLin+450	,1080)//Quinta Linha

//Autenticação mecanica
oPrint:Line(_nLin+377	,1100,_nLin+377	,1900)
oPrint:Line(_nLin+377	,1100,_nLin+420	,1100)
oPrint:Line(_nLin+377	,1900,_nLin+420	,1900)
oPrint:Say (_nLin+380	,1400,"Autenticação Mecânica",oFont8) 
                 
//Pontilhado
FOR i := 100 TO 1900 STEP 20
	oPrint:Line(_nLin+535,i,_nLin+535,i+10)
NEXT i
                                                
//Boleto
//Logo
oPrint:SayBitmap(_nLin+560,0100,cLogoBol,260,80 )
oPrint:Line(_nLin+560	,0480,_nLin+640		,0480)
oPrint:Line(_nLin+560	,0630,_nLin+640		,0630)
oPrint:Say(_nLin+565	,0490,aDadosBanco[1]+"-2"	,oFont14n)// [1]Numero do Banco
oPrint:Say(_nLin+570	,0640,CB_RN_NN[2]			,oFont12n)// [2] Linha Digitavel do Codigo de Barras

         
oPrint:Line(_nLin+640	,0100,_nLin+640		,1900)//Primeira linha
oPrint:Line(_nLin+640	,0100,_nLin+1730	,0100)//Primeira COluna
oPrint:Say (_nLin+643	,0106,"Local de Pagamento"	,oFont8)
oPrint:Say (_nLin+690	,0116,"PAGAVEL EM QUALQUER BANCO ATE O VENCIMENTO"	,oFont10)
oPrint:Say (_nLin+720	,0116,"APOS O VENCIMENTO ACESSE HTTP://BOLETOS.BAML.COM ",oFont10)
oPrint:Say (_nLin+643	,1451,"Vencimento"			,oFont8)
oPrint:Say (_nLin+720	,1671,DTOC(aDadosTit[4])	,oFont10)

oPrint:Line(_nLin+760	,0100,_nLin+760				,1900)
oPrint:Say (_nLin+763	,0106,"Cedente"		   		,oFont8)    
oPrint:Say (_nLin+810	,0116,aDadosEmp[1]			,oFont10)
oPrint:Say (_nLin+763	,1451,"Agência/Código Cedente",oFont8)
//oPrint:Say (_nLin+810	,1481,SUBSTR(aDadosBanco[3],1,4)+"-"+SUBSTR(aDadosBanco[3],5,1)+"/"+aDadosBanco[4]+aDadosBanco[5],oFont10)
oPrint:Say (_nLin+810	,1481,STRZERO(VAL(aDadosBanco[7]),12),oFont10)

oPrint:Line(_nLin+850	,0100,_nLin+850		,1900)                 
...
oPrint:Say (_nLin+853	,0106,"Data do Documento"		,oFont8)
oPrint:Say (_nLin+900	,0136,DTOC(aDadosTit[2])		,oFont10)
oPrint:Say (_nLin+853	,0366,"Nro.Documento"			,oFont8)
oPrint:Say (_nLin+900	,0370,aDadosTit[7]+aDadosTit[1]	,oFont10)
oPrint:Say (_nLin+853	,0636,"Espécie Doc."			,oFont8) 
oPrint:Say (_nLin+900	,0656,aDadosTit[8]				,oFont10)
oPrint:Say (_nLin+853	,0919,"Aceite"			   		,oFont8)
oPrint:Say (_nLin+900	,0939,"Não"						,oFont10)
oPrint:Say (_nLin+853	,1141,"Data do Processamento"	,oFont8)
oPrint:Say (_nLin+900	,1171,DTOC(aDadosTit[3])		,oFont10)
oPrint:Say (_nLin+853	,1451,"Nosso Número"			,oFont8)
oPrint:Say (_nLin+900	,1480,aDadosTit[6],oFont10)

oPrint:Line(_nLin+940	,0100,_nLin+940		,1900)
oPrint:Line(_nLin+940	,0445,_nLin+1030	,0445)
oPrint:Line(_nLin+940	,0640,_nLin+1030	,0640)
oPrint:Line(_nLin+940	,0850,_nLin+1030	,0850)
oPrint:Line(_nLin+940	,1135,_nLin+1030	,1135)
oPrint:Say (_nLin+940	,0106,"Uso do Banco"			,oFont8)
oPrint:Say (_nLin+940	,0451,"Carteira"				,oFont8)
oPrint:Say (_nLin+990	,0481,aDadosBanco[6]			,oFont10)
oPrint:Say (_nLin+940	,0646,"Espécie Moeda"			,oFont8)
oPrint:Say (_nLin+990	,0676,"R$"						,oFont10)
oPrint:Say (_nLin+940	,0856,"Quantidade Moeda"		,oFont8) 
oPrint:Say (_nLin+990	,0886," "						,oFont10)
oPrint:Say (_nLin+940	,1141,"Valor Moeda"				,oFont8) 
//oPrint:Say (_nLin+990	,1220,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)
oPrint:Say (_nLin+940	,1451,"Valor do Documento"		,oFont8)
oPrint:Say (_nLin+990	,1671,AllTrim(Transform(aDadosTit[5],"@E 999,999,999.99")),oFont10)

oPrint:Line(_nLin+1030	,0100,_nLin+1030	,1900)
oPrint:Say (_nLin+1033	,0106,"Instruções"				,oFont8)
//oPrint:Say (_nLin+1233	,0116,"TITULO SUJEITO A PROTESTO APÓS 60 DIAS DO VENCIMENTO.",oFont10)  // JSS - 08/09/2014  Add mensagem para solucionar o caso 021238.
oPrint:Say (_nLin+1203	,0116,"Após o vencimento cobrar multa de R$ "+AllTrim(Transform((aDadosTit[5]*0.03),"@E 99,999.99")),oFont10)   
oPrint:Say (_nLin+1233	,0116,"Mora Diaria de R$ "+AllTrim(Transform((aDadosTit[5]*0.01/30),"@E 99,999.99")),oFont10)

oPrint:Line(_nLin+1120	,1445,_nLin+1120	,1900)
oPrint:Line(_nLin+1210	,1445,_nLin+1210	,1900)
oPrint:Line(_nLin+1300	,1445,_nLin+1300	,1900)
oPrint:Line(_nLin+1390	,1445,_nLin+1390	,1900)
oPrint:Say (_nLin+1036	,1451,"(-)Desconto/Abatimento"	,oFont8)
oPrint:Say (_nLin+1080	,1671,IF(aDadosTit[9]<>0,AllTrim(Transform(aDadosTit[9],"@E 999,999,999.99")),""),oFont10)
oPrint:Say (_nLin+1126	,1451,"(-)Outras Deduções"		,oFont8)
oPrint:Say (_nLin+1216	,1451,"(+)Mora/Multa"			,oFont8)
oPrint:Say (_nLin+1306	,1451,"(+)Outros Acréscimos"	,oFont8)
oPrint:Say (_nLin+1396	,1451,"(=)Valor Cobrado"		,oFont8)

oPrint:Line(_nLin+1480	,0100,_nLin+1480	,1900)
oPrint:Say (_nLin+1483	,0106,"Sacado"					,oFont8)
oPrint:Say (_nLin+1513	,0106,aDatSacador[1]+" "+TRANSFORM(aDatSacador[7],"@R 99.999.999/9999-99")	,oFont10)
oPrint:Say (_nLin+1543	,0106,aDatSacador[3]											,oFont10)
oPrint:Say (_nLin+1573	,0106,aDatSacador[6]+"    "+aDatSacador[4]+" - "+aDatSacador[5],oFont10) // CEP+Cidade+Estado

oPrint:Say (_nLin+1613	,0106,"Sacado/Avalista"		,oFont8)                                                           

oPrint:Say (_nLin+1700	,1030,"Código de Barra"			,oFont8)
oPrint:Line(_nLin+1730	,0100,_nLin+1730	,1900)//Ultima linha
oPrint:Line(_nLin+640	,1900,_nLin+1730	,1900)//Ultima Coluna
oPrint:Line(_nLin+640	,1445,_nLin+1480	,1445)//Penultima Coluna

oPrint:Say (_nLin+1733,1000,"Autenticação Mecânica"		,oFont8)
oPrint:Say (_nLin+1733,1400,"Ficha de Compensação"		,oFont10)

MSBAR("INT25"  ,22.8,3.0,CB_RN_NN[1],oPrint,.F.,,,,1.4,,,,.F.)
...
Parametros  : 
Retorno     : 
Objetivos   : Retorna o Nosso número.
Autor       : Jean Victor Rocha
Data/Hora   : 10/05/2013
TDN         : 
*/
*---------------------*
User Function MR755NN()  
*---------------------*
Local cCart:="02"
Local cNroDoc:=""
Local cCartNN:=""
