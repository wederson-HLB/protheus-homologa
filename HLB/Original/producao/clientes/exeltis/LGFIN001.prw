...
*-----------------------------------------*
USER FUNCTION LGFIN001(lSelAuto,lPreview,lItau,lSantander)   
*-----------------------------------------*
...
if !lSelAuto
	...
	Aadd(aPergs,{"Ate Emissao","","","mv_che","D",8,0,0,"G","","MV_PAR14","","","","31/12/20","","","","","","","","","","","","","","","","","","","","","","","","",""})
	Aadd(aPergs,{"De Vencimento","","","mv_chf","D",8,0,0,"G","","MV_PAR15","","","","01/01/80","","","","","","","","","","","","","","","","","","","","","","","","",""})
	Aadd(aPergs,{"Ate Vencimento","","","mv_chg","D",8,0,0,"G","","MV_PAR16","","","","31/12/20","","","","","","","","","","","","","","","","","","","","","","","","",""})
	
	AjustaSx1("BLTBARS73",aPergs)
...
Local cMailTo		:= GetNewPar( 'MV_P_00064' , '' )
...
//If Alltrim(SM0->M0_CODIGO)=="LG"
	cPortGrv    := "033" 
	cAgeGrv     := PadR( "3979" , Len( SA6->A6_AGENCIA ) )
	cContaGrv   := PadR( "13003502" , Len( SA6->A6_NUMCON ) )
	cCodEmp		:= "6724701"
//EndIf                                    
...
Do While SQL->(!EOF())
	...
	If Empty(SE1->E1_PORTADO)
		DbSelectArea("SA6")
		DbSetOrder(1)
		IF DbSeek(xFilial("SA6") + cPortGrv + cAgeGrv + Left( cContaGrv , 7 ) )
			RecLock("SE1",.F.)
...
WHILE SQL->(!EOF())
	...
	If SE1->( E1_PORTADO + E1_AGEDEP + Left( E1_CONTA, 7 ) ) <> ( cPortGrv + cAgeGrv  + Left( cContaGrv , 7 ) )
		MsgStop( 'Boleto ' + SE1->E1_NUM + ' nao sera gerado. Codigo portador do boleto pertence a outro banco.' )		
		SQL->(DBSKIP())
		LOOP	
	EndIf
	...
	dbSelectArea("SEE")
	dbSetOrder(1)
	dbSeek(xFilial("SEE")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA)+cSubCta,.T.)
	...
	aDadosTit := {	AllTrim(E1_NUM)+AllTrim(E1_PARCELA),;			// [1] Numero do Titulo
					...
					If( FieldPos( 'E1_P_MSGBO' ) > 0 , E1_P_MSGBO , "" ) }                              		//[12] Msg para nota
...
//AOA - 07/06/2018 - Ajuste no nosso número que estava calculando o DAC duas vezes
//cS    := cNroDoc
//nDvnn := modulo11(cS,.F.) // digito verifacador
cNNSD := SubStr(Alltrim(cNroDoc),1,len(Alltrim(cNroDoc))-1) //Nosso Numero sem digito
cNNCD := PADL(cNroDoc,13,'0')
cNN   := cCart + SubStr(Alltrim(cNroDoc),1,len(Alltrim(cNroDoc))-1) + '-' + SubStr(Alltrim(cNroDoc),-1,1)
//----------------------------------
