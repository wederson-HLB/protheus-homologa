...
*--------------------------------------------------------------------------------------*
	...
	Private limite       := 80
	Private tamanho      := "P"
	Private nomeprog     := "LGFAT003" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo        := 15
	...
	Private CONTFL       := 01
	Private m_pag        := 01
	Private wnrel        := "LGFAT003" // Coloque aqui o nome do arquivo usado para impressao em disco
	Private cString      := ""
	...
	If SC5->(DBSeek(xFilial("SC5")+cPedido))
		...
		if !(Empty(SC5->(C5_NOTA+C5_SERIE))) .and. !(Empty(nVolume))
			...
			If Empty(SF2->F2_VOLUME1)
				...
				RecLock("SF2",.F.)
				//RSB - 10/11/2017 - Quando o tipo do campo é numero da erro. #18449
