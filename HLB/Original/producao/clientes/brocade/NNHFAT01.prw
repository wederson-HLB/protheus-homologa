...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Nota Fiscal Brocade - Entrada e Saída 
...
//   If Pergunte("KJNFAT    ",.T.)  
   ...
   Endif
Else
    MsgInfo("Especifico BROCADE","A T E N C A O")  
Endif   
...
tamanho  :='G'
limite   :=220
titulo   :="Nota Fiscal - Entrada / Saida - BROCADE"
cDesc1   :=' '
cDesc2   :=''
...
If _cTpMov == 1
   fGerSf1()
   RptStatus({|| fImpSF1()},"Nota de Entrada - BROCADE")
Else
   fGerSf2()
   RptStatus({|| fImpSF2()},"Nota de Saida - BROCADE")
Endif   
...
Do While.Not.Eof()
   ...
   While SQL->F1_DOC + SQL->F1_SERIE == cCompara           
      ...
      @ nLin,078 pSay SQL->D1_QUANT  Picture "@E@Z 999,999.99"       	//Quantidade
      @ nLin,086 pSay SQL->D1_VUNIT  Picture "@E@Z 99,999,999.99"    	//Vlr Unitário    
      @ nLin,102 pSay SQL->D1_TOTAL  Picture "@E@Z 999,999,999.99"   	//Vlr Total
      @ nLin,120 pSay SQL->D1_PICM   Picture "99"               			//%ICMS    
      @ nLin,124 pSay SQL->D1_IPI    Picture "99"               			//%IPI
      @ nLin,128 pSay SQL->D1_VALIPI Picture "@E 99,999.99"          	//Vlr IPI
   ...
   // CALCULO DO IMPOSTO
   @ 043,009 pSay nBASEICM Picture "@E 999,999,999.99" //Base ICMS
   @ 043,038 pSay nVALICM  Picture "@E 999,999,999.99" //Vlr  ICMS
   @ 043,058 PSAY nBrIcms Picture "@E 999,999,999.99"  // Base ICMS Ret.
   @ 043,085 PSAY nIcmsRet Picture "@E 999,999,999.99"  // Valor  ICMS Ret.
   @ 043,120 pSay nVALMERC Picture "@E 999,999,999.99" //Vlr Produtos
   @ 045,009 pSay nFRETE   Picture "@E 999,999,999.99" //Vlr Frete
   @ 045,038 pSay nSEGURO  Picture "@E 999,999,999.99" //Vlr Seguro
   @ 045,058 pSay nDESPESA Picture "@E 999,999,999.99" //Vlr Despesa
   @ 045,085 pSay nVALIPI  Picture "@E 999,999,999.99" //Vlr IPI
   @ 045,120 pSay nVALBRUT Picture "@E 999,999,999.99" //Vlr Nota       
   

   If! Empty(cMensTes)
	  @ 054,006 pSay SUBSTR(cMensTes,1,50)
      @ 055,006 pSay SUBSTR(cMensTes,51,50)
   EndIf
...
Static Function fCabSf1()
   ...
   If! AllTrim(cTipo) $ "B/D"
      ...
      If Len(AllTrim(SA2->A2_CGC)) == 14 
         @ 013,085 pSay SA2->A2_CGC Picture "@R 99.999.999/9999-99"
      ElseIf Len(AllTrim(SA2->A2_CGC)) == 11   
         @ 013,085 pSay SA2->A2_CGC Picture "@R 999.999.999-99"
      Else   
         @ 013,085 pSay SA2->A2_CGC
      Endif   
      @ 013,122 pSay Dtoc(cEMISSAO)
      @ 015,005 pSay SA2->A2_END
      @ 015,075 pSay SA2->A2_BAIRRO
      @ 015,100 pSay SA2->A2_CEP   Picture "@R 99.999-999"
      @ 017,005 pSay SA2->A2_MUN
      @ 017,055 pSay SA2->A2_TEL   
      @ 017,078 pSay SA2->A2_EST
      If AllTrim(SA2->A2_INSCR) == "ISENTO" 
         @ 017,085 pSay "ISENTO" 
      Else      
	     @ 017,085 pSay SA2->A2_INSCR Picture "@R 999.999.999.999"
      Endif       
   Else   
      @ 013,005 pSay SA1->A1_NOME
      If Len(AllTrim(SA1->A1_CGC)) == 14 
         @ 013,085 pSay SA1->A1_CGC Picture "@R 99.999.999/9999-99"
      ElseIf Len(AllTrim(SA1->A1_CGC)) == 11   
         @ 013,085 pSay SA1->A1_CGC Picture "@R 999.999.999-99"
      Else   
         @ 013,085 pSay SA1->A1_CGC
      Endif   
      @ 013,122 pSay Dtoc(cEMISSAO)
      @ 015,005 pSay SA1->A1_END
      @ 015,075 pSay SA1->A1_BAIRRO
      @ 015,100 pSay SA1->A1_CEP   Picture "@R 99.999-999"
      @ 017,005 pSay SA1->A1_MUN
      @ 017,055 pSay SA1->A1_TEL   
      @ 017,078 pSay SA1->A1_EST
      If AllTrim(SA1->A1_INSCR) == "ISENTO" 
         @ 017,085 pSay "ISENTO" 
      Else      
         @ 017,085 pSay SA1->A1_INSCR Picture "@R 999.999.999.999"  
      Endif       
   Endif      
...
//Verifica os CFOPs correspondentes (distintos)
   ...
   IF Alltrim(cTipo) $ "N/D/C/B"  
      While nMerc <= Len(aMerc)
         ...
         @ nLin,076 pSay aMerc[nMerc][05]                                 //Unidade
         @ nLin,078 pSay aMerc[nMerc][06] Picture "@E@Z 999,999.99"       //Quantidade
         @ nLin,086 pSay aMerc[nMerc][07]+(aMerc[nMerc][16]/aMerc[nMerc][06]) Picture "@E@Z 9,999,999.99"     //Vlr Unitário    
         @ nLin,102 pSay aMerc[nMerc][08]+aMerc[nMerc][16] Picture "@E@Z 9,999,999.99"     //Vlr Total
         @ nLin,120 pSay aMerc[nMerc][09] Picture "99"               	  //% ICMS    
         @ nLin,124 pSay aMerc[nMerc][10] Picture "99"               	  //% IPI
         @ nLin,128 pSay aMerc[nMerc][11] Picture "@E 99,999.99"          //Vlr IPI                   
      ...
      @ 046,065 pSay nDESPESA Picture "@E 999,999,999.99" //Vlr Despesa
      @ 046,090 pSay nVALIPI  Picture "@E 999,999,999.99" //Vlr IPI
      @ 046,123 pSay nVALBRUT Picture "@E 999,999,999.99" //Vlr Nota
      ...
      ElseIf SC5->C5_TPFRETE == "F"   
         @ 048,65 PSay "2"   
      Endif   
      @ 048,120 pSay SA4->A4_CGC    Picture "@R 99.999.999/9999-99"
      @ 050,006 pSay SA4->A4_END
      @ 050,080 pSay SA4->A4_MUN
      @ 050,107 pSay SA4->A4_EST
      If AllTrim(SA4->A4_INSEST) == "ISENTO" 
         @ 050,090 pSay "ISENTO" 
      Else      
         @ 050,090 pSay SA4->A4_INSEST Picture "@R 999.999.999.999"
      Endif       
      ...
      @ 054,006 pSay SUBSTR(SC5->C5_MENNOTA,1,50)         
      @ 055,006 pSay SUBSTR(SC5->C5_MENNOTA,51,50)         
   ...
   Elseif Alltrim(cTipo) = "P"                                                                
      ...
      End 
         ...
         @ 054,006  pSay SUBSTR(SC5->C5_MENNOTA,01,50)          
         @ 055,006  pSay SUBSTR(SC5->C5_MENNOTA,51,50)          
   ...
   Else    
      ...
      EndIf   
         @ 058,006  pSay SUBSTR(SC5->C5_MENNOTA,01,50)          
         @ 059,006  pSay SUBSTR(SC5->C5_MENNOTA,51,50)      
   EndIf
