...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Gerar arquivo baseados no dados do RH
Autor     	: 
Data     	: 
Obs         :
TDN         : 
      ...
      SRC->(DbSeek(xFilial("SRC")+SRA->RA_MAT))  	  
	   ...
	   While SRC->RC_MAT == SRA->RA_MAT                                           	
	       ...
	       If (SRC->RC_DATA >= dPeriodoDe) .And. (SRC->RC_DATA <= dPeriodoAte) .And. (SRC->RC_PD $ cVerba)
			   ...
			   cLin := Stuff(cLin,033,15, StrTran(Strzero(SRC->RC_HORAS, 09,2), ".", "") + "0000")
   			   cLin := Stuff(cLin,048,04, "0025") 
   			   cLin := Stuff(cLin,052,11, Substr(SRA->RA_CIC, 1, 11))  +cEOL 
		       nCont := nCont + 1
		       cControle := "S"
		       nSoma := nSoma + SRC->RC_VALOR                                             
	       End If     
