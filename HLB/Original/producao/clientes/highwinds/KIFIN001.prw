...
±±³ Programa  ³ KIFIN001().PRW ³ Autor ³               ³ Data ³ 21/09/2011 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Descricao ³ Impressao de Boleto Bancario do Banco HSBC      com Codigo ³±±
...
Parametros  : _nOpc
Retorno     : _cRet
Objetivos   : Impressao de Boleto Bancario do Banco HSBC  com Codigode Barras, Linha Digitavel e Nosso Numero.
Autor       : 
TDN         : 
Revisão     : Anderson Arrais 
Data/Hora   : 29/01/2016
Módulo      : Financeiro.
*/    

*--------------------------*
  USER FUNCTION KIFIN001()     
*--------------------------*
...
Tamanho  := "M"
titulo   := "Impressao de Boleto HSBC
cDesc1   := "Este programa destina-se a impressao do Boleto HSBC."
cDesc2   := ""
cDesc3   := ""
cString  := "SE1"
wnrel    := "KIFIN001()"
...
LOCAL oPrint
LOCAL n         := 0
LOCAL aDadosEmp := {Subs(SM0->M0_NOMECOM,1,40),;													  	//[1]Nome da Empresa
...
LOCAL aDadosBanco
LOCAL aDatSacado
LOCAL aBolText  := {"Após vencimento cobrar multa de 2% e mora de 0,03% ao dia."}
LOCAL i         := 1
LOCAL CB_RN_NN  := {}
LOCAL nRec      := 0
LOCAL _nVlrAbat := 0
...
Local _nRecSE1  := 0
LOCAL nRange   	:= "22212"
Private aBitmap   := {	"LOGO399.BMP",;	// Banner Publicitario HSBC
...
While xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "KI"     // SHOPZILLA
			cBanco    := "399"
			cAgencia  := "1879 "
			cConta    := "0017876   "
			cSubConta := "001"	  
	...
	" ",;//SUBSTR(SA6->A6_NUMCON,Len(AllTrim(SA6->A6_NUMCON),0),;	    // [5]Digito da conta corrente
	...
	IF EMPTY(SE1->E1_NUMBCO)
		...
		If dbSeek(xFilial("SEE")+Padr(AllTrim(aDadosBanco[1]),TamSX3("EE_CODIGO")[1])+Padr(AllTrim(aDadosBanco[3]),TamSX3("EE_AGENCIA")[1])+Padr(AllTrim(aDadosBanco[4]),TamSX3("EE_CONTA")[1])+cSubConta)    						
			RecLock("SEE",.F.)
			cNroDoc			:= SubStr(AllTrim(SEE->EE_FAXATU),8,5)
	    ...
	    snn   := nRange+cNroDoc             // Nosso Numero
		dvnn  := Alltrim(Str(mod117(snn)))  //Digito verificador no Nosso Numero  
		cNN   := snn + dvnn
		 
		SE1->E1_NUMBCO 	:= cNN	//cNroDoc   // Nosso número (Ver fórmula para calculo)
		SE1->E1_PORTADO := SA6->A6_COD
		SE1->E1_AGEDEP  := SA6->A6_AGENCIA
		SE1->E1_CONTA   := SA6->A6_NUMCON
		MsUnlock()
	Else
		cNroDoc 	:= SubStr(ALLTRIM(SE1->E1_NUMBCO),6,5)
	EndIf
	DbSelectArea("SE1")
	dbGoTo(_nRecSE1)
...
oFont24t := TFont():New("Times New Roman",9,20,.T.,.F.,5,.T.,5,.T.,.F.)
oBrush  := TBrush():New("",4)
...
oPrint:SayBitmap(080,0350,aBmp[1],110,060)					// [2] Logo do Banco
oPrint:Say (0064,0100,aDadosBanco[2],oFont24t)				// [2] Nome do Banco
...
oPrint:SayBitmap(_nLin+0639,0350,aBmp[1],110,060)			// [2] Logo do Banco
oPrint:Say (_nLin+0624,0100,aDadosBanco[2],oFont24t)     	// [2]Nome do Banco
...
oPrint:Say (_nLin+0760,0100,"PAGAR EM QUALQUER AGÊNCIA BANCARIA ATÉ O VENCIMENTO OU CANAIS ELETRONICOS DO HSBC.",oFont8)
//oPrint:Say (_nLin+0775,0100,"APOS O VENCIMENTO PAGUE SOMENTE AGÊNCIA DO HSBC",oFont8)
...
oPrint:Say (_nLin+1010,0555,"CSB",oFont10)
...
//oPrint:Say (_nLin+1589,1950,nRange+SUBSTR(aDadosTit[6],1,8)+"-"+SUBSTR(aDadosTit[6],9,1),oFont10)
...
oPrint:SayBitmap(_nLin+1928,0350,aBmp[1],110,060)		// [2] Logo do Banco
oPrint:Say (_nLin+1914,0100,aDadosBanco[2],oFont24t) 	// [2] Nome do Banco
...
oPrint:Say (_nLin+2050,0100,"PAGAR EM QUALQUER AGÊNCIA BANCARIA ATÉ O VENCIMENTO OU CANAIS ELETRONICOS DO HSBC.",oFont8)
//oPrint:Say (_nLin+2065,0100,"APOS O VENCIMENTO PAGUE SOMENTE EM AGÊNCIA DO HSBC",oFont8)
...
oPrint:Say (_nLin+2300,0555,"CSB",oFont10)
...
//oPrint:Say (_nLin+2879,1950,nRange+SUBSTR(aDadosTit[6],1,8)+"-"+SUBSTR(aDadosTit[6],9,1),oFont10)
...
WHILE L > 0
	...
	IF (B)
		P := P * 2
		IF P >= 10   //JSS
			P:= Val(SubStr(cValToChar(P),1,1))+Val(SubStr(cValToChar(P),2,1)) // P := P - 2//JSS
		ENDIF
	ENDIF
	D := D + P
	L := L - 1
...
//campo 1 
campo1  := cBanco + cMoeda + nRange 
dvC1    := Alltrim(Str(modulo10(campo1)))                                  
cCampo1 := campo1 + dvC1

// Montagem das DACs de Representacao Numerica do Codigo de Barras
//campo 2 
campo2  := SubStr(cNN,6,6) + cAgencia  
dvC2    := Alltrim(Str(modulo10(campo2)))
cCampo2 := campo2 + dvC2   

// Montagem das DACs de Representacao Numerica do Codigo de Barras
//campo 3 
campo3  := SubStr(cConta,1,5)+SubStr(cConta ,6,7)+"001" 
dvC3    := Alltrim(Str(modulo10(campo3)))
cCampo3 := campo3 + dvC3 //+ substr(cAgencia,4,1)

// Montagem das DACs do Codigo de Barras
//campo 4
campo4  := cBanco + cMoeda + cFator + cValToChar(StrZero((nValor*100),10)) + nRange + cNroDoc + dvnn + cAgencia + cConta + "001"
cDacCB  := Alltrim(Str(Mod119(campo4)))
cCampo4 := cDacCB

// Montagem 
//campo 5
cCampo5  := cFator + StrZero((nValor*100),10)
//////////////////////////////////////////////////////////////////////////////MONTAGEM DO CODIGO DE BARRAS

cCB      := cBanco + cMoeda + cDacCB + cFator + StrZero((nValor*100),10) + nRange + cNroDoc + dvnn + cAgencia + cConta + "001" // codigo de barras
