...
If Empty(cNumOrc)  
	MsgStop("Nenhum Orçamento encontrado para impressão","HLB BRASIL")   
	lRet:=.F.
	Return .F.
EndIf 
...
While SCK->(!EOF()) .And. SCK->CK_NUM == Alltrim(cNumOrc)  
	...
	If nLinha>2730
		...
		If Empty(SCJ->CJ_P_VEND1) 
			oPrint:Say(3240,400,"NÃO PREENCHIDO NO ORÇAMENTO",oFont5)
		Else     
			DbSelectArea("SA3")
			SA3->(DbSetOrder(1))
...
If Empty(SCJ->CJ_P_VEND1) 
	oPrint:Say(3240,1000,"VENDEDOR NÃO PREENCHIDO NO ORÇAMENTO",oFont5)
Else 
	DbSelectArea("SA3")
	SA3->(DbSetOrder(1))
	If SA3->(DbSeek(xFilial("SA3")+SCJ->CJ_P_VEND1))
...
If Empty(SCJ->CJ_P_VEND1)
	oPrint:Say(1350,160,"VENDEDOR NÃO PREENCHIDO NO ORÇAMENTO",oFont5) 
Else
