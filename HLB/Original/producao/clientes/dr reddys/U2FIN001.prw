...
*-----------------------------------------*
USER FUNCTION U2FIN001(lSelAuto,lPreview)
*-----------------------------------------*
Local aArea			:= GetArea()
...
Local cDirBol	:= "\FTP\" + cEmpAnt + "\U2FIN001\"    
Local cNroDoc	:= ""
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "U2"//Dr. Reddys
			cBanco    := "341"
			cAgencia  := "8170 "
			cConta    := "017456  "
			cSubConta := "001"
