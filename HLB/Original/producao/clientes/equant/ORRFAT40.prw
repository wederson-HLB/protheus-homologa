...
±±³ Funcao   ³ ORRFAT40  ³ Autor ³ Andre Minelli        ³ Data ³01/06/2013³±±
...
Local nLimTot	 := 413 //Limita a quantidade de linhas por paginas sem boleto
...
End If
//RRP - 18/12/2013 - Retirada a coluna C5.C5_MENNOT1, pois não existe em nossa base. 
...
cQuery += " AND F2.F2_CLIENTE <= '" + MV_PAR04 + "' AND F2_LOJA >= '" + MV_PAR05 + "' AND F2_LOJA <= '" + MV_PAR06 + "' AND F2.F2_DOC >= '" + mv_par07 + "' AND F2.F2_DOC <= '" + mv_par08 + "' AND F2.F2_SERIE >= '" + mv_par09 + "' AND F2.F2_SERIE <= '" + mv_par10 + "' AND F2.D_E_L_E_T_ = '' AND A1.D_E_L_E_T_ = '' AND F2.F2_ESPECIE = 'RPS'"
...
While SQLSF2->(!EOF())
	...
	//RRP - 07/02/2014 - Ajuste no nome do arquivo anexado.
...
/*/
Static Function KBLTCITI(oNfServ,cNatureza,cFil_,cDoc_,cSerie_,cCli_,cLoja_,cContr_,dEmiss_,cPrefix_)
//cDoc_,cContr_,dEmiss_,cCli_,cLoja_
Private lEnd     := .F.

Processa({|lEnd|MontaRel(@oNfServ,cNatureza,cFil_,cDoc_,cSerie_,cCli_,cLoja_,cContr_,dEmiss_,cPrefix_)})

Return Nil

Static Function MontaRel(oNfServ,cNatureza,cFil_,cDoc_,cSerie_,cCli_,cLoja_,cContr_,dEmiss_,cPrefix_)
...
//RRP - 16/10/2015 - Ajuste para procurar o tú‘ulo NF. Chamado 030158.
...
cQueryBol += " WHERE E1_NUM = '" + cDoc_ + "' AND E1_PREFIXO = '"+cPrefix_+"'"
cQueryBol += " AND E1_CLIENTE = '" + cCli_ + "' AND E1_LOJA = '" + cLoja_ + "' AND E1_SALDO > 0 AND E1_VENCREA >= '" + DTOS(mv_par16) + "' AND E1_VENCREA <= '" + DTOS(mv_par17) + "'"
...
While SQLSE1->(!EOF()) 
	...
	aDadosBanco  := {"745",; 																     // [1]Numero do Banco
				     Alltrim(SA6->A6_NREDUZ),;  												 // [2]Nome do Banco
	                 "0001",;																	 // [3]Agência
	...
	//Inicio Alteração...
		//ANTIGO cNroDoc	:= Strzero(Val(Alltrim(SQLSE1->E1_NUM)),9)+StrZERO(_nParcela,2)               
		//ANTIGO cDigNNum	:= KCALCDp(ALLTRIM(cNroDoc),"1")     
		//ANTIGO cNroDoc	:= cNroDoc+""+cDigNNum   
	...
	Impress(oNfServ,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
	nX := nX + 1
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Impress(oNfServ,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
...
If nPagina == 1 .And. nLinhas < 30  
	If lSaltaBol
		oNfServ:EndPage()
		oNfServ:StartPage()
		KIMPCAB(@oNfServ,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
		nLin += 40
...
Static Function KImpCab(oNfServ,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
...
oNfServ:Say(nLin,aCol[4]+75,"IMPRESSÃO DO RECIBO PROVISORIO DE SERVIÇOS - RPS",,RGB(255,102,000))
...
oNfServ:Say(nLin+10,aCol[6]-30,"ISS")
...
Static Function SaltaPag(oNfServ,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
...
Static Function ImpCorpo(oNfServ,cAux,nSalta,nSaltaLnIt,nLimBol,nLimTot,nTamLine,nSpRow,nLin2,cFil_,cDoc_,cSerie_,cCli_,cLoja_,cContr_,cEspec_,cMenNot_,dEmiss_,nValBru_,nBaseIcm_,nValIcm_,lImpBol,cPrefix_)
	...
	oNfServ:=FWMSPrinter():New("nfserv_" + alltrim(cCli_)+alltrim(cContr_),6,.F.,,.T.,,,,,,,.F.)
	...
	*/
	...
	cQuery2 := "SELECT D2.D2_COD,B1.B1_GRUPO,B1.B1_DESC,B1.B1_ALIQISS,D2.D2_TOTAL,D2.D2_BASEISS,D2.D2_VALISS FROM " + RetSqlName("SD2") + " D2 LEFT JOIN " + RetSqlName("SF2") + " F2 ON D2.D2_FILIAL = F2.F2_FILIAL AND D2.D2_DOC = F2.F2_DOC AND D2.D2_SERIE = F2.F2_SERIE"
	...
	KIMPFORM(@oNfServ)
 	...
 	While SQLSD2->(!EOF())
		...
		If nLinhaAtu*nSaltaLnIt > nLimTot
			SaltaPag(@oNfServ,cNatureza,cDoc_,cContr_,dEmiss_,cCli_,cLoja_)
		End If
		...
		oNfServ:Say(nLin+20,aCol[6]-140 ,Transform(SQLSD2->D2_BASEISS,"@E 999,999,999.99"))
		oNfServ:Say(nLin+20,aCol[6]-110 ,Transform(SQLSD2->B1_ALIQISS,"@E 999,999,999.99"))
		oNfServ:Say(nLin+20,aCol[6]-50  ,Transform(SQLSD2->D2_VALISS,"@E 999,999,999.99"))

		nLin    += nSaltaLnIt

		nTotal     += SQLSD2->D2_TOTAL
		nTotalBase += SQLSD2->D2_BASEISS
		nTotalIss  += SQLSD2->D2_VALISS
	...
	If Alltrim(cSerie_) # "R"
		...
		oNfServ:Say(nLin,aCol[6]-50 ,Transform(nTotalIss ,"@E 999,999,999.99"))
	...
	oNfServ:Say(nLin+11,aCol[1]+5,"TRIBUTADO ISS")
	oNfServ:SetFontEX(7,cFonte,.F.,.T.,.F.)
	oNfServ:Say(nLin+11,aCol[3]+90,Transform(nTotalBase,"@E 999,999,999.99"))
		
	oNfServ:SetFontEX(7,cFonte,.F.,.T.,.F.)
	oNfServ:Fillrect( {nLin-10,aCol[6]-142,nLin-10 + (nTamLine*6), aCol[7] }, oBrush0, "-2")
	oNfServ:Say(nLin,aCol[5]-40  ,"BASE CÁLCULO" + Space(16) + "ISS",,CLR_WHITE)
	oNfServ:Say(nLin+10,aCol[5]-40 , Transform(nTotalBase,"@E 999,999,999.99") + Space(10) + Transform(nTotalIss,"@E 999,999,999.99"))
	
	nLin += 35
	
	oNfServ:SetFontEX(6,cFonte,.F.,.T.,.F.)
	oNfServ:Box(nLin+nSpRow,aCol[1],nLin + 35, aCol[7])
	oNfServ:Say(nLin+10,aCol[1]+5  ,"DADOS ADICIONAIS" )

	oNfServ:Say(nLin+18,aCol[1]+30  ,IIF(cMenNot_#"NIL",cMenNot_,""))  
	oNfServ:Say(nLin+25,aCol[1]+30  ,"Por favor, observe que em caso de inadimplemento poderEhaver a cobrança de multas e juros ou outras medidas cabú“eis.") //RRP - 22/10/2014 - Inclusão da Msg. Chamado 022118
	oNfServ:Say(nLin+32,aCol[1]+30  ,"Esta RPS serEconvertida em NFS-e, conforme Art. 10 do DECRETO Nº 47.350, DE 6 DE JUNHO DE 2006.") //RRP - 14/08/2014 - Inclusão da Msg. Chamado 020794

	//oNfServ:Say(nLin+31,aCol[1]+30  ,IIF(SQLSF2->C5_MENNOT1#"NIL",SQLSF2->C5_MENNOT1,"")) //RRP - 18/12/2013 - Retirada a coluna C5.C5_MENNOT1, pois não existe em nossa base. 
	...
	if lImpBol
		If  SuperGetMv( "MV_OGERBOL" , .F. , .T. , Nil )
			KBLTCITI(@oNfServ,cNatureza,cFil_,cDoc_,cSerie_,cCli_,cLoja_,cContr_,dEmiss_,cPrefix_)
			SQLSE1->(DbCloseArea())
		End If
	endif	
	oNfServ:EndPage()  
