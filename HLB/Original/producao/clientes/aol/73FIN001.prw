...
*-----------------------------------------*
USER FUNCTION 73FIN001(lSelAuto,lPreview)   
*-----------------------------------------*
...
Local cFile := ""
Local cDirBol 	  := "\FTP\" + cEmpAnt + "\73FIN001\" 
...
If !LisDir( cDirBol )
	MakeDir( "\FTP" )
	MakeDir( "\FTP\" + cEmpAnt )	
	MakeDir( "\FTP\" + cEmpAnt + "\73FIN001\" )		
EndIf	

If Alltrim(SM0->M0_CODIGO)=="73"
	cPortGrv    := "033"
	cAgeGrv     := "3853 "
	cContaGrv   := "130058820"
	cCodEmp		:= "7469594"
...
Else
	...
	If CpyT2S( cLocal + cFileName + ".pdf" , cDirBol ,.T. )
		...
		/*
		cSubject := 'AOL - Boleto ' + SF2->F2_DUPL
		...
		cEmail += '<p align="left">Segue anexo boleto gerado pelo ERP Protheus Totvs</p>'
		cEmail += '<br />'
		cEmail += '<br />'
		cEmail += '<br />'
		...
		EnviaEma(cEmail,cSubject,cMailTo,,cFile)
		*/
	Else
...
/*
Função..........: PosTitPai
Objetivo........: Posicionar Titulo Pai Ref. Transferencia de titulo
Autor...........: Leandro Brito
Con
*/                             
*------------------------------------*
User Function PosTitTr
*------------------------------------*
...
If !Empty( SE1->E1_NUMSOL )
	If SE6->(  DbSeek( xFilial() + SE1->E1_NUMSOL ) )
		SE1->( DbSetOrder( 2 ) , DbSeek( xFilial() + SE6->( E6_CLIENTE + E6_LOJA + E6_PREFIXO + E6_NUM ) ) )
	EndIf
EndIf	
