...
*-----------------------------------------*
USER FUNCTION 73FIN002(lSelAuto,lPreview)
*-----------------------------------------*
Local aArea			:= GetArea()
...
Local cDirBol	:= "\FTP\" + cEmpAnt + "\73FIN002\"    
Local cNroDoc	:= ""
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "73"     //AOL
			cBanco    := "341"
			cAgencia  := "0445 "
			cConta    := "477946  "
			cSubConta := "001"
