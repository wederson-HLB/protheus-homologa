...
//JSS				
	...
	aBolText  := {"Após o vencimento cobrar multa de 2,00% R$ ",;
					"Mora de 2,40% ao mês, Diaria de R$ ",;
	...
	//Monta o diretório de gravação dos anexos.	  <------------------------------------------- VOLTA PARA JPG   	
	If !ExistDir(cPasta)
		If !MontaDir(cPasta)
	...
	//Função que salva os arquivos como .JPG no servidor
...
If !Empty(cPastaTo)
	...
	CpyS2T(cFile,cPastaTo,.F.)		//Copia arquivos do diretorio do servidor para diretorio do usuário  
	Erase(cFile)  					//Deleta os arquivos do diretorio dos servidor de pois de copiar para diretorio usuário.
Else
...
oPrint:Say (_nLin+0750,0100,"Pagável em qualquer banco até o vencimento.",oFont10)
...
oPrint:Say (_nLin+2040,0100,"Pagável em qualquer banco até o vencimento.",oFont10)
...
cS    :=  cConv + substr(AllTrim(cNroDoc),2,10) // Construção do Nosso Nume //cS    :=  cCart + substr(alltrim(cNroDoc),1,len(alltrim(cNroDoc))) 
...
// 	CAMPO 1:
//	AAA	  = Código do Banco na Câmara de Compensação “001”
//	B     = Codigo da moeda, sempre 9
//	CCCCC = Posição 20 a 24 do código de barras
...
// 	CAMPO 2:
//	DDDDDDDDDD = Posição 25 a 34 do Codigo de Barras
...
// 	CAMPO 3:
//	FFFFFFFFFF = Posição 35 a 44 Codigo de Barras
