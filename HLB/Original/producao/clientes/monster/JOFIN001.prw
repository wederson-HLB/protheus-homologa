...
TDN         : 
Empresa		: Monster Energy
Módulo      : Financeiro.
*/ 

*-----------------------------------------*
 USER FUNCTION JOFIN001(lSelAuto,lPreview)  
*-----------------------------------------*
...
Local cDirBol	:= "\FTP\" + cEmpAnt + "\JOFIN001\"
Local cNroDoc	:= ""    
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "JO"     //MONSTER ENERGY
			cBanco    := "237"
			cAgencia  := "3381 "
			cConta    := "29173   "
			cSubConta := "003"
	...
	cSubConta   := If(Empty(cSubConta),"003",cSubConta)
...
oPrint:Say (_nLin+_nLin2+0750,0100,"Pagável preferencialmente na Rede Bradesco ou Bradesco Expresso",oFont10)
...
oPrint:Say (_nLin+_nLin2+0910,1910,"Carteira / Nosso Número",oFont8) 
...
oPrint:Say (_nLin+_nLin2+2040,0100,"Pagável preferencialmente na Rede Bradesco ou Bradesco Expresso",oFont10)
...
oPrint:Say (_nLin+_nLin2+2200,1910,"Carteira / Nosso Número",oFont8)
