...
//cQry+=" AND E1_TIPO IN ( 'NF','FT' ) - AOA - 06/04/2016 - Alterado para tratar todos tipos inciados por FT além das NF já tratadas
cQry+=" AND (E1_TIPO LIKE 'FT%' OR E1_TIPO = 'NF' )
...
LOCAL aBolText  := {"",;
					"Mora Diaria de R$ ",;
					"Enviar a cartório após 03 (tres) dias do vencimento"}
...
If Alltrim(SM0->M0_CODIGO) $ "MN"//ZB 
	cPortGrv    := "237"
	cAgeGrv     := "0105 "
	cContaGrv   := "230685-9"
	//cPortGrv    := "237"
...
WHILE SQL->(!EOF())
	...
	dbSelectArea("SEE")
	SEE->(dbSetOrder(1))
	If !(SEE->(dbSeek(xFilial("SEE")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA),.T.)))//AOA - 29/02/2016 - Quando não seekar tabela sair do while.
		SQL->(dbSkip())
		LOOP
	EndIf		
	...
	EndIf
	                                                                                                                                         //- Antes não tinha +E1_ACRESC, foi adicionado para tratar o acrescimo como juros
	                                                                                                                                         //- Antes não tinha -E1_DECRESC, foi adicionado para tratar o decrescimo
	CB_RN_NN := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",SUBSTR(aDadosBanco[3],1,4),aDadosBanco[4],aDadosBanco[5],cNroDoc,(E1_SALDO-_nVlrAbat)+E1_ACRESC-E1_DECRESC,E1_VENCREA)
	...
	aDadosTit := {	AllTrim(E1_NUM)+AllTrim(E1_PARCELA),;	// [1] Numero do Titulo
					...
					(E1_SALDO - _nVlrAbat)+E1_ACRESC-E1_DECRESC,;				// [5] Valor do Titulo    - Antes não tinha +E1_ACRESC, foi adicionado para tratar o acrescimo como juros		-- Antes não tinha -E1_DECRESC, foi adicionado para tratar o decrescimo
