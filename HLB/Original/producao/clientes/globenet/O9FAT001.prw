...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE OemToAnsi("Emissão Nota Fiscal Modelo 21 - GLOBENET") FROM 0,0 TO 300,380 OF oMainWnd PIXEL 
...
Begin Sequence
   ...
   oPrint:= TMSPrinter():New( "Impressão de Nota Fiscal da Globenet Telecom" )  
...
//Mensagem padrão para nota
Local cMenFixo01  := "Contribuição p/ FUST e FUNTTEL 1,5% do valor dos serviços não repassados ao cliente conf. Lei nº 9998/00 e 10052/00."
...
Begin Sequence 
   ...
   oPrint:Say(200,1610,"MODELO 21 - SÉRIE 1 ÚNICA",oFont07,,CLR_BLACK)
...
nLinha := 1438  
	...
	While SQL->F2_DOC <> '' .And. SQL->(!EOF()) 
		...
		If cDoc+cSer <> SQL->F2_DOC+SQL->F2_SERIE
			...
			cSer:= SQL->F2_SERIE
			cPedido := SQL->D2_PEDIDO
			cMenFixo01:="Contribuição p/ FUST e FUNTTEL 1,5 do valor dos serviços não repassados ao cliente conf. Lei nº 9998/00 e 10052/00."
			//RRP - 16/01/2015 - Tratamento referente lei transparência. Chamado 023741.
			If SQL->A1_TIPO == "F"
				SF2->(DbSetOrder(1))
			 	If SF2->(DbSeek(xFilial("SF2")+SQL->F2_DOC+SQL->F2_SERIE+SQL->F2_CLIENTE+SQL->F2_LOJA))
					If (SF2->(FieldPos("F2_TOTIMP")) > 0)
