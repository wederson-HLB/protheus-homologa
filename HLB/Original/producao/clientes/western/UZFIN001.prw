...
±±³ Programa  ³ UZFIN001().PRW ³ Autor ³               ³ Data ³ 21/09/2011 ³±±
...
TDN         : 
Revisão     : João Silva 
Data/Hora   : 15/07/2013
Módulo      : Financeiro.
*/    

*--------------------------*
  USER FUNCTION UZFIN001()     
*--------------------------*
...
LOCAL aDadosBanco
LOCAL aDatSacado
LOCAL aBolText  := {"Cobrar juros apos 07 dias do vencimento"}
LOCAL i         := 1
LOCAL CB_RN_NN  := {}
LOCAL nRec      := 0
LOCAL _nVlrAbat := 0
...
Local _nRecSE1  := 0
LOCAL nRange   	:= "32925"
Private aBitmap   := {	"LOGO399.BMP",;	// Banner Publicitario Itau
...
While xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "EF/99/UZ"     // WESTERN
			cBanco    := "399"
			cAgencia  := "1879 "
			cConta    := "0032492   "
			cSubConta := "001"	  
