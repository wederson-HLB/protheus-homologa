...
Data/Hora   : 09/11/2016
Módulo      : Financeiro.
Empresa		: JDSU
*/

*-----------------------------------------*
USER FUNCTION 41FIN001(lSelAuto,lPreview)
*-----------------------------------------*
Local aArea			:= GetArea()
...
Local cDirBol	:= "\FTP\" + cEmpAnt + "\41FIN001\"    
Local cNroDoc	:= ""
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "41"     //JDSU
			cBanco    := "341"
			cAgencia  := "0262 "
			cConta    := "061096  "
			cSubConta := "001"
