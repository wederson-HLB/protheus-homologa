...
U_PUTSX1( cPerg, "01", "Arquivo de Entrada ?"	, "Arquivo de Entrada ?", "Arquivo de Entrada ?", "", "C",80,00,00,"G","" , ""		,"","","MV_PAR01")
U_PUTSX1( cPerg, "02", "Arquivo de Config ?"	, "Arquivo de Config ?"	, "Arquivo de Config ?"	, "", "C",50,00,00,"G","" , ""		,"","","MV_PAR02")
U_PUTSX1( cPerg, "03", "Codigo do Banco ?"	, "Codigo do Banco ?"	, "Codigo do Banco ?"	, "", "C",03,00,00,"G","" , "SA6"	,"","","MV_PAR03")
U_PUTSX1( cPerg, "04", "Codigo da Agencia ?"	, "Codigo da Agencia ?"	, "Codigo da Agencia ?"	, "", "C",05,00,00,"G","" , ""		,"","","MV_PAR04")
U_PUTSX1( cPerg, "05", "Codigo da Conta ?"	, "Codigo da Conta ?"	, "Codigo da Conta ?"	, "", "C",10,00,00,"G","" , ""		,"","","MV_PAR05")
U_PUTSX1( cPerg, "06", "Codigo da Sub-Conta?" , "Codigo da Sub-Conta?", "Codigo da Sub-Conta?", "", "C",03,00,00,"G","" , ""		,"","","MV_PAR06")
...
*---------------------------------------------------------------*
Local cNomeArq:="PFFIN003.log"

//Verifica no diretório corrente do servidor se existe o arquivo
if !FILE(cNomeArq)
	nH := fCreate(cNomeArq) 
	If nH == -1                  
	   CONOUT("PFFIN003-->> Falha ao criar arquivo - erro "+str(ferror())) 
	   Return 
	Endif
else
	nH := fopen(cNomeArq,FO_WRITE)
	If nH == -1                  
	   CONOUT("PFFIN003-->> Falha ao abrir arquivo - erro "+str(ferror())) 
	   Return 
	Endif
endif
