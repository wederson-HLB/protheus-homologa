...
±±³ Programa  ³ PFFIN001().PRW ³ Autor ³               ³ Data ³ 21/09/2011 ³±±
...
TDN         : 
Revisão     : João Silva 
Data/Hora   : 16/09/2013
Módulo      : Financeiro.
*/    

*--------------------------*
  USER FUNCTION PFFIN001()     
*--------------------------*
...
LOCAL aDatSacado
LOCAL aBolText  := {"",;
					"Cartório após 30 dias e juros de 1,3%am. ",;
					""}
LOCAL i         := 1
LOCAL CB_RN_NN  := {}
LOCAL nRec      := 0
...
While xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "PF"     // DUCATI
			cBanco    := "341"
			cAgencia  := "2938 "
			cConta    := "205776    "
			cSubConta := "001"	 
...
oPrint:Say (_nLin+0870,0100,aDadosEmp[2]+"                    "+aDadosEmp[4],oFont10) //Endereço + CEP
...
oPrint:Say (_nLin+2160,0100,aDadosEmp[2]+"                    "+aDadosEmp[4],oFont10) //Endereço + CEP
...
Local cCarteira    := alltrim(cCarteira)//"112"
