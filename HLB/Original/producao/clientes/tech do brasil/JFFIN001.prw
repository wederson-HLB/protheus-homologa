...
Data/Hora   : 25/04/2017
TDN         : 
Empresa		: Tech do Brasil
Módulo      : Financeiro.
*/ 

*-----------------------------------------*
 USER FUNCTION JFFIN001(lSelAuto,lPreview)  
*-----------------------------------------*
...
Local cDirBol 		:= "\FTP\" + cEmpAnt + "\JFFIN001\"
Local cNroDoc  		:= ""    
...
While !EOF() .AND. xFilial("SE1") == SE1->E1_FILIAL .AND. SE1->E1_PREFIXO == MV_PAR01 .AND. SE1->E1_NUM <= Mv_Par03
	...
	If Empty(cBanco) .Or. Empty(cAgencia) .Or. Empty(cConta)
		...
		If Alltrim(SM0->M0_CODIGO) $ "JF" //Tech do Brasil
			cBanco    := "237"
			cAgencia  := "2205 "
			cConta    := "257974  "
			cSubConta := "001"
