...
lAllFil  :=IIF(MV_PAR06==1,.T.,.F.) //exibe todas as filiais
...
if dbseek(xfilial("SA6")+mv_par03+mv_par04+mv_par05)
	...
	setprc(000,000)
    //MSM - 19/01/2015 - Chamado: 023257, adicionado para tratar outras filiais
    if lAllFil
		bCondicao := {|| DTOS(E5_DTDIGIT) >= DTOS(MV_PAR01) .AND. DTOS(E5_DTDIGIT) <= DTOS(MV_PAR02)}
		cCondicao := "DTOS(E5_DTDIGIT) >= '"+DTOS(MV_PAR01)+"' .AND. DTOS(E5_DTDIGIT) <= '"+DTOS(MV_PAR02)+"'"
		DbSelectArea("SE5")
		SE5->(DbSetOrder(1))
		SE5->(DbSetFilter(bCondicao,cCondicao))

		dbselectarea("SE5")
		indregua("SE5",cTempo,"DTOS(E5_DTDIGIT)+E5_BANCO+E5_AGENCIA+E5_CONTA+E5_NUMCHEQ",,,"Aguarde ordenação os registros...")
    
		cInicio:=dtos(mv_par01)
...
@nLin,186 psay nSdabert picture "999,999,999,999.99"
nLin:=nLin+2
...
do while !eof() .and. dtos(se5->e5_dtdigit)>=dtos(mv_par01) .and. dtos(se5->e5_dtdigit)<=dtos(mv_par02)
		...
		if se5->e5_valor<>0 .and. se5->e5_recpag="R" //Quando tratar-se de recebimento, o valor impresso será líquido. Já para pagamentos, o valor impresso será o bruto e separadamente, seus descontos e acréscimos
			@nLin,148 psay se5->e5_valor picture "999,999,999,999.99"
		endif
		
		if se5->e5_valor-se5->e5_vlcorre-se5->e5_vljuros-se5->e5_vlmulta-se5->e5_vldesco<>0 .and. se5->e5_recpag="P" .and. se5->e5_tipodoc<>"ES"
			@nLin,167 psay se5->e5_valor-se5->e5_vlcorre-se5->e5_vljuros-se5->e5_vlmulta-se5->e5_vldesco picture "999,999,999,999.99"
		elseif se5->e5_valor-se5->e5_vlcorre-se5->e5_vljuros-se5->e5_vlmulta-se5->e5_vldesco<>0 .and. se5->e5_recpag="P" .and. se5->e5_tipodoc="ES"
			@nLin,167 psay se5->e5_valor picture "999,999,999,999.99"
		endif
		...
		if dtos(se5->e5_dtdigit)<>dtos(dSubtotal)
			...
			@nLin,148 psay nSubrec picture "999,999,999,999.99"
			@nLin,167 psay nSubpag picture "999,999,999,999.99"
			@nLin,186 psay nSdabert picture "999,999,999,999.99"
...
@nLin,023 psay "Total:"
@nLin,148 psay nTotalrec picture "999,999,999,999.99"
@nLin,167 psay nTotalpag picture "999,999,999,999.99"
@nLin,186 psay nSdabert picture "999,999,999,999.99"
...
aadd( _aPerg, { cPerg, "06", "Todas as filiais?",  "", "","mv_ch6", "N", 01, 0, 0, "C", "", "mv_par06", "Sim", "", "", "", "", "Não", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""} )
...
if !dbSeek(cPerg)
	...
	for _nCont1 := 1 To Len(_aPerg)
		if DbSeek(_aPerg[_nCont1][1]+_aPerg[_nCont1][2])
			RecLock("SX1",.F.)
			for _nCont2 := 1 To 38
				fieldPut(_nCont2,_aPerg[_nCont1,_nCont2])
			next
