...
Endif 
//ECR - 10/06/2013 - Tratamento customizado para bloqueio da transmissão.
If ExistBlock("PE04NFESEFAZ")
	lOkGt := ExecBlock("PE04NFESEFAZ",.F.,.F.,{cTipo,cNota,cSerie,cClieFor,cLoja})
	
	If lOkGt
		Return({cNfe,cString,cNotaOri,cSerieOri})
	EndIf
EndIf	
If cTipo == "1"
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				//JVR - 03/08/2018 - Ponto de entrada para manipulação do endereço do destinatario			
				IF ExistBlock("PE07NFESEFAZ")                   
					aParam := {aDest,SF2->F2_DOC,SF2->F2_SERIE}
					aParam := ExecBlock("PE07NFESEFAZ",.F.,.F.,aParam)
					If ( Len(aParam) >= 1 )
						aDest := aParam[1]
					EndIf
				Endif 
			...
			Else
				...
				dbSetOrder(3)
				#IFDEF TOP
					//ECR - 16/11/2011 -  Tratamento customizado para ordenação da query
					cOrd := "%D2_FILIAL,D2_DOC,D2_SERIE,D2_CLIENTE,D2_LOJA,D2_ITEM,D2_COD%"
					
					If ExistBlock("PE02NFESEFAZ")
						cOrd := ExecBlock("PE02NFESEFAZ",.F.,.F.,cOrd)
					EndIf
					lQuery  := .T.
				...
				*/
				...
				Else
					...
					*/
					...
					//JVR - 03/08/2018 - Ponto de entrada para manipulação do endereço do destinatario			
					IF ExistBlock("PE07NFESEFAZ") .and. SF1->F1_TIPO $ "D"
						SD1->(dbSetOrder(1))
						SD1->(dbSeek(xFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA))
						aParam := {aDest,SD1->D1_NFORI,SD1->D1_SERIORI}
						aParam := ExecBlock("PE07NFESEFAZ",.F.,.F.,aParam)
						If ( Len(aParam) >= 1 )
							aDest := aParam[1]
						EndIf
...
IF lPe01Nfe     
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,;
				aPedCom,aExp,aTotal,aCst,aIcms,aIpi,aICMSST,aPIS,aCOFINS,aCOFINSST,aISSQN,aAdi,aICMUFDest,aIPIDevol,aPisAlqZ,aCofAlqZ,aCsosn }
...
If Len(aPedCom) > 0 .And. !Empty(aPedCom[01])
	...
	//RRP - 14/11/2012 - Validação para carregar o nItemPed, empresa Intralox carrega o xPed mas não carrega o nItemPed.
	If !Empty(aPedCom[02])
...
If  !lIssQn    
	...
	ELse
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			...
			If Empty(cUltAqui) 
				SPEDRastro2(aProd[20],aProd[19],aProd[02],@nBaseIcm,@nValICM,,,lCalcMed,@nAlqICM,,,,,,,,,,,@nBfcpant,@nAfcpant,@nVfcpant)
			//RRP - 07/11/2017 - Ponto de entrada para tratamento do ICMSST
			If ExistBlock("PE06NFESEFAZ")
				aParam2 := {nBaseIcm,nValICM,aProd,aNota}
				aParam2 := ExecBlock("PE06NFESEFAZ",.F.,.F.,aParam2)
...
If len(aEEC) > 0
	...
	Next ny
EndIf 
//ER - 20/01/2012: Ponto de Entrada para tratar a Mensagem do Fisco.
If ExistBlock("PE03NFESEFAZ")
	cMsgFis := ExecBlock("PE03NFESEFAZ",.F.,.F.,cMsgFis)
EndIf
/*
...
EndIf
//AOA - 19/06/2017 - Tratamento para tag <nProc>
If ExistBlock("PE05NFESEFAZ")
	cString := ExecBlock("PE05NFESEFAZ",.F.,.F.,cString)
EndIf 
