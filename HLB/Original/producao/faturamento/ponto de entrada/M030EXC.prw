...
Parametros  : PARAMIXB
Retorno     : 
Objetivos   : P.E. na exclusão do cliente
Autor     	: Matheus Massarotto
Data     	: 05/01/15
Obs         : 
TDN         : APÓS CONFIRMAR A EXCLUSÃO. Este P.E. será executado após o usuário confirmar a exclusão; Depois da execução do mesmo, será feita a exclusão efetiva dos dados do Cliente no arquivo.
Módulo      : Faturamento.    
*/
...
if cEmpAnt $ "40"
	...
	if lHistTab
		...
		cAssunto:=' Codigo '+alltrim(SA1->A1_COD)+' Cliente '+alltrim(SA1->A1_NOME)+' excluido'
		
		
		cMsg+='<div style="text-align: center;">
		cMsg+='			<span style="color:#6633ff;font-family: tahoma, new york, times, serif">C&oacute;digo '+alltrim(SA1->A1_COD)+' Cliente '+alltrim(SA1->A1_NOME)+' exclu&iacute;do</span></div>
		cMsg+='		<div style="text-align: center;">
		cMsg+='			&nbsp;</div>
		cMsg+='		<div>
		...
		cMsg+='						<td><span style="color:#6633ff;font-family: tahoma, new york, times, serif">Exclus&atilde;o</span></td>
		cMsg+='					</tr>
		cMsg+='				</tbody>
		cMsg+='			</table>
...
IF lOk 
	conout("M030EXC--->>> E-mail enviado com sucesso, sobre alteração de cliente")
ELSE
	conout("M030EXC--->>> Falha no envio do e-mail, sobre alteração de cliente")
ENDIF
