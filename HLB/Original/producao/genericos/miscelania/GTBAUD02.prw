...
TRCell():New(oSection2,"DATAINC","","Data Inclusão"	,"@!",10)
		...
		If Len(aUsers[nI][2][6]) > 0
			For nY := 1 to Len(aUsers[nI][2][6])
				...
				aAdd(aUserSec2,{cEmpUsr,;				//Empresa
								...
								IIF(aUsers[nI][1][17],"Sim","Não"),;//Usuario bloqueado
								aUsers[nI][1][14],;		//e-mail
								aUsers[nI][1][12],;		//Departamento
								aUsers[nI][1][13],;		//Cargo
								cGrupo,;				//Grupo
								DTOC(aUsers[nI][1][24])})      //Data de inclusão
			Next nY
		EndIf
	Next nI
	...
	For nI := 1 to Len(aEmpVld)
		...
		If nPEmpFil > 0
		    ...
		    For nY := nPEmpFil to Len(aUserSec2)
				...
				oSec2:Cell("CODIGO"	):SetBlock( { || Alltrim(aUserSec2[nY][3])} )
				oSec2:Cell("USUARIO"):SetBlock( { || Alltrim(aUserSec2[nY][4])} )
				oSec2:Cell("NOME"	):SetBlock( { || Alltrim(aUserSec2[nY][5])} )
				oSec2:Cell("USRBLQ"	):SetBlock( { || Alltrim(aUserSec2[nY][6])} )
				oSec2:Cell("EMAIL"	):SetBlock( { || Alltrim(aUserSec2[nY][7])} )
				oSec2:Cell("DEPART"	):SetBlock( { || Alltrim(aUserSec2[nY][8])} )
				oSec2:Cell("CARGO"	):SetBlock( { || Alltrim(aUserSec2[nY][9])} )
				oSec2:Cell("GRUPO"	):SetBlock( { || Alltrim(aUserSec2[nY][10])} )
	  			oSec2:Cell("DATAINC"):SetBlock( { || Alltrim(aUserSec2[nY][11])})
		  		oSec2:PrintLine()
