...
Parametros  : Nil
Retorno     : Nil
Objetivos   : Alterar o parametro MV_SPEDURL do ambiente de HOMOLOGAÇÃO, de http://10.0.30.55:5159 para http://192.168.11.12:5150"
Autor       : Cesar Alves
Data/Hora   : 08/09/2020 - 01/10/2020
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
Begin Sequence
   ...
   Define Window oMainWnd From 0,0 To 01,30 Title "Atualização do Dicionário " + GetEnvServer()

   Activate Window oMainWnd ;
       On Init If(lHistorico,(Processa({|lEnd| UPDProc(@lEnd)},;
                                         "Processando... "+ GetEnvServer(),;
                                         "Aguarde , Processando Preparação dos Arquivos",;
                                         .F.) , Final("Atualização efetuada. "+ GetEnvServer())),;
                                         oMainWnd:End())
End Sequence
   ...
   If ( lOpen := MyOpenSm0Ex() )
	  ...
	  If lOpen := MyOpenSm0Ex()
	     For nI := 1 To Len(aRecnoSM0)
			 ...
			 cTexto += "Empresa : "+SM0->M0_CODIGO+SM0->M0_NOME+CHR(13)+CHR(10)
	  		 ...
	  		 For i := 1 To Len(aChamados)
  	  		    ...
  	  		    ProcRegua(1)
			    IncProc("Analisando Dicionario de Dados... " + GetEnvServer())
		 ...
		 If lOpen
			...
			cTexto := "Log da atualizacao - " + GetEnvServer() +CHR(13)+CHR(10)+cTexto
			__cFileLog := MemoWrite(Criatrab(,.f.)+".LOG",cTexto)

			Define FONT oFont NAME "Mono AS" Size 5,12   //6,15
			Define MsDialog oDlg Title "Atualizacao concluida. " + GetEnvServer() From 3,0 to 340,417 Pixel
...
aAdd(aDefine, {"MV_SPEDURL"	,"http://192.168.11.12:5150"})
...
For i:=1 to Len(aDefine)
	If SX6->(DbSeek(xFilial() + aDefine[i][1]))
		...
		Else
			cTexto += "Não foi possivel atualizar o Parametro "+aDefine[i][1]+"."+CHR(13)+CHR(10)
		EndIf
	Else
		cTexto += "Não foi encontrado o parametro "+aDefine[i][1]+"."+CHR(13)+CHR(10)
	EndIf
Next i
...
oDlg1      := MSDialog():New( 091,232,401,612,"Equipe TI da HLB BRASIL "+GetEnvServer() ,,,.F.,,,,,,.T.,,,.T. )
