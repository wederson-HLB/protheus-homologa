...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Ajuste de VLDUSER no SX3
Autor       : Jean Victor Rocha
Chamado		: 019070 e 019071
Data/Hora   : 24/06/2014
*/
*---------------------*
User Function USX3039()
*---------------------*
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
*-----------------------*
Local cTexto  := ''
Local aVldUSer := {	{"B1_LOCPAD","IIf(len(Alltrim(M->B1_LOCPAD)) == 1,(MSGALERT('Local invalido!'),.F.) ,.T.)"},;
					{"B1_TIPO"	,"IIf(M->B1_TIPO == 'AI',(M->B1_LOCPAD:='99',.T.),Iif(M->B1_TIPO == 'MC',(M->B1_LOCPAD:='00',.T.),(M->B1_LOCPAD:='  ',.T.)))"}}

SX3->(DbSetOrder(2))//X3_CAMPO
For i:=1 to Len(aVldUSer)
	If SX3->(DbSeek(aVldUSer[i][1]))
		If EMPTY(SX3->X3_VLDUSER)
			SX3->(Reclock("SX3",.F.))
			SX3->X3_VLDUSER := aVldUSer[i][2]
			SX3->(MsUnlock())
			cTexto += "- X3_VLDUSER atualizado com sucesso. '"+aVldUSer[i][1]+"'"+ CHR(10)+CHR(13)	
		Else
			cTexto += "- X3_VLDUSER já possui conteudo. '"+aVldUSer[i][1]+"'"+ CHR(10)+CHR(13)		
		EndIf
	EndIf	
Next i
