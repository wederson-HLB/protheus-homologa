...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Ajuste na SX3 dos campos CVD_CTAREF/CVD_CUSTO e na Consulta Padrão CVN2, devido ao ECD-2020 
Autor       : Cesar Alves
Chamado		: 
Data/Hora   : 17/04/2020
*/
*---------------------*
User Function USX3088()
*---------------------*
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
//Manutenção De Para - Campos ----------------------------------------------------------                          
SX3->(DbSetOrder(2))
SX3->(DbGotop())
if SX3->(DbSeek("CVD_CTAREF"))
	Reclock("SX3",.F.)
		SX3->X3_VALID   := ""    
		SX3->X3_TRIGGER := ""
		SX3->X3_WHEN    := ""
	SX3->(MsUnlock())
	cTexto += "Alterado o X3_VALID/X3_TRIGGER/X3_WHEN do campo CVD_CTAREF "+NL
Else
	cTexto += "Campo CVD_CTAREF nao encontrado "+NL
endif             

SX3->(DbSetOrder(2))
SX3->(DbGotop())
if SX3->(DbSeek("CVD_CUSTO"))
	Reclock("SX3",.F.)
		SX3->X3_VALID   := ""    
		SX3->X3_TRIGGER := ""
		SX3->X3_WHEN    := ""
	SX3->(MsUnlock())
	cTexto += "Alterado o X3_VALID do campo CVD_CUSTO "+NL
Else
	cTexto += "Campo CVD_CUSTO nao encontrado "+NL
endif  

//Manutenção De Para - Consulta Padrao ----------------------------------------------------- 
SXB->(DbSetOrder(1))
If SXB->(DbSeek("CVN2  "+"6"))
	While SXB->(!EOF()) .and. ALLTRIM(SXB->XB_ALIAS) == "CVN2" .and. ALLTRIM(SXB->XB_TIPO) == "6"
		SXB->(RecLock("SXB",.F.))
			SXB->XB_CONTEM := ""
		SXB->(MsUnlock())
		SXB->(DbSkip())
	EndDo
	cTexto += "Foi atualizado a consulta padrão 'CVN2'."+CHR(13)+CHR(10)
EndIf
