...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Remover duplicatas do SX2 da empresa Y5
...
*/  
*--------------------------------*
User Function USX2002(lAmbiente)
*--------------------------------*  
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
Local aChamados := { {05, {|| AtuSX2()}} } //05 - SIGAFAT
...
Parametros  : oProcess
Retorno     : Nenhum
Objetivos   : Remove os registro duplicados
Revisao     :
Obs.        :
*/ 
*------------------------------*
Static Function AtuSX2(oProcess)
*------------------------------*
	Local cTexto    := ''
	Local cUltimo   := ''
	Local nApagados := 0
	...
	While !SX2->(Eof())
		...
		/* Caso registro atual tenha mesma chave do anterior remove o atual */
		If SX2->X2_CHAVE == cUltimo
			SX2->(RecLock('SX2', .F.))
			SX2->(DbDelete())
			nApagados += 1
			SX2->(MSUnlock())
		Else
			cUltimo := SX2->X2_CHAVE
		EndIf
		
		SX2->(DbSkip(1))
	EndDo
	
	cTexto += AllTrim(Str(nApagados)) + ' Registros removidos'
Return cTexto