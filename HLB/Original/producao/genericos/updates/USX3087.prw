...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : Criação do campo EA_VERSAO e dois índices necessários da tabela SEA
Autor       : Thomas Moraes
Data/Hora   : 21/10/2019
*/

*---------------------*
User Function USX3087()
*---------------------*
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
AADD(aSix,{'SEA','3','EA_FILIAL+EA_NUMBOR+EA_CART+EA_VERSAO+EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA','Bordero + Carteira + Versão + Prefixo + EA_NUM+Parcela + Tipo + Fornec','Bordero + Cartera + Versión + Prefijo + EA_NUM+Cuota + Tipo + Proveedo','Bordero + Portfolio + Version + Prefix + EA_NUM+Installment + Type + S','S','','','S'})
AADD(aSix,{'SEA','4','EA_FILORIG+EA_NUMBOR+EA_CART+EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA','Filial Orig. + Bordero + Carteira + Prefixo + EA_NUM+Parcela + Tipo +','Sucurs.Orig. + Bordero + Cartera + Prefijo + EA_NUM+Cuota + Tipo + Pro','Source Bran. + Bordero + Portfolio + Prefix + EA_NUM+Installment + Typ','S','','','S'})
...
cOrdem := NextOrdem('SEA')
AADD(aSX3,   {'SEA',cOrdem,'EA_VERSAO','C','4','0','Versão','Versión','Version','Versão do Borderô','Versión del borderô','Version of Bordereau','','','€€€€€€€€€€€€€€','','','1','þÀ'                    ,''       ,''       ,''        ,'N'     ,'A'      ,'R'       ,''         ,''        ,''     ,''       ,''        ,''          ,''    ,''       ,'',       ''       ,'S'    ,''         ,''       ,'N'      ,'N'       ,'N',''      ,''})
