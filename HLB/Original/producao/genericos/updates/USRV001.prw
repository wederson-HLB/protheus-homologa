...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Alteração do campo  RV_MED13 se o conteúdo for SP para branco.
Autor       : Renato Rezende
Data/Hora   : 24/02/2014
Revisao     :
Obs.        :
*/  

*--------------------------------*
User Function USRV001(lAmbiente)
*--------------------------------*  
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
SRV->(DbGoTop())
While SRV->(!Eof())
	If Alltrim(SRV->RV_MED13) == "" .AND. Alltrim(SRV->RV_MEDFER) == "SP"
		RecLock("SRV", .F.)
		SRV->RV_MED13 := "S"
		SRV->(MsUnlock())
		cTexto += "Verba Atualizada: "+SRV->RV_COD+ NL
	EndIf 
	SRV->(dbSkip())
EndDo
SRV->(DbCloseArea())
