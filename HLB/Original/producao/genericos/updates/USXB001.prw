...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Ajuste SXB
Autor       : Jean Victor Rocha
Data/Hora   : 21/07/2015
*/                      
*---------------------*
User Function USXB001()
*---------------------*
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
SXB->(DbSetOrder(1))
If SXB->(DbSeek("SED   "+"6"))
	While SXB->(!EOF()) .and. ALLTRIM(SXB->XB_ALIAS) == "SED" .and. ALLTRIM(SXB->XB_TIPO) == "6"
		SXB->(RecLock("SXB",.F.))
		SXB->(DbDelete())
		SXB->(MsUnlock())
		SXB->(DbSkip())
	EndDo
	cTexto += "Foi atualizado a consulta padrão 'SED'."+CHR(13)+CHR(10)
EndIf
