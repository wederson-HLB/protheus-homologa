...
Parametros  : Nil
Retorno     : Nil
Objetivos   : Alteração do Parâmetro MV_PPDIFAL e MV_DIFALIQ para atender a nota técnica 003.2015
Autor       : Renato Rezende
Data/Hora   : 22/12/2015    14:20
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
*------------------------------*
Local cTexto	:= ''
Local aFils		:= FWAllFilial()//Retorna as filiais  

//Atualizando Parâmetro MV_DIFALIQ
lPar1:= PUTMV("MV_PPDIFAL","{{2015,20,80},{2016,40,60},{2017,60,40},{2018,80,20},{2019,100,0}}")
If lPar1
	cTexto += "Atualizado Parametro MV_PPDIFAL "+NL
Else
	cTexto += "Parâmetro MV_PPDIFAL não encontrado "+NL
EndIf	

//Atualizando Parâmetro MV_PPDIFAL por filial
DbSelectArea("SX6")
SX6->(DbSetOrder(1))
For nR:=1 to len(aFils)
If SX6->(DbSeek(aFils[nR]+"MV_DIFALIQ"))
	RecLock("SX6",.F.)
		SX6->X6_CONTSPA := "18"
		SX6->X6_CONTENG := "18"
		SX6->X6_CONTEUD := "18"
	SX6->(MsUnLock())
	cTexto += "Atualizado Parametro MV_DIFALIQ para Filial: "+aFils[nR]+NL
Else
	cTexto += "Parâmetro MV_DIFALIQ não encontrado para Filial: "+aFils[nR]+NL
EndIf
Next nR
