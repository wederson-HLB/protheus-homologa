...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : REINF - Apontar tabelas AutoContidas para Modelo(YY)
Autor       : César Alves
Data/Hora   : 28/01/2019
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
Begin Sequence
   ...
   If ( lOpen := MyOpenSm0Ex() )
      ...
      //Busca a Lista de Tabelas das AutoContidas, da tabela T2UYY0, e armazena no Array ------------------------------------------------- 
	  If Select("QRYT2U") > 0
	     QRYT2U->(DbCloseArea())	               
	  EndIf

	  //Verificando a Lista de Tabelas das AutoContidas   
	  cQuery := "SELECT * FROM T2UYY0 WHERE D_E_L_E_T_ <> '*' "

	  TCQuery cQuery ALIAS "QRYT2U" NEW
	
	  QRYT2U->(DbGoTop())
	  If !(QRYT2U->(!BOF() .and. !EOF()))
		  MsgStop("Tabela T2UYY0 sem registros, favor atualizar as tabelas AutoContidas da YY!","HLB BRASIL")
		  Return .F.
	  EndIf
	      
	  //Adicionando a Lista de tabelas no Array 
	  QRYT2U->(DbGoTop())
	  While QRYT2U->(!EOF())  	  
	  	  Aadd(aTabAutCon,{QRYT2U->T2U_FILIAL,QRYT2U->T2U_ID,QRYT2U->T2U_ESCOPO,QRYT2U->T2U_CODIGO,QRYT2U->T2U_DESCRI,QRYT2U->T2U_ROTINA,QRYT2U->T2U_VERSAO})		  
		  QRYT2U->(DbSkip())
	  EndDo
	  //----------------------------------------------------------------------------------------------------------------------------------       
	  ...
	  While !Eof()
		 ...
		 If lCheck
			If Ascan(aRecnoSM0,{ |x| x[2] == M0_CODIGO}) == 0 .And. SM0->M0_CODIGO <> "YY"
...
Begin Sequence
   For nLoop := 1 To 20
       dbUseArea( .T.,, "SIGAMAT.EMP", "SM0", .T., .T. )	//Exclusivo	.F., .F. 
	   If !Empty( Select( "SM0" ) ) 
	      lOpen := .T. 
...
For i:=1 to len(aSX2)
	...
	if SX2->(DbSeek(aSX2[i][4]))
	
		SX2->(RecLock('SX2', .F.))
    	SX2->X2_ARQUIVO := aSX2[i][4] + "YY0"	
		SX2->(MsUnlock())
                                    
		cTexto += "- Arquivo "+aSX2[i][4]+" atualizado para empresa Modelo!"+CHR(13)+CHR(10)
	else                                                      
		cTexto += "- Arquivo "+aSX2[i][4]+" não encontrado, nao atualizado para empresa Modelo!"+CHR(13)+CHR(10)
	endif 
	...
	T2U->(DbGoTop())	
	T2U->(DbSetOrder(2))     
	If T2U->(!DbSeek(aSX2[i][1]+aSX2[i][4]))
		lIncSX2:=.T.
	else
		lIncSX2:=.F.
	endif
	
	T2U->(Reclock("T2U",lIncSX2))
		T2U->T2U_FILIAL	:= xFilial("T2U")	
		T2U->T2U_ID		:= aSX2[i][2]
		T2U->T2U_ESCOPO	:= aSX2[i][3]
		T2U->T2U_CODIGO	:= aSX2[i][4]
		T2U->T2U_DESCRI	:= aSX2[i][5]
		T2U->T2U_ROTINA	:= aSX2[i][6]
		T2U->T2U_VERSAO	:= aSX2[i][7]
	T2U->(MsUnlock())	
	
	if lIncSX2
		cTexto += "Incluido no T2U - a Tabela:"+aSX2[i][4]+NL
	else
		cTexto += "Alterado no T2U - a Tabela:"+aSX2[i][4]+NL
	endif
...
cAux:= ""
While SM0->(!EOF())
	If cAux <> SM0->M0_CODIGO .And. SM0->M0_CODIGO <> "YY"
