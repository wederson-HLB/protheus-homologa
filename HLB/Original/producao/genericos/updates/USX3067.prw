...
Parametros  : Nil                      
Retorno     : Nil
Objetivos   : Atualização de validação do campo RA_SALARIO
Autor       : Matheus Massarotto
Data/Hora   : 24/11/2016    10:14
Revisão		:                    
Data/Hora   : 
Módulo      : Genérico
*/
...
SX3->(DbSetOrder(2))
SX3->(DbGotop())
if SX3->(DbSeek("RA_SALARIO"))

Reclock("SX3",.F.)
	SX3->X3_VLDUSER:='IF(INCLUI,.T.,IF((UsrRetGrp(,RetCodUsr())[1] $ "000033/000000"),.T.,.F.))'
SX3->(MsUnlock())

cTexto += "Alterado o campo RA_SALARIO no SX3 da tabela SRV "+NL
