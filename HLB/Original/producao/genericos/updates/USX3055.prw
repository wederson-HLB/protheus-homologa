...
Parametros  : Nenhum
Retorno     : Nil
Objetivos   : Update para ajuste dos campos de margem
Autor       : Matheus Massarotto
Chamado		: 
Data/Hora   : 03/06/2015
*/
*---------------------*
User Function USX3055()
*---------------------*
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil

Private cMessage
Private aArqUpd	 := {"SD1","SD2","SFT","SF7","CD2","SB1"}
Private aREOPEN	 := {}
Private oMainWnd
...
*-------------------------*
Local cTexto := ""
Local aCampos:= {"D2_MARGEM","D1_MARGEM","FT_MARGEM","F7_MARGEM","CD2_MVA","B1_PICMRET","B1_PICMENT"}
...
For i:=1 to len(aCampos)
	...
	If SX3->(DBSeek(aCampos[i]))
		If SX3->X3_TAMANHO < 6
			RecLock("SX3", .F.)
				SX3->X3_TAMANHO	:= 6
				SX3->X3_PICTURE := "@E 999.99"
			SX3->(MsUnlock())
			cTexto += "Campo atualizado: "+SX3->X3_CAMPO+ CHR(13) + CHR(10)  
		Else
			cTexto += "Campo não precisou ser atualizado: "+SX3->X3_CAMPO+ CHR(13) + CHR(10)  	
		EndIf
	EndIf
Next
