...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Altera o conteudo do campo X3_OBRIGAT para obrigatório e X3_USADO com o módulo estoque incluso.
Autor       : Renato Rezende
Data/Hora   : 28/02/2013
Revisao     :
Obs.        :
*/  

*--------------------------------*
User Function USX3023(lAmbiente)
*--------------------------------*  
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
                                   	
Private cMessage
Private aArqUpd	 := {"SA2","SA1"}
Private aREOPEN	 := {}
Private oMainWnd 
...
//Altera o conteudo do campo X3_OBRIGAT para obrigatório e X3_USADO com o módulo estoque incluso.
If SX3->(DBSeek("A2_PAIS"))
	RecLock("SX3", .F.)
	SX3->X3_OBRIGAT := '€'
	SX3->X3_USADO	:= 'ˆ€‘‚ð€€A€€€€€€€'
	SX3->(MsUnlock())
	cTexto := " - Campo A2_PAIS atualizado."+CHR(13)+CHR(10)
EndIf
//RRP - 14/03/2013 - Alteração para deixar os campos abaixo obrigatórios. 
//Altera o conteudo do campo X3_OBRIGAT.
If SX3->(DBSeek("A2_COD_MUN"))
	RecLock("SX3", .F.)
	SX3->X3_OBRIGAT := '€'
	SX3->(MsUnlock())
	cTexto += " - Campo A2_COD_MUN atualizado."+CHR(13)+CHR(10) 
EndIf
...
If SX3->(DBSeek("A2_CODPAIS"))
	...
	SX3->X3_OBRIGAT := '€'
	SX3->(MsUnlock())
	cTexto += " - Campo A2_CODPAIS atualizado."+CHR(13)+CHR(10)
EndIf
...
If SX3->(DBSeek("A1_COD_MUN"))
	...
	SX3->X3_OBRIGAT := '€'
	SX3->(MsUnlock())
	cTexto += " - Campo A1_COD_MUN atualizado."+CHR(13)+CHR(10)
EndIf
...
If SX3->(DBSeek("A1_CODPAIS"))
	...
	SX3->X3_OBRIGAT := '€'
	SX3->(MsUnlock())
	cTexto += " - Campo A1_CODPAIS atualizado."+CHR(13)+CHR(10) 
EndIf
