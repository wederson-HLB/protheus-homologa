...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Atendendo a desativacao de acordo com ajuste SINIEF 16/12. Usar o Evento de Cancelamento (NT 2011/006).
			  Alterar o parâmetro MV_NFECAEV para T, assim o cancelamento das Nfe são feitos por evento.
...
*--------------------------------*
User Function USX6015(lAmbiente)
*--------------------------------*  
cArqEmp := "SigaMat.Emp"
__cInterNet := Nil
...
SX6->(DbSetOrder(1))
//Alterar o parâmetro MV_NFECAEV para T, assim o cancelamento das Nfe são feitos por evento.
If SX6->(DBSeek(xFilial("SX6")+"MV_NFECAEV"))
	RecLock("SX6", .F.)
	SX6->X6_CONTEUD := "T"
	SX6->(MsUnlock())
	cTexto := " - Parâmetro MV_NFECAEV atualizado." 
EndIf
