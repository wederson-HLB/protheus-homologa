...
if cEmpAnt $ "ZR/OZ/6I/MZ/OM/JU/H9/XN/8P/69/T6/JV/I0/TP/UZ/GY/EK/KI/JG/E6/7M"//Código das empresas
	...
	ElseIf alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"745DC.2PE") 
		//if SUBSTR(SRA->RA_BCDEPSA,1,3)=="745" .OR. SUBSTR(SRA->RA_BCDEPSA,1,3)<>"745" .AND. NVALOR>=250
			lRet:=.F.	
		//EndIf
	//trata TED - AOA 03/02/2016 - Alterado tratamento para todos diferentes de 745 gerar TED independente de valor.
...
if cEmpAnt $ "PL/4J/XC/VC/IW/76/9P/GN/73/OU/G0/G8/IB/"//Código das empresas
...
if cEmpAnt $ "XY/7V" //Tegram
	
	//trata somente banco BRANCO DO BRASIL credito em conta
	if alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"001.2PE")
		if SUBSTR(SRA->RA_BCDEPSA,1,3)<>"001"
					lRet:=.F.	
		endif 
	//trata DOC/TED - outros bancos	
	elseif alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"001DV.2PE") 
		if SUBSTR(SRA->RA_BCDEPSA,1,3)=="001"
			lRet:=.F.	
		endif
	endif
...
if cEmpAnt $ "M5/SV/5M/H9/9M/KY/ZJ/8O/NE/9N/60/6T/NM/3Z/KS/6G/6U/9X/10/4W/MR/BZ/DV/FU/NX/JP/QU/I9/"//Código das empresas
	...
	ElseIf alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"755DC.2PE") 
			lRet:=.F.	
	//trata TED - AOA 03/02/2016 - Alterado tratamento para todos diferentes de 755 gerar TED independente de valor.
...
If cEmpAnt $ "VN/AY/UZ/N8/HP/I2/4V/WA/DP/46/M7/JO/6X/L1/KI/VW/3Q/H8/B8/1F/TM/40/10/4K/8F/CH/RH/Z4/Z8/ZB/ZF/ZG/H9/BW/BY/AT/"//Código das empresas
...
//AOA - 24/01/2017 - Tratamento para CNAB HSBC/Bradesco exclusivo para BUZZFEED (CNAB HSBC em ambiente do Bradesco)
If cEmpAnt $ "BI"
    
    //trata somente banco Bradesco credito em conta
    If alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"399F.2PE")
        If SUBSTR(SRA->RA_BCDEPSA,1,3)<>"237"//Contas Bradesco carrega como transferencia
                    lRet:=.F.    
        Endif 
    //trata DOC/TED - outros bancos    
    ElseIf alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"399DT.2PE") 
...
//Os arquivos CNAB na pasta system tem que ter o padrão: Kxx237F.2PE / Kxx237DT.2PE onde xx é o código da empresa
//Tratamento para CNAB BRADESCO - AOA Anderson Arrais 10/03/2016
If cEmpAnt $ "U6/DS/7M/C1/7X/JF/FG/JF/G1"//Código das empresas
    
    //trata somente banco BRADESCO credito em conta
    If alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"237F.2PE")
        If SUBSTR(SRA->RA_BCDEPSA,1,3)<>"237"
                    lRet:=.F.    
        Endif 
    //trata DOC/TED - outros bancos    
    ElseIf alltrim(UPPER(cArqent))==UPPER("K"+AllTrim(cEmpAnt)+"237DT.2PE") 
        if SUBSTR(SRA->RA_BCDEPSA,1,3)=="237"
            lRet:=.F.    
        EndIf
    EndIf
	...
	DO CASE 
		...
		CASE nOP == 6 .AND. NVALOR > 0000 .AND. SUBSTR(SRA->RA_BCDEPSA,1,3) == '399' .AND. !EMPTY(SRA->RA_BCDEPSA,1,3) //DOC
   		...
   		CASE nOP == 7 .AND. NVALOR >= 1000 .AND. SUBSTR(SRA->RA_BCDEPSA,1,3) <> '033' .AND. !EMPTY(SRA->RA_BCDEPSA,1,3) //TED
		     lCod := .T. 	
		CASE nOP == 8 .AND. SUBSTR(SRA->RA_BCDEPSA,1,3) <> '033' .AND. !EMPTY(SRA->RA_BCDEPSA,1,3) //AOA - Retirado validação de valor minimo para TED
		     lCod := .T.
