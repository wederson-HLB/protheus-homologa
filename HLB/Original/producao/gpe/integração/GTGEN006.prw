...
DEFINE MSDIALOG oDlg TITLE "HLB BRASIL. - Integração Visa Vale" FROM 00, 00 to 320, 380 PIXEL  
...
For i:=1 to len(aBenef)
	...
	If FERROR() # 0 .Or. nHandle < 0
		MsgAlert("Não foi possivel a criação de arquivo" + Chr(13) + ;
		         "       Processo será abortado        ","HLB BRASIL")
		FClose(nHandle)
		Return(.f.)
	EndIf
...
While !FT_FEof() .AND. !EMPTY(FT_FReadln())
   	...
   	If aLinha[1] == "%"
	    If (nPos:=Ascan(aInfos,{ |x| x[1] == aLinha[3]}) ) == 0
			aAdd(aInfos, {STRZERO(aLinha[9],8)/*Filial+Matricula*/, VAL(aLinha[6])* nPorc / 100/*VALORES*/  })	
		EndIf 
	EndIf    
    INCPROC("Aguarde...")
	FT_FSkip() // Proxima linha
