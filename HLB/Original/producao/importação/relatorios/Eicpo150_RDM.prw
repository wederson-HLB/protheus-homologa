...
oSend( oPrn, "Say",  Linha, 630, SA2->A2_NREDUZ + Space(2) + Alltrim(IF(EICLOJA(),"Loja:" + Alltrim(SA2->A2_LOJA),"")) , aFontes:TIMES_NEW_ROMAN_12 )
Linha := Linha+50
...
cDescrItem := SB1->B1_COD+" - "+SB1->B1_DESC
...
If cEmpAnt $ "TM"
	oSend( oPrn, "Say",  Linha, 370, TRANS(SW3->W3_QTDE,E_TrocaVP(nIdioma,"@E 999,999,999")),aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
Else
...
oSend( oPrn, "Say",  Linha,/*1250*/1510, AllTrim(SA2->A2_NREDUZ) + Space(2) + IF(EICLOJA(),"Loja:" + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
...
While !Empty(cRemarks)
   If (nPos := At(CHR(13)+CHR(10), cRemarks)) > 0
      ...
      if nPos > nTamanhoLn
         cLinha := SubStr(cRemarks, 1, nTamanhoLn - 1)
         //Imprime
   ...
   If nLinRemark == 10
      nTamanhoLN := nTamanhoLN +(nTamanhoLN/ 2)
   EndIf
