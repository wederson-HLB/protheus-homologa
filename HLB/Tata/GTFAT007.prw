...
Parametros  : Nenhum
Retorno     : Nenhum
Objetivos   : Emitir Nota Fiscal Fatura de Serviï¿½os de Comunicaï¿½ï¿½o/Telecomunicaï¿½ï¿½o - Modelo 21/22
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE OemToAnsi("Emissão Nota Fiscal Modelo 21/22") FROM 0,0 TO 300,380 OF oMainWnd PIXEL 
      ...
      @ 060,023 Say "Série:" COLOR CLR_HBLUE, CLR_WHITE
...
*--------------------------------------* 
//Declara a variï¿½vel objeto do relatï¿½rio
Private oPrint

//Cria os objetos fontes que serï¿½o utilizadoas atravï¿½s do mï¿½todo TFont()                            
...
Begin Sequence
   ...
   oPrint:= TMSPrinter():New( "Impressão de Nota Fiscal da "+Alltrim(SM0->M0_NOMECOM) )  
      
   //Pï¿½gina tipo retrato
   oPrint:SetPortrait()  
   
   //Inicia uma nova pï¿½gina
   oPrint:StartPage()   
   ...
   //Cria o Cabeï¿½alho do Relatï¿½rio
   ReportHeader(oPrint)      
      
   //Cria os Detalhes do Relatï¿½rio
   ReportDetail(oPrint)    
   ...
   //Imprime direto na impressora padrï¿½o do APx
   //oPrint:Print()   
...
Begin Sequence 
	...
	If cModelo == aItems[1]
		...
		oPrint:Say(170,1456,"NOTA FISCAL FATURA DE SERVIçOS DE COMUNICAçÃO ",oFont07,,CLR_BLACK)
		oPrint:Say(200,1610,"MODELO 21 - SÉRIE "+Alltrim(Substr(SQL->F2_SERIE,1,1))+" ÚNICA",oFont07,,CLR_BLACK)
	Else
		oPrint:SayBitmap(040,200,"\System\LGRL22"+Alltrim(cEmpAnt)+".bmp",1090,400)  
		oPrint:Say(170,1456,"NOTA FISCAL FATURA DE SERVIÇOS DE TELECOMUNICAÇÃO ",oFont07,,CLR_BLACK)
		oPrint:Say(200,1610,"MODELO 22 - SÉRIE "+Alltrim(Substr(SQL->F2_SERIE,1,1))+" ÚNICA",oFont07,,CLR_BLACK)
	EndIf    

   oPrint:Say(260,1650,"No.",oFont20t,,CLR_BLACK)
   ...
   oPrint:Say(490,1470,"NATUREZA DA OPERAÇÃO: ",oFont07a,,CLR_BLACK)
   ...
   oPrint:Say(860,1260,"MUNICÍPIO: ",oFont10a,,CLR_BLACK)
   ...
   //oPrint:Say(1020,180,"Nï¿½ DO CONTRATO: ",oFont10a,,CLR_BLACK)
   ...
   oPrint:Say(3150,685,"ALÍUOTA",oFont10a,,CLR_BLACK)
...
If cModelo == aItems[2] .AND. !(cEmpAnt) == "0F"//Imprime no modelo 22
	cMenFixo01   := "Contribuição p/ FUST e FUNTTEL 1,5 do valor dos serviços não repassados ao cliente conf. Lei no. 9998/00 e 10052/00."
EndIf     
...
nLinha := 1438
	...
	//Tratamento referente lei transparï¿½ncia  - TLM 20141223
	If SQL->A1_TIPO == "F" 
		SF2->(DbSetOrder(1))                                                                                                  
	...
	While SQL->F2_DOC <> '' .And. SQL->(!EOF()) 
		If cDoc+cSer <> SQL->F2_DOC+SQL->F2_SERIE
			//Impressï¿½o da Mensagem para nota
			...
			cSer:= SQL->F2_SERIE
			cPedido := SQL->D2_PEDIDO
			//RRP - 19/01/2015 - Ajuste para impressï¿½o de 2 ou mais notas no filtro
			If cModelo == aItems[2] .AND. !(cEmpAnt) == "0F"//Imprime no modelo 22
				cMenFixo01   := "Contribuição p/ FUST e FUNTTEL 1,5 do valor dos serviços nÃ£o repassados ao cliente conf. Lei n 9998/00 e 10052/00."
			Else
				cMenFixo01   := ""	
			EndIf
			If SQL->A1_TIPO == "F"
      ...
      If SC6->(dbSeek(xFilial("SC6")+SQL->D2_COD+SQL->D2_PEDIDO+SQL->D2_ITEMPV))               
      	...
      	Else
	         ...
	         If Len(Alltrim(cMsgCircuito)) > 104
	            nLinha+=20 
	            cMsgCircuito:=Substr(Alltrim(cMsgCircuito),85,85)+cMsgPeriodo  
	            oPrint:Say(nLinha,177,Substr(Alltrim(cMsgCircuito),105,105),oFont10,,CLR_BLACK) 
	         EndIf  
	         
	         nLinha+=20
	         oPrint:Say(nLinha,177,Substr(Alltrim(cMsgPeriodo),1,104),oFont10,,CLR_BLACK)    
	         nLinha+=20
...
Begin Sequence 
   ...
   oPrint:Line(743,340,743,2210)   //Usuï¿½rio
   oPrint:Line(823,360,823,1450)   //Endereï¿½o
   oPrint:Line(823,1610,823,2210)  //Complemento Endereï¿½o
...
Local cQry := ""
	...
	If cModelo == aItems[1]
    	oPrint:Say(490,1800," SERVIÇO COMUNICAÇÕES",oFont07,,CLR_BLACK)
	Else
    	oPrint:Say(490,1800," SERVIÇO TELECOMUNICAÇÕES",oFont07,,CLR_BLACK)
	EndIf	
    ...
    //MSM - 06/01/2015 - Tratamento para demonstraï¿½ï¿½o das parcelas,valores,datas de vencimento, Chamado: 023296 
    //RRP - 18/09/2017 - Ajustado para E1_FILORIG, empresas com o SE1 compartilhado.
	//Tratamento para as informaï¿½ï¿½es da fatura
...
If nRet > 3057
    oPrint:EndPage()   
 	oPrint:StartPage() 
  	...
  	//Molduras externas
    BoxGeral(oPrint)
    //RRP - 03/02/2014 - Ajuste para impressï¿½o de uma nova pï¿½gina mesmo que chegou no final da nota. 
...
Retorno     : Nenhum
Objetivos   : Informar abaixo dos itens a mensagems padrï¿½o e mensagems do pedido.
Autor       : Joï¿½o Dos Santos Silva 
Data        : 22/03/2013
*/    
*-------------------------------------------------*
...
Local cSubMen := ""
	...
	//WFA - 31/01/2019 - Inclusï¿½o de tratamento para utilizar a mensagem de um campo MEMO. Ticket: #2752.
 	cMenNota := IIF(cEmpAnt == "M6", SC5->C5_P_MEMOM, SC5->C5_MENNOTA)            
  		...
  		If Alltrim(SC5->C5_TIPO) <> "I"
	    	...
	    	If SM4->(DbSeek(xFilial("SM4")+SC5->C5_MENPAD)) 
	            ...
	            nLinha := ChkLinha(nLinha)   
	       		...
	       		//ALTERADO MATHEUS
	        	if len(Alltrim(cMenNota))>104 
	               		
	           		nLMenN:=0 //controla a posiï¿½ï¿½o da coluna
	          		nPosMenN:=1 //controla a posiï¿½ï¿½o de corte da msg
	               		
	            	while !empty(Substr(Alltrim(cMenNota),nPosMenN,113))                		
	            		cSubMen:=Substr(Alltrim(cMenNota),nPosMenN,113)
	            	 ...
	            	 enddo              
	               		...
	               		oPrint:Say(nLinha,180,Substr(Alltrim(cMenNota),1,113),oFont10,,CLR_BLACK)
	               		nLinha+=60
	               		nLinha := ChkLinha(nLinha)   		
	        	endif                                                                                            
	     	Else     
		...
		//RRP - 16/01/2015 - Ajuste para nï¿½o encavalar e cortar a impressï¿½So do cMenFixo01
